...
def main():
    ...
    require(
        ...
        and "maxTokens: req.body?.maxTokens ?? req.body?.max_tokens" in server
        and "Structured output truncated: finish_reason=length" in server
        and "text.includes('structured output truncated')" in server,
        "Structured skills must accept a bounded output budget and treat token-truncated JSON as fallback-eligible instead of successful content",
    )
