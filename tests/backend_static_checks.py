...
def main():
    ...
    require("def _ensure_agent_entry_visibility_smoke_ref" in agent_service_source and "文档打印首页百度网盘入口可见性短链路冒烟" in agent_service_source and '"autoGeneratedSmoke": True' in agent_service_source, "Agent must add a deterministic entry-visibility smoke YAML when generated cases lack a stable first-smoke candidate")
    ...
    require('"校验百度网盘入口可见"' in agent_service_source and 'flow.append(f"进入{label}")' in agent_service_source and 'business_flow_source = "requirement_text"' in agent_service_source, "Agent business flow must prefer explicit requirement entry chains over incidental Figma page text")
