...
def main():
    ...
    require("def _agent_generate_yaml_from_mindmap_pipeline" in agent_service_source and "generate_mindmap_from_request" in agent_service_source and "cases_to_midscene_yaml" in agent_service_source and 'draft_reason="mindmap_pipeline"' in agent_service_source, "Agent new-requirement YAML generation must reuse the existing mindmap/Figma/case pipeline before fallback")
    ...
    require("def _save_agent_yaml_draft" in agent_service_source and '"WAIT_CONFIRM"' in agent_service_source and '"generated_yaml_draft"' in agent_service_source, "Agent generated YAML drafts must always require manual confirmation")
    ...
    require('"device_strategy": "auto"' in agent_service_source, "Agent Runner jobs must opt into automatic online-device assignment")
    ...
    require(backend.MAX_BODY_SIZE == 20 * 1024 * 1024, "Default JSON body limit must be 20MB")
    require(backend.MAX_UPLOAD_BODY_SIZE >= 100 * 1024 * 1024, "Upload body limit must be explicitly larger than JSON limit")
