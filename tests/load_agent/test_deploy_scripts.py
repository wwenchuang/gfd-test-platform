...
def test_shell_scripts_have_valid_syntax_and_expected_files():
    ...
    assert "midscene-load-agent:0.1.4" in (DEPLOY / "docker-compose.yml").read_text(encoding="utf-8")
    assert "midscene-load-agent:0.1.4" in (DEPLOY / ".env.example").read_text(encoding="utf-8")
