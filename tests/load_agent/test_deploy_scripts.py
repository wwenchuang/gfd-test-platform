...
def test_shell_scripts_have_valid_syntax_and_expected_files():
    ...
    from load_agent import __version__
    assert f"midscene-load-agent:{__version__}" in (DEPLOY / "docker-compose.yml").read_text(encoding="utf-8")
    from load_agent import __version__
    assert f"midscene-load-agent:{__version__}" in (DEPLOY / ".env.example").read_text(encoding="utf-8")
