...
def main():
    ...
    workflow_stepper_block = api_testing_js[
        api_testing_js.index("function renderApiWorkflowStepper"):
        api_testing_js.index("function updateApiWorkflowStepper")
    ]
    workbench_page_block = api_testing_js[
        api_testing_js.index("async function showApiTestingDashboard"):
        api_testing_js.index("async function apiWorkbenchSelectSource")
    ]
    workbench_render_block = api_testing_js[
        api_testing_js.index("function renderApiWorkbenchPage"):
        api_testing_js.index("async function showApiTestingDashboard")
    ]
    environment_page_block = api_testing_js[
        api_testing_js.index("async function showApiEnvironmentPage"):
        api_testing_js.index("function renderApiExecutionHistory")
    ]
    api_sidebar_block = html[
        html.index('data-nav-group="api-testing"'):
        html.index('<!-- 执行 -->', html.index('data-nav-group="api-testing"'))
    ]
    ...
    for workflow, icon in {
        ...
        require(
            ...
            f"API testing workflow must use visual sidebar icons from the approved design: {workflow}",
        )
    ...
    require(
        "const selectedCount = selectedApiPlanEndpointIds().length;" in workflow_stepper_block
        and "if (selectedCount || plans.length) completedSteps.add('debug');" in workflow_stepper_block,
        "API workflow stepper must define selectedCount before marking debug progress",
    )
    require(
        "{id: 'plan', label: 'AI测试设计', handler: 'showApiPlanPage()'}" in workflow_stepper_block
        and "setApiTestingPage('api_plan', 'AI测试设计'" in api_testing_js,
        "API plan workflow must consistently display AI测试设计 in stepper and page title",
    )
    require(
        "API_WORKBENCH_CACHE_KEY" in api_testing_js
        and "readApiTestingWorkbenchCache" in api_testing_js
        and "persistApiTestingWorkbenchCache" in api_testing_js
        and "hydrateApiTestingWorkbench(data);" in api_testing_js
        and "renderApiWorkbenchPage(cached)" in workbench_page_block
        and "renderApiEnvironmentPage(cached)" in environment_page_block,
        "API workbench and environment pages must render saved local snapshots before background refresh",
    )
    require(
        "手动更新 Apifox" in api_testing_js
        and "更新接口" in api_testing_js
        and "apiAssetNoSnapshotState" in api_testing_js
        and "api-source-sync-enabled" not in api_testing_js
        and "启用定时同步" not in api_testing_js
        and "自动同步" not in api_testing_js,
    ...
    require(
        ...
        and "选择接口 → AI分析 → 调试参数 → 执行 → 报告" in api_testing_js
        and "api-runner-sidebar" in api_testing_js
        and "api-runner-env-grid" in api_testing_js
        and "api-runner-command-list" in api_testing_js
        and "api-runner-primary-button" in api_testing_js
        and "api-runner-history" in api_testing_js
    ...
    require(
        ...
        and "api-runner-advanced-links" in api_testing_js
        and "执行日志只展示关键流水" in api_testing_js
        and "高级配置" in api_testing_js
        and "接口资产 / 环境 / AI 草稿 / 报告详情" in api_testing_js,
        "API workbench default path must be a compact execution console; detailed asset/env/AI/report pages belong behind advanced links",
    )
    require(
        "手动更新 Apifox" in api_testing_js
        and "刷新接口状态" not in api_testing_js
        and "更新 Apifox 快照" not in api_testing_js
        and "立即同步" not in api_testing_js,
    ...
    require(
        ...
        and "metadata.environment_name || source.environment_id" not in runner_environment_block,
        "API workbench runner environment card must display the Apifox environment name before falling back to raw environment id",
    )
    business_auth_environment_name_block = api_testing_js[
        api_testing_js.index("function apiBusinessAuthEnvironmentName"):
        api_testing_js.index("function apiBusinessAuthProjectName")
    ]
    require(
        "apiSourceEnvironmentDisplayName(context.source" in business_auth_environment_name_block,
        "API execution environment card must prefer the Apifox environment display name before falling back to raw ids",
    )
    require(
        "renderApiWorkbenchRunnerBoard(data)" in workbench_render_block
        and "renderApiWorkbenchSourceCard(data)" not in workbench_render_block
        and "renderApiWorkbenchAssetCard(data)" not in workbench_render_block,
        "API workbench homepage must not default to dense source metrics or asset module panels",
    )
    require(
        ...
        and "api-endpoint-detail-tabs" in api_testing_js
        and "api-endpoint-ai-assistant" in api_testing_js
        and "AI助手" in api_testing_js
        and "分析接口" in api_testing_js
        and "生成测试" in api_testing_js
        and "补充异常" in api_testing_js
        and "生成断言" in api_testing_js
        and "分析失败" in api_testing_js
        and "立即调试" in api_testing_js
        and "接口定义" in api_testing_js
        and "AI分析" in api_testing_js
        and "测试用例" in api_testing_js
        and "执行历史" in api_testing_js,
        "API asset page must use the documented three-column layout with endpoint detail tabs",
    )
    require(
        "showApiDebugPage" in api_testing_js
        and "api-debug-workspace" in api_testing_js
        and "响应结果" in api_testing_js
        and "断言结果" in api_testing_js
        and "执行日志" in api_testing_js,
        "API automation must expose a Chinese online debug workspace",
    )
    require(
        "api-execution-record-layout" in api_testing_js
        and "api-execution-record-list" in api_testing_js
        and "api-execution-record-drawer" in api_testing_js
        and "Request" in api_testing_js
        and "Response" in api_testing_js
        and "AI分析" in api_testing_js,
        "API execution history must use a list plus right-side drawer with request, response, assertion, log, and AI analysis context",
    )
    require(
        "renderApiReportAiSummary" in api_testing_js
        and "api-report-ai-summary" in api_testing_js
        and "AI 总结" in api_testing_js
        and "下一步建议" in api_testing_js,
        "API reports must include a first-class AI summary and next-step recommendation section",
    )
    require(
        "showApiRegressionPage" in api_testing_js
        and "api-regression-center" in api_testing_js
        and "基线接口测试中心" in api_testing_js
        and "执行进度" in api_testing_js,
        "API automation must expose an automatic regression center instead of hiding execution behind history",
    )
    require(
        ...
        "API assets must use server-side sources and backend-tracked manual update jobs",
    )
    require(
        ("手动更新 Apifox" in api_testing_js or "重新读取 Apifox 资产" in api_testing_js)
        and "上传 OpenAPI JSON" in api_testing_js
        and "api-source-token" in api_testing_js
        and 'type="password"' in api_testing_js,
        "Apifox manual update must be the primary asset action while manual OpenAPI upload remains available",
    )
    require(
    ...
    require(
        'ontoggle="toggleApiAssetSyncLog' in api_testing_js
        and 'onchange="toggleApiAssetSyncLog' not in api_testing_js,
    ...
    require(
        "apiCandidatePlans" in api_testing_js
        and "保存为测试资产" in api_testing_js
        and "已保存测试资产" in api_testing_js
        and "API 基线" not in api_testing_js
        and "采纳为基线" not in api_testing_js,
        "Confirmed AI plans must be treated as saved test assets, not exposed as a separate API baseline workspace",
    )
    require(
    ...
    require(
        "api-asset-context-bar" in api_testing_js
        and "api-asset-workbench-grid" in api_testing_js
        and "api-asset-action-panel" in api_testing_js
        and "renderApiAssetActionPanel" in api_testing_js
        and "refreshApiAssetActionPanel" in api_testing_js
        and "资产 -> 测试设计 -> 执行 -> 报告" in api_testing_js,
    ...
    require(
        ...
        and "进入测试设计" in api_testing_js
        and "生成任务尚未开始" in api_testing_js
        and "点击“生成测试资产”后才会调用 AI" in api_testing_js,
    ...
    require(
        "renderApiWorkflowStepper" in api_testing_js
        and "apiWorkflowNextAction" in api_testing_js
        and "completedSteps" in api_testing_js
        and "api-workflow-stepper" in api_testing_js
        and "接口资产" in api_testing_js
        and "环境配置" in api_testing_js
        and "测试设计" in api_testing_js
        and "在线调试" in api_testing_js
        and "基线接口测试" in api_testing_js
        and "执行记录" in api_testing_js
        and "测试报告" in api_testing_js,
        "Every API page must expose the documented AI API Testing Studio workflow and next action",
    )
    require(
        "api-workflow-desktop-steps" in api_testing_js
        and "api-workflow-mobile-steps" in api_testing_js
        and ".api-workflow-mobile-steps" in html,
        "Mobile API workflow must use a current-step summary with expandable full steps",
    )
    require(
        "apiBusinessLineOptions" in api_testing_js
        and "api-business-line-switcher" in api_testing_js
        and "business_lines" in api_testing_js,
        "API assets must derive and expose business-line filtering from server asset facts",
    )
    require(
        "renderApiPlanScopeSummary" in api_testing_js
        and "renderApiPlanEndpointGroups" in api_testing_js
        and "groupApiPlanCasesByEndpoint" in api_testing_js
        and "apiPlanReviewFilter" in api_testing_js
        and "affected_case_ids" in api_testing_js
        and "生成测试资产" in api_testing_js,
        "API plan generation must use a concise scope summary and endpoint-grouped review",
    )
    require(
        "apiPlanReviewStateByPlan" in api_testing_js
        and "apiPlanReviewState" in api_testing_js
    ...
    require(
        ...
        and "确认后保存为测试资产" in api_testing_js,
        "API plan review must explain why review exists and how to turn AI drafts into test assets",
    )
    workflow_action_source = api_testing_js.split(
        "function apiWorkflowNextAction", 1
    )[1].split("function renderApiWorkflowStepper", 1)[0]
    require(
        workflow_action_source.index("if (reports.length)")
        < workflow_action_source.index("if (confirmed)"),
    ...
    require(
        ...
        not in api_testing_js,
        "API plan page must not render every selected-source endpoint as its default review surface",
    )
    plan_detail_source = api_testing_js.split("function renderApiPlanDetail", 1)[1].split(
        "async function confirmApiTestPlan", 1
    )[0]
    require(
        "api-case-table" not in plan_detail_source,
        "API plan detail must not flatten all cases into the legacy dense table",
    )
    require(
    ...
    require(
        ...
        and "['succeeded', 'partial'].includes(generation.status)" in api_testing_js
        and "latestGeneratedPlan" in api_testing_js
    ...
    require(
        "api-env-form-table" in api_testing_js
        and "addApiEnvironmentBaseUrlRow" in api_testing_js
        and "addApiEnvironmentVariableRow" in api_testing_js
        and "readApiEnvironmentSnapshotRows" in api_testing_js
        and "添加服务地址" in api_testing_js
        and "添加变量" in api_testing_js
        and "api-env-base-urls-json" not in api_testing_js
        and "api-env-variables-json" not in api_testing_js,
        "Apifox environment snapshot editing must use structured rows instead of raw JSON textareas",
    )
    require(
        ...
        and "active_runs: data.active_runs || []" in api_testing_js
        and "recent_runs: data.recent_runs || []" in api_testing_js
        and "查看执行日志" in api_testing_js
        and "最近执行记录" in api_testing_js
    ...
    require(
        ...
        and "renderApiBusinessAuthTarget" in api_testing_js
        and "apiBusinessAuthProjectName" in api_testing_js
        and "expected_project_id" in api_testing_js
        and "expected_environment_id" in api_testing_js
        and "expected_binding_version" in api_testing_js
        and "expected_profile_version" in api_testing_js,
        "Environment authentication writes must carry the visible binding and profile CAS identity",
    )
    require(
        "绑定业务" in api_testing_js
        and "绑定环境" in api_testing_js
        and "Token 标记" in api_testing_js
        and "api-business-auth-target" in api_testing_js,
    ...
    require(
        ...
        and "apiSourceEnvironmentDisplayName" in api_testing_js
        ...
        and "敏感值已在平台保存" in api_testing_js
    ...
    require(
        "renderApiExecutionLiveLogPanel" in api_testing_js
        and "实时执行日志" in api_testing_js
        and "发送请求" in api_testing_js
        and "收到响应" in api_testing_js
        and "断言结果" in api_testing_js
        and "api-execution-live-log" in api_testing_js,
        "Native API execution must show a readable live log panel during execution",
    )
    require(
        "renderApiDebugEnvironmentEditor" in api_testing_js
        and "saveApiDebugEnvironmentAndRun" in api_testing_js
        and "保存环境并批量调试" in api_testing_js
        and "Apifox 只读快照" in api_testing_js
        and "平台本地值用于调试和回归" in api_testing_js
        and "敏感值已在平台保存" in api_testing_js
        and "Apifox 仅返回分组名" in api_testing_js
        and "run_mode === 'debug_batch'" in api_testing_js
        and "批量调试" in api_testing_js,
        "Online API debugging must expose editable local environment values, batch debug entry, and clear Apifox/local value semantics",
    )
    require(
        "renderSavedApiSourceShelf" in api_testing_js
        and "已保存 Apifox 项目" in api_testing_js
        and "同事可直接切换已有项目" in api_testing_js
        and "api-saved-source-shelf" in api_testing_js
        and "selectApiExecutionSource" in api_testing_js,
        "API asset workspace must expose saved Apifox projects for teammates without re-entering IDs",
    )
    require(
