...
def main():
    ...
    require("function renderRerunDetail" in html and "agent-rerun-overview" in html and "原失败" in html and "AI 修复" in html and "固定设备重跑" in html and "function renderLearningDetail" in html and "沉淀内容" in html and "学习明细" in html, "Agent timeline must show task-level failure, AI repair, fixed-device rerun, and learning contents")
    ...
    require("agent-checkpoint-trace" in html and "内部执行轨迹" in html and "agentCheckpointTraceOpen" in html, "Agent internal checkpoints must remain available as an expandable audit trace")
    ...
    require("rerunProgressHistory" in html and "progress.items" in html and "agentRerunCycleMetrics" in html and "AI 修复重跑累计" in html, "Agent rerun UI must preserve cumulative task outcomes across serial and bounded repair cycles")
    require("agentRunnerProgressMetrics" in html and "jobProgressByPhase" in html and "Runner 真实执行累计" in html and "当前阶段" in html, "Runner detail must separate cumulative phase outcomes from the latest phase and timeout limit")
    ...
    require("js/execution.js?v=20260701-install-refresh" in html and "js/app.js?v=20260701-smoke-dynamic" in html and "js/state.js?v=20260714-agent-observability" in html and "js/agent-workbench.js?v=20260714-agent-results" in html and "css/app.css?v=20260714-agent-results" in html and "css/round5.css?v=20260714-agent-observability" in html and "js/agent-status.js?v=20260702-agent-artifacts" in html, "Frontend cache version must be bumped for install refresh, smoke rerun, Agent phases, artifacts, and result semantics")
