...
(async () => {
  ...
  try {
    ...
    for (const [width,height] of [[1366,768],[768,480],[390,600]]) {
      ...
      const deletion=await page.locator('[data-action="delete-recording-step"]').boundingBox();
      const evidence=await page.locator('[data-recorder-evidence]').boundingBox();
      assert.ok(deletion.y+deletion.height<=evidence.y && deletion.x>=0 && deletion.x+deletion.width<=width, `delete clipped or below screenshot at ${width}`);
