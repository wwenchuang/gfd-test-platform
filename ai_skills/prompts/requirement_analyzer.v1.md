...
11. 如果 payload 含 `requirementContract`，它是平台直接从原始需求抽取出的“覆盖分支 + 验收维度”契约，不是预设页面路径。必须为每个 `businessFlows` 分支保留其全部 `checks`，可由你判断页面层级、最短路径和自动化方式；不得把明确分支改写成“是否需要 / 待确认是否存在”。
12. `requirementContract` 之外的授权态、登录态、空态、弱网、异常处理、深色模式、横竖屏或其他设备形态，只有原始需求明确写出时才能新增为 `requirement_points`。由 Figma 画布、常见产品逻辑或你的推断发现的此类内容只能进入 `risks / questions / assumptions`，不能升级为硬覆盖点。
