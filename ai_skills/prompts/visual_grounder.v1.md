...
5. 如果视觉资料和需求冲突，保留需求和原候选分类，在 `repair_hints` 说明冲突；不得把输入 `cases` 中的自动候选移动到 `manual_cases`。
6. 如果入口不确定，保留用例并写明“入口待确认”。视觉 Skill 只做证据校准，是否转人工由后续可执行规划 Skill 结合需求、基线和全部视觉批次统一判断。
...
7. 外部 App、相册、文件、权限、优惠券、地址、真实设备状态等强依赖，要补充 `data_requirements`；已有人工候选可继续留在 `manual_cases`，但不得由单个视觉批次重分类输入自动候选。
...
10. 如果 Figma 中只有中间弹窗或局部 UI，不能把它当作完整业务路径；结合需求和页面知识仍补不齐时，在原候选上记录 `repair_hints`，交由后续统一规划，不在视觉阶段改分类。
...
- 输入 `cases` 中的记录只能在输出 `cases` 中按相同 ID 返回增量；输入 `manual_cases` 中的记录只能在输出 `manual_cases` 中返回增量。不得跨数组移动记录。
