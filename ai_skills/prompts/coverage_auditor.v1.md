...
9. 数量合理性：参考输入 payload.review.generation_targets 或 generation_targets。完整测试计划按小需求 5-8 条、中需求 10-20 条、大需求 20-50 条展开；平台当前自动化 YAML 按小需求 5 条、中需求 8 条、大需求最多 12 条收敛；超出稳定自动化范围的覆盖点应进入 manual_cases/脑图，不因首批 Runner 数量少于完整计划而判薄。
10. 可执行性：自动化用例必须具备稳定起点、清晰 UI 目标、可见断言、可清理收尾；如果缺少这些条件，应判为待修复或转人工清单。
11. 达到自动化数量上限不等于覆盖完成：如果多个 cases 重复覆盖同一需求而遗漏显式 requirement_point，应要求合并/替换重复 case，不得只看总数。
12. 显式多设备/屏幕形态需求可以由“设备无关、真实可见文字定位”的复用 case 承接当前设备 UI 检查，其他未执行形态进入完整 manual_cases；不得要求 YAML 自行选择第二台设备或使用坐标。
