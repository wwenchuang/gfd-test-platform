...
9. 数量合理性：参考输入 payload.review.generation_targets 或 generation_targets。完整测试计划按独立业务分支、前置状态、业务规则和可观察结果动态计算；3/5/8 只是 Runner 自动化候选容量。不要因为数量少于某档就判薄，也不要把相同路径上的多个 UI 检查点拆成重复用例。
