...
10. `candidatePath` 必须逐字复制候选的 `provenancePath`；YAML 候选不是 Figma，不得声称候选来自设计稿、截图或其他未提供来源。
11. 选择理由必须说明“为什么这条基线适合仿写”以及使用它的角色，而不是复述标题。
12. 如果输入包含 `selectionValidationIssues`，说明上一轮选择未通过平台分支覆盖审计；必须针对缺失分支纠正选择，不能重复原结果。
