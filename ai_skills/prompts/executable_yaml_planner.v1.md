...
3. 不要因为已达到 Runner 容量就遗漏显式需求点。`cases` 应形成覆盖最完整的最小组合；可以用一个候选同时覆盖同路径上的多个真实验收点，但不得伪造映射。
...
8. 平台按需求文档动态计算测试计划，3/5/8 仅是 Runner 自动化候选容量，不是最终可执行数量的硬下限。显式需求已由更少的独立、可执行短 case 完整覆盖时，不得为了凑数升级弱网、深色模式、系统设置、重复路径或深层授权项；在 `review` 中如实说明即可。
