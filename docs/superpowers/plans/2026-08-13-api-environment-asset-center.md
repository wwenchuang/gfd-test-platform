...
- [x] Write tests that create two projects and prove a project query returns only its stable environment assets, including counts and active revision metadata.
- [x] Run the focused tests and confirm they fail because the list/lifecycle methods do not exist.
- [x] Add repository queries and immutable summary contracts.
- [x] Add service ownership validation, status validation, idempotent archive and restore, and revision history ordering.
...
- [x] Write route tests for owner-scoped list, revisions, archive and restore responses.
- [x] Run the route tests and confirm the new routes return not found.
- [x] Add the minimal dispatch branches and payload serialization.
- [x] Re-run route tests and confirm they pass.
...
- [x] Write store tests for project list loading, history loading, archive and restore.
- [x] Run the store tests and confirm requests/actions are missing.
- [x] Add typed API contracts and minimal Pinia actions.
...
- [x] Write component tests for project switching, active/archived filters, environment selection, archive/restore and workbench navigation query.
- [x] Run the tests and confirm they fail because the asset center is absent.
- [x] Implement the project list, environment asset list, read-only detail, explicit edit mode and revision history.
- [x] Keep the existing structured editor, but show friendly service labels while preserving stable service keys.
- [x] Route “前往同步” to the existing asset sync workflow and “进入工作台” with project/source/environment context.
- [x] Re-run component and workbench tests and confirm they pass.
...
- [x] Run the full frontend Vitest suite and production build.
- [x] Run Python compilation, backend and frontend static checks, and `git diff --check`.
- [x] Review the implementation against every acceptance item in the design spec.
- [x] Update `CODEX_STATE.md` with implemented behavior, tests and the deferred task/schedule boundary.
- [x] Commit all scoped changes directly to `main` with a concise message.
