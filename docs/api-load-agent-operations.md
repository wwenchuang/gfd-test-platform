...
若升级、容器配置或宿主机变化使 Agent 上报的硬上限降低，平台会把原软上限自动收紧到新的硬上限，并要求重新校准；心跳不会因旧软上限更大而被拒绝。硬上限恢复后，平台不会擅自放大软上限，管理员可在节点页重新调整。
...
节点页每 3 秒静默读取最新心跳、校准结果和容量，不需要用户手动点击刷新。平台以最后心跳时间为准：超过 45 秒没有新心跳时，即使数据库曾保存 `online`，页面、校准入口和任务调度也会按离线处理；未回传终态的校准会明确显示“节点心跳超时，校准未完成”，不会永久停在“校准中”。
...
先部署包含实际 VU 指标校验和报告判定的新版平台后端，再更新各节点代码并运行：

```bash
LOAD_AGENT_IMAGE=midscene-load-agent:0.1.3 bash deploy/load-agent/upgrade.sh
bash deploy/load-agent/check.sh
```

不要先升级节点连接旧后端：旧后端不认识 `vu_gauge` 字段。升级保留节点凭据；查看节点版本与校准状态，如提示校准失效则重新校准，再运行已授权的低压力验收。

旧版没有采集实际 VU 的历史执行仍可查看，但不能补造实际并发证据，重新生成报告可能显示“证据不足”。0.1.3 上报每 5 秒的 VU 采样数量、最小/最大、合计、首末采样时间和最大采样间隔；固定并发判定要求节点共同时间段持续达到目标，1秒左右采样采用1.1秒容差，属于当前保守规则，尚未开放为页面策略。

阶梯压测当前显示计划阶段量，但没有精确阶段对齐证据的结果保持“证据不足”。指标聚合保留当前和前一个窗口；超出缓冲范围的迟到点会让执行明确失败，避免已上传结果被部分数据覆盖。历史已经丢失的数据不能恢复。这一版不意味着全部统计计划已经实现，真实生产容量结论必须待完整采样验收后使用。
