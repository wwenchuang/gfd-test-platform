...
- 用户于 2026-09-10 明确指定：后续真实设备测试先统一只使用 `9888E0094F2A`，直到用户明确变更。禁止使用启迪设备 `18CEDF5BA7B2` 或自动改选其他设备；指定设备不可用时停止设备相关步骤，不回退到其他在线设备。
- 此设备选择同时约束请求体、查询参数、环境变量、前置/清理步骤、基线和定时任务；历史执行证据保留原值。指定设备不等于授权任意打印文件或跳过设备状态、任务归属、取消及恢复终态验证。
