...
### 2026-08-25 API 测试平台渐进式操作体验设计

用户确认采用“默认简洁、按需展开高级配置”的方案，目标是在不改变现有数据结构和执行流程的前提下，让新用户能够完成接口选择、用例设计、前置/清理编排、变量传递、调试和基线采纳。

- 设计规范：`docs/superpowers/specs/2026-08-25-api-testing-progressive-ux-design.md`。
- 保留工作台设计/调试定位，以及独立的用例管理、任务管理、执行记录和测试报告页面。
- 添加前置/清理步骤改为先打开可搜索接口选择器，确认选择后才创建步骤；目录默认折叠，搜索命中高亮。
- 步骤列表采用单项展开，折叠摘要展示参数、断言、提取、轮询和错误状态；请求、断言、提取改为结构化编辑，原始 JSON 保留在高级配置。
- 必需变量改为带来源的可搜索多选，保持历史未知变量兼容；调试结果直接呈现执行器已有 workflow trace，并支持返回失败步骤修改。
- 不新增数据库迁移，不修改 setup/main/cleanup 执行语义、always-run 清理、轮询限制、业务断言和基线门禁。
- 当前只完成并提交设计规范，业务代码尚未开始；下一步需在用户确认规范后编写实施计划，再按测试驱动方式分阶段实现。
