...
用户提供四张当前页面证据并要求继续完整点检：2220×1258的Agent三栏右侧执行中心被截断；操作记录底部状态栏与“缺陷草稿”导航重叠；脚本编辑“去用例资产选择”点击无响应；API接口搜索连续输入不给候选机会并出现英文不可输入状态。上述四项均已登记为待真实复现和修复；视觉图、静态门禁和相邻入口不能替代对应按钮、连续输入、候选/空结果/清空、跳转/返回及窄高视口复验。
