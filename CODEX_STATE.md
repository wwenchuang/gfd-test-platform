...
用户已明确批准按设计实施。详细实施计划已写入`docs/superpowers/plans/2026-09-03-api-load-testing.md`，按15个测试驱动任务拆为持久化域、Agent控制面、场景编译与分片、Docker Agent、指标报告、AI诊断、用户API、Vue体验、部署和单双节点真实验收。当前仍只有设计与计划，生产代码尚未开始；执行时必须逐任务先看到测试因缺失行为失败，再写最小实现并运行聚合门禁。
