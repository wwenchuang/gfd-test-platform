...
### 2026-08-25 家用 API 基线：248 个唯一接口与三轮稳定性验证

本轮继续对 3D 家用接口做真实执行和生命周期编排，所有采纳用例都校验业务码、关键响应数据和清理结果，不以 HTTP 200 单独判定成功。

线上结果：

- 当前 608 个家用接口中有 248 个唯一接口具备有效基线，覆盖率 40.79%；其中 239 个为可重复定时基线，9 个标记为 `API Test / 一次性`。
...
- 重新打印和打印记录删除只作为一次性用例；执行后使用动态返回的打印任务标识完成清理。检测到设备处于打印中后，已通过动态设备与任务信息执行取消，并验证设备恢复 `ready / standingBy`。
...
- 剩余 360 个未覆盖接口中，后台接口 213 个、供应商接口 57 个；其余 90 个主要是外部登录/公众号、设备控制和打印、废弃切片/CAD 路由、缺少失败种子或历史业务数据的接口。历史成长报告已再次真实执行，但账号没有历史周数据，主体明确返回历史报告不存在，未冒充通过。
