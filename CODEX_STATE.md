...
- 接续核对确认平台仍为华为qa `10.130.4.31`（外部 `101.34.197.12:8088`），腾讯云 `43.157.42.206` 只是独立演示服务所在机器。回放静态发布修复 `0c09bd6` 与AI资源结论约束 `d8474ad` 已依次推送main并部署，线上health最终回读为 `d8474ad39d7af003d12e3daf2536c434b941b9c8`。
- 回放已真实读取 `metric.series[].points`，支持冻结负载快照、同一按钮播放/暂停、Safari切后台暂停和缺失P95不计恢复。CPU/内存绝对用量保留cores/MiB，只有百分比或同点配额换算结果参加高占用判断。Safari同时发现采样桶比执行开始早数秒时首窗显示“计划—”；本节新增回归并将计划曲线对齐到包含执行起点的首个真实采样桶，发布后需在同一报告复验首窗与播放状态。
...
- 用户指出报告“测试条件”直接显示冻结英文。报告层现将本轮历史值翻译为中文可读信息：`demo-1.0.1`显示为“演示服务1.0.1”，隔离令牌/无业务账号及原曲线复验、非业务接口/人工智能/设备命令边界均显示中文；底层冻结证据不改写。新增历史报告回归覆盖，待与回放首窗修复一同发布并用Safari回读。
