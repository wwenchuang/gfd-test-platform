...
- 生产录制严格限制固定设备 `9888E0094F2A`，设备不在线时阻止开始，不回退其他设备。录制令牌仅经页面间 `postMessage` 传递，不进入 URL，服务端只保存哈希；会话具备归属、超时暂停、事件防重与单设备互斥。
...
- 仍需完成线上发布证据：部署平台提交、在 Sonic Web 主 bundle 前加载 `deploy/sonic-recorder-hook.js`、替换并重启 Windows Runner、Safari 使用固定设备完成安全只读录制到 YAML 保存的全链路验收。
