...
用户部署`c532647`后，公开health确认线上revision为`c532647adf4e86c234bbcb26302429e76f906b66`、PyYAML=true，PID3823047，RSS/峰值402.6/701.0MiB，后台队列和重任务均为0。Safari强制刷新后继续主平台逐控件点检：执行中心空安装表单能给出设备选择提示，“不安装，直接执行”可进入调试；同步Sonic未选YAML时按钮正确禁用并能跳到资产选择；Runner待处理仍有127条，批量操作在未选择时禁用。用例资产真实选择一条YAML后，批量移动和批量删除确认均能打开并取消，未修改线上数据；Safari原生拼音输入“模型”能够进入资产搜索并筛出3条，证明该输入框的真实输入法可用。新建YAML和上传YAML点击后无可见弹窗或反馈，继续登记为待定位问题，不能计为通过。
