...
本次提交为第三批：修复只读资产按钮/编辑器、资产连续搜索丢焦、Agent需求与历史参考混淆及明确阻断被超时降级绕过、API任务零基线却显示可执行。Top3成功基线仍用于后续写法参考，用户提供的当前YAML需求不剥离；普通review/部分视觉证据不足不一律阻断。前端权限与后端分别对应ui.edit、ui.delete、ui.execute、ui.baseline及平台配置+完整UI范围，保留delete-only合法能力。独立复核无Important/Critical项。
