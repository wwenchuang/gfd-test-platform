...
## 2026-09-14 压测动画回放静态发布修复（main，待线上部署与 Safari 复验）

- 接续核对确认本地/远端main及线上health起点均为 `1b32959c6968c1b1a60cdc509cb211c2724b9f01`。平台仍为华为qa `10.130.4.31`（外部 `101.34.197.12:8088`）；腾讯云 `43.157.42.206` 只是独立演示服务所在机器，不能当作平台服务器。
- 回放源码虽已在 `e8fe765` / `30c8f03` 提交，但仓库部署静态包仍来自 `86dccd0` 的 `index-B-XFUIv2.js`，不含“压测过程回放”；组件本身另有未闭合模板字符串，证明此前没有完成生产构建，不能把代码提交或health revision当成页面已上线。
- 本批修复回放读取真实监控结构 `metric.series[].points`、运行配置缺失时使用冻结负载快照、同一按钮播放/暂停、Safari切后台暂停，以及缺失P95不能计为恢复。CPU/内存绝对用量保留cores/MiB；只有百分比或同点配额换算结果可参加85%高占用判断，未知配额明确标为不能判断。
- 新增4项组件回归；前端全量90文件/673项、生产构建、前后端静态检查、视觉巡检、阶段证据与报告定向8项均通过。已重新生成 `/api-test` 静态包，入口改为 `index-BStXQ3M4.js`，报告chunk `LoadReportsView-5VdK0BjD.js` 包含回放文案。
- 最新阶梯吞吐报告仍为 `7731abc4-4781-4f97-a9dd-8f9aa027fd18`：原1→2→4→2次/秒曲线，333 HTTP/111链路、三类失败率0，P95 1532.649ms，第三段少1次并有1个dropped iteration；服务监控峰值0.11核/91.34MiB。当前只能排除“已证实CPU/内存打满”，不能仅据绝对值断言服务资源充足。下一步保持曲线，仅提高pre-allocated VU做单变量复验，再结合演示机本地响应和隧道对照区分压力机调度、链路与服务耗时。
- 当前Safari平台会话已过期，停在登录页；生产静态包部署、真实报告回放、上述单变量复验、演示机本机对照、双节点路径和故障/保护停止仍待恢复平台及相关服务器会话后继续。未启动新压测、未注入故障、未操作任何真实设备；家用定时继续停用，唯一允许设备仍为 `9888E0094F2A`。
- 09:07线上已部署 `0c09bd6f0381b447e62ccd4a190c830e09a81853`，health及静态资源回读一致。Safari实报 `7731abc4…` 已看到11个回放窗口、开始/三阶段/降压/完成事件、P95/错误/服务CPU内存联动，阶段证据22/22、45/45、44/45；首窗计划压力显示为空暴露旧报告仅有load_goal阶段时的回退缺口，已补组件测试与回退实现，待下一提交发布后复验。
- 同次Safari实报发现v9 AI置信说明写“资源充足”，将短窗口资源观测扩大成容量结论。新增v10提示词和服务端硬校验，禁止自由结论/置信说明写资源充足、余量充足、资源正常或据此排除资源瓶颈；“不能/无法证明资源充足”等明确限制表述保留。相关AI、推荐与下一轮策略123项通过。发布后需对该报告重新诊断，确认旧v9历史保留、新v10不再越界。
