...
Task 1 持久化基础已经完成：新增11张性能测试表、`0010`迁移和事务仓库，支持场景不可变版本、运行/分片、五秒指标桶幂等写入、有序事件及有限样本。测试先以缺失模块失败；实现后又真实发现并修复PostgreSQL 63字符约束名上限。隔离PostgreSQL schema内4项仓库测试与16项迁移测试通过，迁移覆盖空库升级、重复升级和降级到base；后端静态63项及`git diff --check`通过。用户要求所有压测术语同时提供中文名称和用途/风险提示，该要求已加入前端测试与验收。Task 2起的Agent权限、注册和后续执行能力尚未实现，当前版本还不能发起压测。
