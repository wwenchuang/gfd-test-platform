...
新增回归覆盖默认家用范围、切换共享、跨范围搜索、唯一归属、无归属和歧义不自动填充，并在页面测试确认`智小白3D/家用`真实带入。完整API前端65文件489项、TypeScript/Vite生产构建、前端静态84项、25张响应式截图和diff检查通过，新构建为`index-hgVhlxVs.js`/`index-DeaUqoo1.css`。A87已以`ee54a32`提交推送；A88待提交推送和用户部署后Safari强刷复验。线上仍为`99f7592`，不能把A83-A88写成线上已解决；全平台按钮、多成员矩阵及写入/执行/清理闭环仍未全部结束。
