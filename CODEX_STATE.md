## 2026-09-24 步骤删除入口与迟到刷新修复（已部署，待 Chrome 验收）

- 用户已部署 48ed196，健康接口及线上 JS/CSS 内容一致。新增取消/失败重试后录制 UI 56 项、hook 31 项通过；三种布局和前后端必跑检查通过。用户明确仍在操作 Chrome/PHM110，暂不操作浏览器和手机，等用户交还后继续线上删除验收及第二、第三轮录制+Runner。
- 用户发现重复步骤无法找到删除；现将删除此步骤置于详情标题旁，截图上方常显。删除选中相邻步骤、清除生成结果、保留其他输入；失败保留原步骤，连续点击去重。
- 新回归真实复现删除完成后旧刷新回包复活步骤、删除迟到回包覆盖另一个会话；增加步骤版本及会话校验。自动/手动识别旧回包同样丢弃。
- 52abbf3 已由用户部署，健康 release_revision 确认一致。第一轮 Runner 报告截图已复核弹窗、取消、列表、最终首页；其余两轮仍待完成，默认触控未响应和同名删除按钮的目标歧义仍保留为限制。

# CODEX_STATE.md

## 2026-09-24 三轮实机验收进行中：识别提示时序补修

- 用户授权本次代部署，已通过 Chrome 堡垒机部署 2a9c607，health revision 核对一致；Sonic hook 同步。历史弹窗/首页原截图实际模型分别返回“确认”“首页”，不再沿用背景日期/resource ID。
- PHM110/ecbfd645 原生短触摸出现识别正确但手机未响应；保留诊断会话 deebff6e2f944dfcbf632c1f97048bdd，不能算通过。Sonic 官方“手动修复→修复触控”模式启用后点击实际生效；这是验收条件，未宣称默认触控已修复。
- 新录制 4de9e0711f374d389d3917d4c410736b：我的→打印记录→删除→取消→返回→首页，六个控件均视觉识别正确、实际页面核对；未删除业务记录。保存 3D打印基线/录制可见控件验收_20260924_1.yaml，Runner job_1790241112945_00001 首次执行成功（CLI 58.17 秒）。尚需报告截图复核和其余两轮，不能称三轮完成。
- 验收新发现 Sonic bridge 用 XML 非空判成功、识别中清空等待序号导致下一轮绿色成功提示。补修为只接受视觉/人工语义，后续轮询仍追踪最后步骤的识别/失败/无变化。新增重复轮询回归先红后绿。
- 自动识别此前依赖平台标签页轮询，Chrome 后台限频导致延迟。新增已认证 Sonic bridge 轮询触发后台识别（最多四会话、同会话去重、无无限队列），不阻塞桥接返回或手机触摸；现有 request ID 防迟到覆盖。新代码本地待完整检查/部署后再验收。


## 2026-09-24 弹窗/首页识别与自动启动展示（本地修复，三轮实机验收待完成）

- 线上历史会话 e6f961f5cc5e44d5a03bc8b582406e2e：显示第5步坐标765,1160落在删除弹窗确认按钮，保存的ui_node却是背景日期；显示第7步坐标102,2337在首页，ui_node仅有app_home_icon_container。两步都因XML非空绕过视觉识别，不是点击类型被转成launch。
- 点击/输入控件统一经截图视觉或人工确认；XML保留作诊断，不能直接命名或放行YAML。强制重新识别也不再被XML短路。模型收到点位局部图和未标注全图，以识别前景弹窗并读取标记遮挡文字；失败不回退背景文本/ID、不无限自动重试。旧生成缓存不能绕过未确认控件保存检查。
- 自动启动加入时间线第1项，仅选中时展示详情；首个点击到达自动切到点击，用户主动选中的启动项在刷新时保留。原截图、坐标和历史结果未批量改写，未修改触摸发送。
- 本地72项录制后端/协议/Windows证据测试、81项页面/Sonic hook检查通过；1366×768、768×480、390×600布局通过；Python编译、后端静态63、前端静态84、JS语法、diff检查通过。测试先复现XML短路、YAML错误放行和启动详情混排，再通过。
- 用户明确要求本次修复后跑三轮完整录制及Runner回放；不能用之前三轮或本地检查替代。当前本机未配置模型密钥，真实视觉与新版线上三轮还未通过；准备在QA现有配置下作隔离模型验证，待用户部署后Chrome/PHM110真机验收。不删除真实业务记录。
- 计划：docs/superpowers/plans/2026-09-24-recorder-visible-target.md。本次只改平台Python/JS，不要求替换Windows Runner或Sonic Agent。

## 2026-09-24 全平台审查第二轮（本地修复，交用户部署）

- 已实时核对用户部署成功：线上 `ad82912189cabe74380db1e28667c0460ed12cbe`，Windows Runner 为 `2026.09.24-midscene1.13-qwen3.7-result-retry-v1-recording-preparation-v4`。本轮不代部署或重启。
- 范围扩展至任务/Agent 生命周期、资产版本与权限、API 环境/用例/调试/通知、压测分配与全局布局。确认并修复 B01–B12：任务历史被默认50条截断、取消被迟到回包覆盖、Agent旧快照取消覆盖已完成子任务、取消反馈失真、同秒版本覆盖、批量移动部分成功不刷新、环境/密钥两次请求半成功、项目/环境迟到串页、通知保存读取竞态、用例接口加载失败隐身、压测时间基准不一致、全局工具栏小屏固定高度造成空白。
- Chrome 实际完成 YAML 新建/保存重开/历史/复制移动/批量删除，两个测试文件GET均404。非业务API项目完成环境新建/修改/版本恢复/归档；GET /demo/ok 真调试成功484ms，再将预期改201，实际200准确断言失败711ms。临时用例/执行记录归档、自动创建的临时任务删除，原隔离环境保留，未发群消息或调用业务写接口。
- API后端全套1385通过2跳过（本机缺k6真实进程清理测试），API前端699通过，类型检查/Vite构建通过；主平台权限等118项、生命周期/版本8项通过；后端静态63、前端静态84、Python编译通过。主平台完整Node/浏览器检查串行238项通过；小屏工具栏约246→80像素，真实截图核对通过。
- 已部署任务列表接口样本2.042/2.700秒，相比此前约23–25秒改善；不当作p95或长时稳定性。21个主要读取API全部成功。
- 限制：本批新代码尚需用户部署后线上复验；没有新增完整Agent真机链或录制长稳结论。Windows已执行动作目前不能保证取消中断，仅平台终态/后续步骤保护；明确列为后续高优先级。PHM110手机号ecbfd645，9888E0094F2A是打印机MAC，不能混用。
- 详细证据与剩余计划：`docs/evidence/platform-audit-round2-2026-09-24.md`、`docs/superpowers/plans/2026-09-24-platform-quality-audit.md`。本轮只需部署平台与静态资源，不用重复更换Runner或Sonic Agent。

## 2026-09-24 全平台审查第一批修复与录制准备进度（待用户部署）

- 用户要求先修复设备空闲却无法开始并提交，由用户部署，再按全平台计划继续；热修复 `94f3deb` 已推送 main。本批保持用户部署，不代为重启线上服务或 Windows Runner。
- 准备阶段明确为等待接单、采集、就绪/失败，平台/Sonic 展示已等秒数、单次 120 秒上限及手动重试；Sonic 桥接整段响应 8 秒超时，晚到/已结束会话结果忽略。Windows 录制截图优先于普通快照，设备详情缓存 30 秒但每轮检查 ADB 在线。需同步根目录 windows-midscene-runner.py 并通过原 winRunner.bat 重启；新默认版本 `2026.09.24-midscene1.13-qwen3.7-result-retry-v1-recording-preparation-v4`，未实测 Windows 提速。
- 录制心跳/识别迟到不恢复旧会话、不覆盖其他页面；刷新保持输入和展开，历史终态停止轮询。手工标签、拖动/重置点使旧识别请求失效，旧模型结果不会覆盖新标注；步骤修改/删除/重识别使旧 YAML 失效。
- 非录制：保存脚本保留请求期间的新输入；快速打开文件只展示最后一次；复制/移动/手动刷新绕过旧模块缓存；接口失败不再伪造演示资产。模型配置加载不抢页面、保存失败不伪更新已存状态、并发保存互斥、保存期间新草稿保留。失败归因优先执行证据并检查真实 teardown，避免把 WaitFor 失败误判为缺少关闭 App。
- Chrome 读取检查复现执行环境错误显示 0 台（录制页 3 台空闲）；修复为进入时读取最新 Runner，明确加载/失败、可见时刷新、离页不重绘。其余已检查入口包括模型/应用配置、资产与新建弹窗、AI 生成；未进行有副作用的发布后验收。
- 线上 21 个读取 API 通过。任务列表 GET 22.8–25.4 秒；去掉逐条重复查历史 Figma URL，单请求共用索引。本地 300 文件/20 返回行读取 12120→420 次，0.3616→0.0110 秒；生产总耗时尚未验证，jobs.json 全扫描/大结果传输仍需后续分段测量。
- 本轮专项：28 个 node:test 文件共 230 项通过；后端录制/Runner/失败归因/生成列表/审查合计 96 项通过；后端静态 63、前端静态 84、核心 Python 编译、JS 语法、diff 检查通过。1366×768、768×480、390×600 的截图/编辑/准备失败重试/历史布局检查通过。
- 全套 Python 不是全绿：原审计身份库直接置 /tmp 导致目录属主检查失败；改用用户自有临时子目录后仍有既有测试导入与同进程模块污染，代表文件隔离运行通过。Sonic 部分旧测试仍调用启动入口已迁出函数。不能把这些专项通过写成全平台完成或所有功能无 bug。
- 计划与部署/验收边界：`docs/superpowers/plans/2026-09-24-platform-quality-audit.md`、`docs/evidence/platform-audit-2026-09-24.md`。下一步由用户部署平台（默认同步 Sonic hook）并更新 Windows Runner，然后 Chrome 测准备阶段/真实录制/保存重开及任务列表延迟；Agent 主链与其他业务完整验收仍在计划中。

## 2026-09-24 开始录制错误读取旧设备缓存（待用户部署）

- 用户截图显示三台手机空闲，但点击开始提示没有空闲 Android 手机。根因是卡片及按钮使用实时 recorderDevices，startDeviceRecording 仍使用旧 runnerDevices；现开始校验复用 recorderHasReadyPhone，与显示一致。连续点击期间加创建锁，避免重复创建会话。
- 回归先复现“新快照在线而旧缓存为空”和重复创建两个问题，修复后录制 UI 42 项、前端静态 84 项、JS 语法与 diff 检查通过。覆盖新快照离线但旧缓存在线时不创建会话。脚本缓存键更新为 20260924-recorder-start-v34。
- 用户指定本次先提交、由用户部署；尚未做新版本线上点击验收。全平台审查及其他缺陷修复另行提交，不混入本次补丁。

## 2026-09-24 录制页实时设备与紧凑时间线（已部署并完成 Chrome 验证）

- 独立读取完整 `/runners` 快照，每 5 秒只更新设备区，切回前台立即读取；单请求、8 秒超时、离页停止。设备消失保留离线行，失败显示过期和灰色状态，不重绘截图和输入。
- 设置栏收紧、输入统一深色、设备紧凑行；空闲绿、占用黄、离线红、未知灰。
- 时间线面板停止拉伸隐式网格行，标题和导航按内容高度；删除详情 430px 内部滚动限制，截图及人工编辑随页面滚动。小屏截图与坐标不横向溢出。
- 按手机序列号去重，优先当前在线 Runner。线上 ELS-AN00 曾同时出现在旧 mac-runner-01 离线行和 win-runner-01 在线行，现只保留当前在线设备，避免同一手机状态冲突。
- 先复现截图截断和重复手机，再验证修复：40 项录制 UI、1366×768 / 768×480 / 390×600 Chromium 布局、前端静态 84 项、后端静态 63 项、JS/Python 语法及 diff 检查通过。
- 通过 Chrome 堡垒机部署 a3d9d65、27832fc；终端最终显示“部署完成：27832fc”，健康接口 revision 为 27832fc797a107a5500b340da823889d24b146d1，服务正常，JS 资源版本为 20260924-recorder-live-v33。
- Chrome 打开历史会话 fe5b06c1da51490c94916e3610794aa2，标题和步骤导航紧凑；选择第 3 步“打印记录”显示对应完整手机截图及红点，截图底部、坐标、重新识别和人工编辑均可见。展开编辑后多轮设备刷新仍保持展开及输入内容。最终只展示 ELS-AN00、BVL-AN00、PHM110 三部手机，均归属 win-runner-01；更新时间持续变化。未修改或删除历史步骤，未新增测试录制。
- 范围限制：每 5 秒获取服务端最新已知状态，仍受 Runner/Sonic 上报延迟影响，不承诺瞬时感知物理掉线。本轮验收针对布局和状态刷新，不把它当作录制识别或长时间回放稳定性的新增证据。

## 2026-09-24 Windows 修复后的回放稳定性复验

- QA `45019ff`、Chrome、PHM110 `ecbfd645`、win-runner-01、Midscene 1.13.1：原始完整录制 YAML 回放两次执行成功，`mime-db/db.json` 缺失不再复现。但第一轮点击打印记录后仍白屏就返回，暴露“等返回按钮出现”不能证明内容加载的验收缺口。
- 验收脚本增加打印记录标题及列表/空列表内容等待，以及首页结果检查。初版三轮 2 通过 / 1 失败；失败任务 `job_1790220405975_00005` 最后一条模型错误判断蓝色 AI 建模按钮为当前页，Chrome 截图实际已是首页。保留失败历史。
- 首页检查改为同时出现“模型系列推荐”和“模型推荐”主体内容，保留打印记录内容检查；后续任务 `job_1790220791528_00006`、`job_1790220893518_00007`、`job_1790220995622_00008` 连续三轮成功，耗时 53.41 / 55.40 / 53.65 秒。Chrome 三份报告均 11 步通过、0 失败、0 超时；Runner returncode 0、App teardown ok。验证脚本静态校验无错误/警告。
- 本次临时稳定性 YAML 已删除并 GET 复查 404，脚本及报告链接保留在 `docs/evidence/recorder-stability-2026-09-24.md`。只证实这条路径短期重复回放，不宣称所有新会话录制/长时间稳定性通过。
- 尚未解决：平台把上述 WaitFor 失败错误归因为“缺少后置关闭 App”，实际 teardown 成功；12 秒 timeout 配置的失败步骤实耗 20.30 秒，不是严格墙钟上限。生成器仍仅自动补入口等待，本轮业务结果检查只补在验收脚本。初始 Runner 心跳一次短暂超阈值后恢复，不能据本次回放排除所有心跳异常。

## 2026-09-24 录制历史应用/模块分组与批量删除（已部署并完成 Chrome 验收）

- 历史列表按应用→所属模块分组，可级联筛选、全选当前筛选结果、批量删除。进行中/生成中的会话不可选；服务端仍逐条鉴权并检查状态，部分失败保留记录、选择和错误信息。删除录制不删除已保存 YAML。
- 创建录制可选所属模块，已有历史可在详情补选；模块归属持久化，创建/修改/保存 YAML 均校验应用与模块关系。没有旧归属数据的记录明确显示“未分组”，不按当前唯一模块猜测。
- 历史 API 从默认 30 条完整会话改成 100 条分页摘要；前端读取后续页，列表不传截图、UI XML 和完整 YAML，打开详情时再加载。
- 顶部裁切由 grid 轨道压缩 + overflow:hidden 共同导致；用真实 Chromium 在旧 CSS 上复现按钮底部超出标题边界。录制页改为不压缩子项、整体滚动，窄屏单列及工具栏换行。1366×768、768×480、390×600 的按钮边界/横向溢出/筛选选择/滚动检查通过。
- 本地录制前端 34 项、服务及接口 45 项、后端静态 63 项、前端静态 84 项和 Python/JS 语法及 diff 检查通过；用户完成部署后，健康接口确认线上 revision `45019ffe1524f6711dd9360a9995e6b905335978`。Chrome 强制刷新显示完整 41 条旧记录；新建两条可丢弃记录分别归属 `3D打印基线` / `AI测试`，刷新后显示对应分组。应用及模块筛选、全选当前筛选、切换筛选清空勾选均通过。Chrome 批量删除两条测试记录提示成功，API 复查测试 ID 均不存在，原 41 条保留；顶部标题和新建/返回按钮完整显示。
...
- Chrome/PHM110（手机序列号 `ecbfd645`）新会话 `967f4899534d4946886fdff451089555` 中，首次点“打印记录”被记录但手机未跳转，平台明确提示画面未变化，该无效测试步骤已删；第二次点文字实际进入打印记录页，左上角返回实际回“我的”，点“首页”实际回首页，后 3 步分别识别为“打印记录”“返回按钮”“首页”。尚未结束生成及真机回放，不能称作完整录制验收。
- 同一会话 Sonic 已恢复并继续同步步骤，平台仍显示旧的 `Failed to fetch`。原因是 Sonic 桥接失败只向平台发错误，成功轮询没有恢复事件，平台的旧错误状态也不会被已绑定会话的心跳清除。现增加带会话 ID 的桥接恢复通知，仅清除该会话的桥接错误；其他动作错误保留。Chrome 上需发布后重开会话复验。
- 截图中红点 `(323,764)` 映射手机原始 `(974,2304)` 落在底部“我的”；旧的“AI建模”是视觉命名错误，不能据此判断触点偏移。旧会话在上一轮线上点击“用当前红点重新识别”后得到“我的”并显示 100% UI 结构证据；新会话仍需独立验证截图、名称与实际跳页。
- 本地 Sonic/前端录制 57 项、后端静态 63 项、前端静态 84 项、核心 Python 编译、JS 语法及 `git diff --check` 通过。下一步发布 QA，完成 Chrome 新会话、结束生成、保存重开和 Runner 真机回放；真实打印记录删除仍等待操作时确认。

## 2026-09-24 录制红点与控件名称错配修复（待线上真机验收）

- 用户截图中第 4 步红点在底部导航“我的”，页面却显示“AI建模”。复现了 1080×2412 手机坐标 `(974,2304)` 缩放到 358×800 截图 `(323,764)` 后，直接用截图坐标匹配原始 UIAutomator XML bounds，导致命中错误节点；现匹配 XML 前先映回手机原始坐标。拖动红点后的重新识别也重新读取对应 XML，不沿用旧名称。
- 没有可靠 XML 时，重新识别清掉旧语义并把红点局部图放在模型输入首位，缩小低清晰度截图的裁剪范围；页面会提示识别中、识别到的名称或失败原因，阻止同一步重复请求。模型名称仍需与红点和手机实际目标页核对，不能把视觉推断当作手机点击生效的证据。
- 本地录制服务 39 项、页面 28 项、后端静态 63 项、前端静态 84 项、主链 Python 编译、JS 语法与差异检查通过。需部署 QA 后用 Chrome/PHM110 新会话实测“我的→打印记录→返回→首页”、截图坐标和 UI 文本、结束生成与真机回放。Windows Runner 的触摸设置优化与 `mime-db/db.json` 依赖状态仍需在 Windows 主机复核。

## 2026-09-24 Chrome 空闲录制误报与证据刷新复核（待 QA 二次部署验收）

- QA `05a7f01` 已发布，Chrome PHM110 会话 `5611fffffd5d471fbc0a9128e8d6a1c4` 真实复现：远控显示“可以开始操作”，空闲约 1 分钟后点击“打印记录”，Sonic 却提示“录制证据尚未准备，本次操作未记录”，步骤计数仍为 0。页面当时仍停在“我的”；不能宣称录制成功。
- 根因是桥接在截图满 45 秒时把 `ready` 直接改成 `pending`，而 Sonic 只每 15 秒轮询一次；两侧状态会短暂相反。现空闲刷新期间保留未过 60 秒的旧截图，并单独追踪刷新请求；到 60 秒仍未收到新截图时明确变成 `pending`，过期动作拒绝且不保存伪步骤。Sonic 改为 3 秒轮询并独立检查截图时间戳，避免沿用过期的“可以开始操作”。刷新失败直接报错。
- Windows Runner 原每次证据请求都重复执行两条开启触摸指示的 ADB 命令；同一进程同一手机改为成功后只执行一次，减少空闲刷新和逐步采集的额外串行等待。Runner 更新仍需在 Windows 主机实际替换才能生效，服务端部署不能代替。
- 新增 45 秒刷新窗口内真实动作仍可采集、60 秒过期阻断并等待新截图、刷新失败不误报 ready、Sonic 过期拒绝及触摸指示只执行一次的回归。录制服务/YAML/Runner 50 项、Sonic 钩子 27 项、后端静态 63 项、前端静态 84 项及 Python/JS 语法检查通过。还需 QA 再部署后的 Chrome 真机录制和生成/回放复验，不能把本地检查视为功能验收。

## 2026-09-24 Chrome 录制证据和卡顿复核（本地修复，待发布与真机复验）

- Chrome 在 PHM110（手机序列号 `ecbfd645`）会话 `b3c0e8b4e742436890d5ca859e0dffc3` 复现：第一次点智小白图标虽记录但未打开，标签误为“小布建议”；第二次打开但误标“高德地图”；点“我的”实际跳转却误标“AI建模”（截图红点确实在“我的”，模型仍报 95%）；“打印记录”和返回实际跳转；连续两次“首页”被记录为该控件但画面未变化、手机停留“我的”。已记录的点击、语义识别、真实导航需分开验收。
- 查明 Sonic 钩子在原触摸送达后，仍同步从 `scrcpy-video` 绘制和 PNG 编码手机帧。该帧仅 358×800，后端优先使用它并跳过 Windows Runner 的 1080×2412 截图及 UI XML，既可能阻塞浏览器主线程，也丢失确定性节点文本。钩子改为只镜像动作，证据统一来自已准备的 Runner 截图和 UI XML。回归先复现 draw/encode，再验证不触及画布。
- 旧的 Runner 缓存截图 5 秒即判过期，但 Sonic 会一直提示“可以开始操作”；切换浏览器标签页就可能超过 5 秒。现放宽有效期至 60 秒，空闲 45 秒时通过轻量桥接轮询请求刷新；10 秒后点击和自动刷新均有先失败、后通过的回归。画面未变化的点击仍保留草稿，但生成结果标记需确认且不可直接调试。
- 本地 Sonic 钩子专项 26 项、录制后端 34 项和录制 YAML 9 项通过；仓库要求的 Python 编译、后端静态 63 项、前端静态 84 项、JS 语法和 diff 检查也已通过。尚需提交发布、Chrome 新会话真机复验；旧会话及临时 YAML 测试资产需要在验收后清理。Windows Midscene CLI 缺失 `mime-db/db.json` 的回放阻塞仍需单独核实，不能计作录制通过。
...
- 同一会话服务端记录的 Runner 原始截图为 1080×2412，Sonic 当前帧为 358×800，Sonic 发出的原始触摸点均为 `(65,154)`。原服务未设置 `pre_action_frame_coordinate_width/height`，把原始坐标直接用于低分辨率视频帧，红点偏到列表。新增回归先复现 `(65,154)` 未换算，再在浏览器帧优先时用 Runner 原始尺寸作为触摸坐标系，映射为 `(22,51)`，并保留原始点和变换信息供人工核对。Runner 截图尺寸不存在时保持原坐标，不猜测缩放。
- 本地录制后端 31 项（1 跳过）、必跑 Python 编译、后端 63 项、前端 84 项和 diff 检查通过。仍需部署本次修复，Chrome 真机重新录制并验证截图红点落在返回按钮、语义正确、手机实际跳转、YAML 可生成及回放。现有错误历史记录不会自动改写。

## 2026-09-23 Sonic 第 3 步误报与远控画质（本地修复，待部署和真机复验）

- Chrome 真实录制会话 `cad94607bf614492bcc986adad5848c3` 使用 PHM110（手机序列号 `ecbfd645`）和应用 `com.kfb.model`；第 3 步点击坐标 `(154,1774)` 位于 UI XML 的“打印记录”节点 `[72,1700][228,1837]`。点击被平台识别，但手机未跳转到打印记录页；不能把“控件识别成功”等同于“手机点击生效”。`9888E0094F2A` 是业务打印机 MAC，不是手机。
- “先识别失败、又显示成功”源于 Sonic 桥接只检查 `semantic_description`，而平台也接受 UI 节点文本。桥接现与平台一致接受节点文本/描述/resource ID，并把步骤提示改成“控件已记录，页面结果请核对”。新截图与点击前 UI XML 完全相同时，服务端标记“页面结构未变化”，Sonic 与时间线显示待核对警告；不自动重试手机点击，以免删除等操作被重复执行。XML 比较只是页面结构信号，不能证明所有视觉页面变化或触控送达。
- 原始 ADB 截图清晰，模糊在 Sonic 远控视频链路。官方 Sonic Agent v2.7.2 启动 scrcpy 1.23 时未传 `bit_rate`，使用其 8 Mbps 默认值。新增只把码率调到 16 Mbps 的源码补丁与 Windows Agent 构建/备份/替换说明。补丁已针对官方对应源码文件做 `git apply --check`；本机下载完整源码压缩包连续超时，尚未构建 Windows JAR，更未替换 Agent 或做画质/延迟真机对比。
- Windows 主机反馈未安装 Git 且 PowerShell 命令被聊天界面改写为 Markdown 链接。构建说明已补充无需 Git 的官方 v2.7.2 ZIP 路线，以及 JDK 17/Maven 环境检查、精确源码替换、构建成功门禁和按时间戳备份；Windows 上的构建与替换仍未实际执行。
- 录制后端专项 25 项、前端专项 21 项、Sonic 钩子专项 16 项本地通过。仍需完成必跑静态检查、提交发布及 Chrome 真机重放“我的 → 打印记录 → 返回首页”；特别核对真实页面是否跳转，若仍未跳转应检查 Sonic Agent 的触控送达或使用其触屏修复模式对照。
...
- Chrome 会复用同名 Sonic 标签并把只有 fragment 变化的地址当成同文档导航；每个录制会话因此改用独立窗口名，钩子同时监听 `hashchange`。无论新开页面还是复用已打开页面，都会消费并清除交接 fragment。
...
- 选定应用后，步骤时间线固定显示第 1 步“启动应用”，包含应用名称和包名并标记“自动加入 YAML”；真实手机动作从第 2 步开始编号，历史记录步数包含该自动启动步骤。自动启动不作为真实录制动作写入会话，因此仍不能用只有启动步骤的空录制生成用例。
- 录制 YAML 生成器强制将会话所选 `app_package` 作为 `flow` 第一条 `launch`，同时移除录制动作中重复或错误包名的其他 `launch`，保证只启动用户选择的应用。
- 修复 Sonic 物理触控坐标与 ADB override 截图尺寸不一致：Windows Runner 上报物理坐标尺寸，后端读取 PNG IHDR 并把点击/滑动映射到截图坐标，同时保留 `raw_point` 和转换说明。截图红点现在严格以图片容器定位，避免被时间线网格拉到图片右侧。
- 截图红点可直接拖动校正；松手后保存截图坐标并清除旧语义，用户可点击“用当前红点重新识别”，也可“重置点击位置”恢复录制时坐标。视觉模型始终读取同一张原始手机截图和当前校正坐标。
- Sonic 收到识别失败结果后会释放当前动作门禁，明确提示步骤已保留并允许继续操作；平台仍只在截图、语义和下一张点击前画面均就绪时提示“记录成功”。
- 录制中可以取消；暂停或生成卡住的会话也提供取消入口。取消后保留审计记录并可立即新建。已取消、已暂停和已结束记录可从历史卡片或详情删除整条会话及截图证据；录制中/生成中必须先取消，且继续校验记录归属。
- Windows Runner v3 采集录制证据前开启 Android `show_touches` 和 `pointer_location`，便于 Sonic 现场观察；命令失败不影响真实操作与证据主链路。
- 专项 Python 38 项和前端/Sonic 30 项通过；后端静态 63 项、前端静态 84 项、Python/JavaScript 语法与 `git diff --check` 通过。真实 Chromium 已验证：红点落在截图内、拖动后从 `(976,2331)` 变为 `(765,2119)`、按新坐标重新识别、重置回原坐标、历史整条删除后回到新建状态。线上部署和 Windows Runner 替换后的真机闭环仍需完成。

## 2026-09-22 Sonic 录制空帧根因与点击前证据门禁

- 线上失败会话 `cdafced84a8e43998178bba6c8c3148b` 的 4 个点击步骤虽然都标记为已采集，但截图均为同一 SHA，文件只有 1572 字节；这证明 Sonic Web 画布读取取得的是无效/空 WebGL 帧。连续 `canvas.toBlob()` 还会干扰 Sonic 视频渲染，符合用户看到第 4 步后页面崩溃重载的现象。用户点击快慢不是这 4 步全部识别错误的根因。
- 删除 Sonic 浏览器画布截图和内联 Base64 上传。Windows Runner 在每个动作前通过 ADB 采集真实 PNG 与 UI XML；平台只有在点击前画面就绪后才向 Sonic 发送 `MIDSCENE_RECORDING_BOUND`，收到动作时原子复制该画面作为本步证据，再请求下一步画面。
- 下一张点击前画面未准备时，后端拒绝动作入库；Sonic 仍先把触摸交给手机，不拦截真实操作，但明确提示“手机已执行、本次未记录”，且不会生成伪步骤。上一动作未确认时同样不记录后续动作；只有证据完成、语义识别成功且下一张画面就绪，才提示“记录成功，可以继续操作”。识别失败只提示一次并保持阻断，人工修正后会重新通知 Sonic 成功。
- Sonic 页面刷新后继承会话会强制回到未绑定状态，必须重新完成 Runner 画面握手；握手前发生的动作不会在稍后回放。ADB 截图失败按 3 秒退避最多重试 3 次，超过后保留明确失败状态，避免无休止请求。
- 新增回归覆盖：点击前画面未就绪拒绝、失败限次重试、连续动作不复用截图、空 step_id 的 Runner 取证、刷新后重握手、握手前动作不回放、未确认动作不重复入库、人工修正后解除阻断，以及 20 次确认点击不读取画布且不丢 Sonic 手机命令。录制后端/Runner 23 项、录制前端/钩子 25 项、后端静态 63 项、前端静态 84 项、JS/Python 语法与 diff 检查通过；真实 Chromium 验证失败提示、历史入口和零伪步骤正常渲染。
- 全量 `tests/visual_smoke_check.js` 仍在未改动的 Agent 应用名断言失败：`Cold Agent entry lost its configured application name`，与本批录制文件无差异关系，未把该失败隐去。当前改动尚未提交/发布；发布后必须用 Chrome 和真实手机验证至少“点击我的 → 打印记录 → 左上角返回”，逐步核对截图尺寸/SHA、红点坐标、识别语义、真实手机响应、Sonic 不重载、结束生成 1.13 YAML、保存后重开。完成这组线上证据前不能宣称录制已修复。

## 2026-09-22 录制记录固定入口

- 操作录制页原先只在接口成功且记录数量大于零时渲染折叠的“录制记录”，接口异常被静默转为空列表；后端又过滤已取消会话，与“取消后保留在审计记录”的页面提示冲突，导致历史真实存在但用户没有入口。
- 页面顶部改为始终可见的“录制记录”按钮并显示数量；历史使用独立卡片页，展示状态、应用、时间、手机、步骤数和 YAML 生成状态。点击记录恢复步骤时间线、截图/坐标、人工标记和已生成 YAML；空记录、加载中、读取失败及重试都有明确状态。
- 本人创建的已结束、录制中、暂停和已取消会话现在都会进入历史列表；访问权限仍按创建人隔离，单条详情继续由后端校验归属。
- TDD 覆盖取消记录可见、固定入口、失败重试和历史详情恢复。真实 Chromium 已渲染并点击入口，确认已结束/已取消卡片可读；录制前端与钩子 21 项、录制后端与 Runner 18 项、前端静态 84 项、后端静态 63 项及 JS/Python 语法、diff 检查通过。线上数据展示仍需发布后读取实际记录验收。

## 2026-09-22 Sonic 录制不得阻塞真实触摸

- Chrome 真机录制时连续点击左上角返回未实际返回。代码回溯确认，录制钩子在 Sonic WebSocket 的 `touch down` 发送后仍于同一个同步调用栈执行大画布 `canvas.toDataURL()`；主线程编码期间会延迟后续 `touch up`，因此平台虽记录了点击，手机却可能没有完成一次真实触摸。
- 录制证据改为完全异步旁路：真实 `down/up` 始终先通过原生 WebSocket 发送；点击前画面使用异步 `canvas.toBlob()` 和 `FileReader` 处理，证据完成后才上报平台。截图不存在、浏览器不支持或编码失败时继续上报动作且不影响手机操作，由既有 Runner 证据降级链路补充截图。
- 新增触摸顺序回归，明确禁止调用同步 `toDataURL()`，并验证截图回调完成前平台不发送证据、真实 `up` 已经发给 Sonic。钩子/录制前端 19 项、录制后端与 Runner 18 项、前端静态 84 项、后端静态 63 项、JS/Python 语法及 diff 检查通过。
- 本地自动检查不能替代线上真机验收；发布后需在 Chrome 刷新 Sonic 设备中心和远控页，单击一次左上角返回，确认手机立即返回且平台随后显示该步骤识别完成。

## 2026-09-22 鸿蒙手机录制证据降级与视觉语义识别

- Windows Runner 的录制证据改为分别采集截图和页面结构：鸿蒙设备无法导出 UI XML 时仍上传点击瞬间截图及结构采集错误，不再把已成功截图误报为整步失败；Runner 版本升级为 `2026.09.22-midscene1.13-qwen3.7-result-retry-v1-recording-evidence-v2`。
- 平台保存截图后，页面结构能命中控件时继续使用节点文本；结构不可用或坐标未命中时，通过视觉模型结合原始手机截图与点击坐标识别“底部导航「我的」”一类语义，并记录来源、置信度及失败原因。生成 YAML 前必须等识别完成；识别失败必须人工补充步骤说明。
- 新增 `/api/device-recordings/recognize` 用户接口和录制页面自动轮询；时间线明确显示证据采集、自动识别、视觉置信度和人工补充状态。
- 专项 17 项后端/Runner 测试及 10 项录制前端测试通过，Python 语法检查通过；完整静态检查、提交发布、Windows Runner v2 替换重启与 Chrome 真机闭环仍待执行，未宣称线上录制已恢复。
- 线上 `9340e0b` 与 Windows Runner v2 发布后，Chrome 使用 PHM110 完成真实动作旁路：Sonic 显示已同步 1 步，平台收到截图并以视觉识别得到中文控件名及 95% 置信度；结束后生成的 1.13 YAML 含 `launch: com.kfb.model` 与语义化 `aiTap`，通过基础校验，保存到 3D打印基线并重新打开核对。验收同时发现会话开始后禁用的应用下拉错误显示第一项，已增加按会话包名回填与结束状态文案修复，待提交发布后再复验显示。
- 用户复核指出进入页面仍停留在上一次完成会话，且生成结果刷新后丢失。录制会话现持久化生成结果；进入功能默认回到新建状态，并提供当前账号专属的折叠“录制记录”，可按完成时间、手机、步数和生成状态重新打开；完成页另提供“开始新的录制”。
...
- 两个入口都增加“管理当前应用模块”，可直接进入既有应用配置；应用配置保存后既有 `renderModules()` 会同步刷新选择数据，无需整页刷新。创建的默认 YAML 仍按所选应用包名生成 launch。
- 新建和上传请求增加 `app_package`，`POST /api/file` 在收到应用上下文时重新读取服务端应用配置，校验应用存在、启用且模块确实归属该应用；普通编辑保存保持原兼容行为。
- 新增/更新回归覆盖应用隔离、单模块自动选择、停用应用隐藏、手工 YAML 携带应用包名及后端归属拒绝。专项前端 9 项、应用配置/录制相关合计 40 项、后端归属与访问控制 54 项、后端静态 63 项通过；完整前端静态首次因缓存键必须保留历史功能前缀而失败，已恢复原前缀并追加本次版本，待最终重跑。
- 尚待提交、发布及 Safari 线上验收；验收产生的临时用例/模块须清理。用户 Word 文件和 `docs/career/` 保留未动。

## 2026-09-21 Sonic 录制跨标签页断点修复

- Safari 实测平台已创建录制会话但始终为 0 个动作；Sonic 设备中心正常显示三台空闲手机。对线上 Sonic 2.7.2 分包 `assets/index.a6cf1032.js` 核对后确认，Android 控制通道路径为 `/websockets/android/{secretKey}/{udId}/{token}`，原路径识别规则正确。
- 根因是 Sonic 设备中心点击“马上使用”使用 `_blank` 新开远控标签页：录制会话仅存在于设备中心标签页，新页的 `window.opener` 又指向设备中心而非平台。远控 WebSocket 因而无法取得会话并通知平台；平台还把消息来源严格限定为设备中心窗口，远控子窗口即使上报也会被丢弃。
- Sonic 桥接现通过同源 `sessionStorage` 的新标签页继承机制传递短期录制会话，沿 `远控页 → 设备中心 → 平台` 通知实际设备；平台校验 Sonic 来源和会话 ID 后绑定，并把确认消息回给真实远控页。绑定确认前最多暂存 20 个操作，确认后按原顺序补传，避免第一个点击丢失。
- 新增两个回归：远控子标签继承/设备识别/首操作排队，以及平台接受子标签绑定并回发确认。录制专项 9 项、前端静态 84 项、后端静态 63 项、两个 JS 语法与 diff 检查通过。尚待提交、发布及 Safari 真实手机端到端验收；在出现真实动作、结束、生成并保存 YAML 前不能宣称录制可用。
- 用户 Word 文件与 `docs/career/` 保留，未纳入修改。

## 2026-09-21 应用内直接新增模块

- “配置 → 应用配置 → 编辑应用 → 关联模块”新增“在当前应用下新增模块”：输入名称后调用现有模块创建接口，创建成功立即加入模块列表并自动勾选；保留当前应用原有勾选，点击“保存分组”后只关联到当前应用。重复输入已有模块时不重复创建，直接勾选；平台临时目录名称被拒绝。
- 修正关联模块筛选栏的复选框被通用输入样式拉成大白方框的问题；“只看未归属模块”恢复为18×18小勾选框，并保持文字同行。新增模块区在窄屏改为上下排列。
- 应用配置专项23项、前端静态84项、后端静态63项和diff检查通过。Chromium本地真实渲染核对复选框18×18、新增区648×38且布局正常；尚未部署，Safari线上验收待发布后进行。三份Word与docs/career保留，未纳入提交。
...
- 保留单设备执行合同与隐藏select数据源，卡片采用原生单选语义和勾选视觉；整卡点击及方向键切换同步实际runner_id/device_id与草稿状态。已异步询问是否需要多设备分别执行，尚无回复，当前按既有单设备语义实现，不能表述为多选分发。
- 刷新后固定设备消失时保留原选择并返回manual_required，提示离线不可用，不回退自动分配；无在线设备时自动选项禁用。元数据按文本转义，缺失应用版本明确显示未上报。
- DOM行为与既有表单回归10项、前端静态84项、后端静态63项、两个JS语法和diff检查通过。Safari本地模拟页实点两款手机，方向键切至OPPO、展开连接详情、模拟离线后显示阻断提示；初次视觉发现旧label宽度挤压，已修正为全宽并复验。没有发真实设备任务。静态资源版本追加新后缀，保留既有发布检查要求的前缀。
- Safari平台机终端已退回华为堡垒机登录页，已请求恢复10.130.4.31终端；本批尚未部署，本地验收不等同线上发布。用户Word及晋升资料保留。

## 2026-09-21 每日基线修正与切片条件恢复（原四项已验收，CAD慢任务待解决）

- 开始时本地/远端 main=010c873，线上 health=25ea12a。Safari 登录恢复后实读今天 08:00:18 家用 `a25e9fb0-a38b-4f91-b082-48db10239423` 297/297，0失败/异常/跳过，202.18秒历时、197.06秒累计；昨天 `aa7dd948-1cf9-4376-936a-53d27fe3787a` 296/297，唯一失败仍是成长报告强制 data=null。今天通过不代表已有前置缺陷修复。
- 成长报告接口定义明确：当前自然周无已完成打印任务时 data=null。昨天真实返回 09-14至09-20报告对象，今天09:29响应 code0/data=null；用户补充周报每周一发送，发送任务的时点与幂等性尚未独立验收。已改为“获取当前周成长报告 - 当前周响应契约”，保留HTTP200、code0、data存在，新增null或完整报告对象结构断言（周区间、进度、用户信息、路径）；61ms、66ms两次生产调试4/4通过，版本 e47ac944-b7f7-44ac-942e-858dd3469686已采纳。首次pattern断言被平台严格拒绝，改为受支持字段/类型/长度/数值约束后保存，未放宽业务码。
- 组件详情固定标识曾下架又恢复。已改“查询组件详情 - 固定设备机型动态组件”：GET devices/info精确校验mac=9888E0094F2A、业务SN=1385270626257522688并提取EP3；model/page查询当前内容；model/detail按同机型提取组件并校验模型SN；主体精确校验组件SN、模型SN和完整结构。真实取到模型1551291050489229312、组件1551280979530846208，非再次替换成固定常量。加强版402ms、408ms两次通过，版本5e4dcf57-419d-415f-9687-c7dbc1f9dbb1已采纳。两项均为已有基线版本修正，不计新增数量。
- 代码补齐功能用例前置/清理 only_if_variable：本轮变量无值时跳过且记录未发送请求；0/false保留为有效值；未配置条件的历史流程不变。条件变量必须有本轮声明来源，预览同步校验；打印取消条件绑定必需且请求实际使用的本轮任务变量。压测复制拒绝任何条件步骤（含停用），不丢弃保护字段。中文条件编辑/摘要与跳过结果显示已实现。
- 本地162项后端专项、46项前端专项、类型检查/构建通过。独立代码审查指出的取消条件误绑定与停用条件导入丢保护两项已补测试并修复，复核通过。Safari本地模拟页确认条件摘要、清空条件、跳过提示和无响应字段；模拟验收未发送设备请求。
- ad4c81f已提交并推送main。Safari平台机10.130.4.31第一次标准发布在GitHub拉取后因堡垒机T_514断连中断；后台重试明确GitHub443连接超时，尚未安装。核对服务器origin/main完整SHA=ad4c81fbf44ec3ef884a38860db0a3e8cea180d1且工作区干净后，以服务器已拉取对象建立/tmp临时裸仓库作为本次REMOTE，继续原update-main-server.sh全部流程；未改origin配置。公网health确认同一SHA，前端入口index-DZv2f-S9.js，Safari已见中文执行条件字段。
- 两条打印基线已修正并采纳：固定设备身份/可恢复状态→currentSliceJob→有任务时详情核对设备与任务SN→条件取消→确认切片清空→在线待机→核验已批准种子1510261518949687296为completed→创建重打→按本轮任务查询、取消、恢复待机、清理。无本轮任务时所有打印清理条件跳过。第二条取消/删除恢复严格code0，取消列表首条历史作业的不确定选取，选中及恢复严格限制固定设备。
- 核心打印定向816ms、773ms通过，13步骤（无切片时3条件步骤明确跳过），最终版本074d383c-561d-4af6-9b49-f82e6812ae74已采纳；另一条935ms、931ms通过，17步骤，最终版本2bd6a3a0-6f5e-4a10-934c-abd18ab479b5已采纳。任务详情真实取消状态为aborting，另以设备ready/standingBy独立证明恢复，不把该字符串单独描述为物理终态。
- Safari线上预览证明无切片时显示“条件未满足，已跳过”“未发送请求”。另10:14:57完成受控有任务分支11步预览：用相同批准种子创建一次任务，强断言currentSliceJob.sn等于本轮创建SN，再核对归属、取消、切片清空、设备待机、任务状态及删除本轮记录，最后code0/data=true。该受控造数只用于本次预览，未采纳进每日基线；已恢复并保存正式配置。首次预览因重复步骤名被静态拒绝，改名后才发送请求。
- 10:18:18通过定时任务“手动执行一次”启动精确家用297条：69fa0e15-0901-4f4e-af1f-25f1ef2437dd，实际296/297，1失败、0异常/跳过，历时300.47秒、累计294.32秒。四条修正全部通过：成长报告64ms、组件459ms、核心打印729ms、另一打印914ms；展开核心打印确认正式13步及条件跳过、恢复待机证据。家用297与共享10每日任务仍启用，每天08:00/飞书通知/家用一次性允许保持，下次09-22 08:00；旧164试验任务仍停用。历史记录“编辑用例”的旧接口版本问题通过基线原编辑入口及显式选择生产v31处理，未修改旧执行证据。
- 上述全量唯一失败为CAD生命周期：创建2111047861成功，30次轮询约150秒仍PROCESSING，删除返回1001“删除 AI CAD 建模任务失败”。10:30只读复查已SUCCESS，不能把“本轮超时”说成永久生成失败，也不能证明具体完成时间。旧清理用records对象数组not_contains数字ID，会产生无效通过；后续同ID仍可查询直接证明未清理。10:39:06受控三步前置预览核对该ID成功态、严格code0删除，再查同ID返回1001“生成记录不存在”且data=null。最后一步原先只设HTTP断言，因此预览按严格业务码校验标失败；这份真实响应作为删除终态证据，不掩盖原失败。
- CAD修正草稿定向执行0cce5676-cdb9-4d66-89ce-add9bfe49a30（版本853962dc-d411-4030-a3a8-1da231bc8fdb，10:40:59）未通过，306501ms：hasGeneratingTask=false前置通过，创建2111047871成功，30次/10秒轮询仍PROCESSING，删除仍1001。新草稿数字ID嵌入名称字符串又被变量引擎严格拒绝，三个改名/列表步骤未发送；已恢复固定名称，同时保留精确任务ID断言，不能把这三项草稿缺陷归因于服务。10:51:48受控预览等待该ID为SUCCESS、严格code0删除、同ID断言1001/生成记录不存在/null，三步全部通过。两个本轮CAD任务均已回收，但无法从间断观测给出准确生成耗时，也没有服务日志证明慢在模型/队列/资源哪一层。
- CAD候选最终保存v11，含有界hasGeneratingTask前置、最多290秒轮询间隔、所有清理本轮ID条件、详情/列表精确ID、按ID验证不存在。固定ID恢复与设备只读预览已移除，未保存为正式生命周期。候选尚未完整通过，因此未采纳；当前每日仍引用原v7，未减少297条范围。v7的对象数组not_contains数字ID弱清理断言仍待替换，不能宣称此项已修复上线。下一步需业务CAD服务排查这两次慢任务，再完整复验候选并采纳；不再连续创建任务或继续无依据扩大等待时间。
- 10:48:45共享每日精确10条手动执行5ba0cfd2-1b8d-445f-a116-dd37af2a587c：10/10，失败/异常/跳过0，历时1.18秒、累计1.00秒。与本轮家用合计306/307，不用今早旧297/297覆盖新失败。Safari已确认两项每日任务均启用，下次09-22 08:00，飞书通知开启；目标总数仍307。10:54:07最终两步只读预览精确断言设备mac=9888E0094F2A、sn=1385270626257522688、ready/standingBy，currentSliceJob.code0/data=null，全部通过。
- Word三份及docs/career保持未跟踪且未修改。未使用其他设备。

## 2026-09-16 每日回归四条失败逐项核查（Safari，只读诊断）

- 登录恢复后发现查看范围仍在压测演示；切回3D家用、接口v12、生产v31并保存查看范围后读到实际运行，空列表不表示定时任务丢失。
- 今天08:00:13家用自动执行 `39bca524-4b48-4609-adde-cf64543ad684`：293/297，失败4、异常0、跳过0，历时338.96秒、累计332.87秒。共享 `dc4134b2-b085-4134-a17f-42fb880bfefc`：10/10，历时1.18秒。因此同批合计303/307，不能以昨天297/297覆盖今天失败。
- “获取当前周成长报告 - 当前无报告场景”：HTTP200/code0，今天data实际是本周报告对象，旧断言写死data=null。无前置，3条断言通过、null断言失败；这是临时账号数据状态被固化成每日契约，不是接口请求失败。应按真实报告契约设计可重复正向或受控无报告前置，不能仅删除业务校验。
- “查询组件详情接口 - 基础正向流程 - 稳定正向数据”：固定componentSn=1380568721287430144_EP3，HTTP200/code60102001，msg=内容已被删除或者下架~，data=null，152ms。3条执行轨迹、未见动态前置；需要从当前可用模型/组件接口提取并校验组件身份，不继续依赖固定内容永久存在。
- “核心链路-真实打印创建查询取消（4F2A）”：35步骤、60826ms；前置查询指定设备30次均未满足printStatus=standingBy，实际slicing。设备身份9888E0094F2A、deviceStatus=ready均通过；响应标题同时为设备空闲/模型处理完成。主体createReprint未执行；5项清理均因本轮任务变量缺失跳过，没有取消其他任务。
- “重新打印判断 - 创建后取消并清理”：66步骤、118910ms；记录原选择和切换指定设备通过，待机前置30次均slicing，主体未执行，取消/删除本次记录跳过；清理又无条件轮询30次待机失败，最后恢复原选择并验证通过。响应还记录jobStatus=completed。两条需核实设备任务状态组合与当前任务归属，不能把slicing直接等同于物理打印或放宽成可发打印。第二条无本轮任务仍轮询恢复造成额外等待，应按本轮是否实际创建任务约束清理。
- 当前完成的是原始证据诊断，尚未修改这4条基线、重新发起回归或修正平台分类。报告自动分类为产品2/脚本数据2，不能当作已确认的产品根因。未操作设备打印/取消/清空；每日目标数保持不变。后续修正需保留强业务断言，并以定向复验及精确每日范围证明。

## 2026-09-15 回放图表刻度与交互修正（main）

- 用户截图指出无刻度、圆点无反馈。本批提取统一 LoadReplayChart：相对时间/数值刻度、同单位共用范围、不同资源单位分图、水平P95阈值、竖直选中时间线；原斜线原因是时间游标缺少x2，原计划与实际还各自独立缩放。
- 圆点为真实采样，保留固定画布比例；悬停窗口预览、点击联动全部图表及回放位置、方向键切换、常驻中文数值详情。资源技术标签收纳到监控来源，缺失窗口不沿用旧值；错误率比例转换为百分数。
...
- 开始时本地/远端 main 为 d31af84，线上 health 为 53b90da；差异仅此前交接记录。Safari 核实平台机仍为华为 qa 10.130.4.31，43.157.42.206 仅是独立演示机。未操作家用设备、基线、每日任务、Word 或晋升资料。
- 13:59:32 平台终端只读核查：隧道绑定 172.22.0.1:18080/19090，经受限 SSH 到演示机 localhost；健康接口 connect=0.000134s、TTFB=1.086165s、total=1.086221s、HTTP200；SSH RTT=217.653ms、minRTT=209.66ms。192.168.8.131 仍经 10.130.4.1 默认路由；Safari 专用节点现有演示草稿重检仍 TCP 失败，不能当成双节点可用。本机与专用节点均在线、校准有效；本机名称含“备用”但调度级别实际为普通。
- Safari 从 94d14030 的下一轮入口核实完整预填：冻结场景 d9f96b20、环境 v5、1→2→4→2 次/秒、三段各15秒、预分配20/max20、四项阈值、停止条件与1项监控均保留。选择本机，监控/目标连通/单用户业务断言清理通过后执行新运行 `7ee84c53-dd7d-4a81-9e15-0e85f1b17fad`（14:05:04创建，14:07:30开始，约50.213秒含收尾）。各阶段实际22/22、45/45、45/45；112链路发起、334 HTTP、HTTP错误率0.30%、业务断言失败率0.30%、链路失败率0.89%；P95=2215.954ms，四项阈值仅HTTP错误率一项通过。
- 失败发生在正常响应步骤，后续CPU/内存步骤各少一次（111次）。节点正常退出；14:08:09 k6记录业务步骤断言失败。对该run进行READ ONLY事务查询，原样本包含http_error、两项断言失败与workflow_failure，但旧采集器只留check文本，status_code/elapsed_ms均为空；不能据此区分网络异常、429或500。不得把本轮直接归因为CPU饱和，也不得用昨天通过覆盖今天失败。监控最终35样本，演示容器CPU峰值0.11核、内存81.64MiB；压力机10样本、CPU0.19核/6.37%、内存104.82MiB。AI最终状态仍需回读。
- 修复回放：使用真实5秒桶，不把缺失桶间隔充当分母；缺桶打断持续异常/恢复判断与请求曲线；0条完成链路不回退成HTTP吞吐；业务错误率使用业务断言计数，历史分母缺失显示未知；停止后的未来阶段不显示成已发生事件；并发模式不再把吞吐画成实际VU或把小于20的VU标成次/秒。
- 新增中文失败样本表；Agent 0.1.6仅保留有界数字status/error_code及实际时间，不复制URL/请求头/原始错误消息。报告和AI证据包接入这些字段；旧样本空值保留未知，不能回填编造。平台更新后仍须单独升级节点采集器并验证心跳版本。
- 本批检查：真实本地PostgreSQL报告/AI与Agent专项161通过、5项依赖缺失跳过；前端回放/失败样本/报告30通过，类型检查与生产构建通过；后端静态63、前端静态84通过，Python与部署脚本语法和diff检查通过。初次数据库测试使用旧本地配置认证失败，改为读取当前开发容器配置后通过，未改数据库密码。
- 发布复验：f912112已推送并经Safari标准更新脚本部署，14:21:10公网health确认同版本；新失败样本表与0链路首窗显示已通过Safari核对，播放/暂停正常。本机Agent实际Compose目录在源码deploy/load-agent，运行目录缺少.env的首次升级被脚本安全拒绝；随后从实际目录构建0.1.6并完成重建，心跳版本待回读。AI新证据d9ac11746221仍因结论复述数字回退规则，不宣称AI建议已验收。Safari另发现降压窗口计划值停留在阶段起点；补回归后修为按已冻结线性曲线计算，前端31项与构建通过，待发布该补丁。
- 最终发布回读：c3fe89a已推送并部署，14:35:15公网health与远端main一致。Safari明确重新载入后，14:07:50窗口计划为2.667次/秒、实际2.4次/秒，阶段内曲线计算正确；失败样本表中文显示且旧状态仍为未记录。14:37:15节点页本机Agent0.1.6、心跳正常、校准有效、0进程/0VU；专用仍0.1.5、0进程/0VU。Safari窗口菜单确认只有平台窗口，演示机终端仍缺；已异步询问其所在窗口或入口。
- 尚未完成：演示机终端仍未在Safari打开，无法完成本机延迟与运行源码/配额对照及可控故障注入；专用节点安全路径仍未打通；自动保护停止、真实稳定/失稳边界及降压恢复仍待实测。未开放公网端口、未改隧道授权/FRP。新失败证据字段需升级Agent后的新样本证明；回放实际VU、管理注入/复位事件还没有完整接入。


## 2026-09-15 动态前置扩充至 309 条、每日 297 条复验与固定设备基线修正（main）

- 按“前置可由现有基线或其他接口提供”的原则继续从接口层复核：前置步骤必须先校验自己的 HTTP 状态、业务码和关键身份，再提取后续变量；主体仍需独立校验业务结果和结构。没有把资产存在、HTTP 200、草稿保存或一次偶然成功直接当作可重复基线。
- 新增并采纳 3 条当前生产可重复基线，当前有效基线由 306 条增至 309 条：
  - `GET /print3d/api/v1/jdHome/model/page?pageNum=1&pageSize=10&searchKey=笔筒&deviceType=EP7`“分页查询京东之家体验模型”，断言 HTTP 200、业务码 0、分页结构及 `current=1`、`size=10`；Safari 两次生产调试 `25b86c6f-0f92-459f-89d6-137b526bc869`、`ce65a7f4-8408-4762-a797-537a2220120b` 均为 69 ms 并通过。
  - `GET /print3d/api/v1/jdHome/print/application/myPage?pageNum=1&pageSize=10&deviceSn=9888E0094F2A`“查询我的打印申请(用户)”，断言 HTTP 200、业务码 0、分页结构及 `current=1`、`size=10`；Safari 两次生产调试 `0ce9c401-c3da-4b74-9229-84acbfcfd112`（65 ms）、`e6b1ddb0-c2b9-4cd1-8108-def82c9485f8`（69 ms）均通过。
  - `GET /print3d/api/v1/jdHome/device/isSharedDevice`“查询是否共享设备”，先复用设备列表接口校验 `data[0].mac=9888E0094F2A` 并提取业务设备 SN，再调用 `changeSelected` 选中该设备，主体断言 HTTP 200、业务码 0 和 `data=false`；两次稳定生产调试 `8d53544b-e5ed-45c5-a972-c0a71609a19a`（1049 ms）、`b7c5108e-a4f1-414b-90ac-00eeb5388e41`（274 ms）均通过。
- 第一轮 297 条完整执行 `8caaa45a-7002-48e4-8965-cf8058b7b7df` 为 296/297，唯一失败是已有“设备工作详情”基线固化了另一台设备的业务 SN 和打印 SN，真实响应设备为 `EP7-681268D090B7`。这条旧基线违反固定设备约束，不能通过放宽断言处理。
- 已将该有效基线替换为“设备工作详情 - 固定设备动态打印记录”：先调用设备列表精确校验外部设备号 `9888E0094F2A` 并提取业务 `deviceSn`，再选中设备，然后调用 `printJob/page` 获取该设备现有打印记录并提取 `printSn`，最后请求 `devices/workInfo`。主体要求 HTTP 200、业务码 0、`deviceName` 包含 `9888E0094F2A`，并要求 `printSn`、`deviceStatus`、`printStatus`、`deviceName` 均存在。两次定向生产调试通过；完整执行中真实响应为 `deviceSn=1385270626257522688`、`deviceName=EP3-9888E0094F2A`、设备和打印状态均为“空闲”。这是对原有效基线的版本修正，不重复计入新增 3 条。
- 修正后一次直接完整执行 `6ce37e1e-0cbd-40e1-a8e1-4c16c444a80f` 在 205/297 后以 `worker execution failed (OperationalError)` 中断，其余 92 条没有请求、响应或耗时，平台将其标为运行异常；该证据归因于平台数据库/Worker 中断，不是接口断言失败。恢复后历史记录重跑 `8d1a07cc-675f-447d-b040-400b1da44e00` 为 297/297，执行历时 198.00 秒、用例累计耗时 190.46 秒。
- 为让定时任务自身的“最近手动结果”恢复为完整绿态，又从定时任务入口执行 `492445e3-b7c8-40b4-acb7-893c07ca2b2e`：297/297、100%，失败/异常/跳过均为 0；执行历时 175.70 秒、用例累计耗时 168.26 秒。末页 3 条新增用例分别为 68 ms、63 ms、383 ms，“设备工作详情 - 固定设备动态打印记录”为 343 ms，四步证据全部通过。
- 每日任务已保存为“家用业务每日回归（297条）”，每天 08:00、固定生产环境 v31、失败重试 0、任务超时 1800 秒、启用、飞书通知、允许一次性基线均保持有效；Safari 最终显示最近手动 297/297、下次执行 2026/09/16 08:00。共享业务每日 10 条仍启用且最近 10/10，因此当前每天正式执行 307 条。164 条未经逐项安全复核的旧设备依赖试验任务仍停用，不能把它整体直接当成固定设备基线。
- 本轮明确实跑或审查但未采纳的接口：`agentHome/deviceOption` 返回业务码 1001；`slice-param-config/device-and-slice-type` 与 `qrCodeRule/page` 返回业务码 4009；`bizKit/content/page`、`slice/queryViewCnt`、`sliceTemplate/recommendByModel/job` 返回 HTTP 404；`users/reportAll` 约 30 秒超时；`scan/query` 虽能从课程接口取得前置数据，但主体返回“二维码无法识别”；印章线稿任务、Qidi 组件、CAD 工作详情分别缺少低成本可清理生命周期、设备语义不明确或缺少安全列表来源。资产上仍标“开发中”的 3 条 jdHome 接口已由生产结果证明可用，说明接口资产状态需要后续同步，但不影响本轮真实执行证据。
- 本轮线上变更仅涉及 API 测试基线、定时任务目标和本交接记录；没有修改或部署服务代码、配置、数据库结构，也没有重启服务。没有使用 `18CEDF5BA7B2`，也没有回退到其他设备。根目录三份 Word 与 `docs/career/` 仍为未跟踪文件，未修改、未纳入提交。

## 2026-09-15 固定设备只读基线扩充至 306 条、每日回归扩充至 294 条（main）

- 按用户指定继续只使用固定测试设备 `9888E0094F2A`，没有使用启迪设备 `18CEDF5BA7B2`，也没有自动切换其他设备。本轮先复核原来因旧设备编号移出的只读接口，未执行打印、取消、升级、解绑、删除或其他设备写操作。
- 新增并采纳 `GET /print3d/api/v1/qidi/user`“设备默认用户信息 - 固定设备只读验证” v6。请求参数严格使用 `deviceId=9888E0094F2A`；最终断言 HTTP 200、业务码 0、响应结构包含 `deviceSn`/`nickname`，并精确断言 `data.deviceId=9888E0094F2A`。加强后的两次 Safari 生产调试 `e3daebb8-a6c4-49ab-86d1-e3e3d3886bac`、`853cc294-c4b7-4ad8-bf26-460290ef380e` 分别为 112 ms、90 ms，4/4 断言通过。
- 新增并采纳 `GET /print3d/api/v1/qidi/users`“设备查询用户列表 - 固定设备只读验证” v13。请求参数严格使用 `deviceId=9888E0094F2A`；最终断言 HTTP 200、业务码 0，并精确断言首条用户记录 `data[0].deviceId=9888E0094F2A`。两次 Safari 生产调试 `b7726525-80d9-4d63-b808-1928500cdbc6`、`c8235707-8a08-4de7-aa34-f48660acacbc` 分别为 94 ms、83 ms，3/3 断言通过。
- `POST /print3d/api/v1/qidi/collection/page`“我的模型收藏”没有采纳。为避免固化账号数据，草稿先通过 `GET /print3d/api/v1/qidi/user` 动态提取当前 `userSn`，且前置对 `9888E0094F2A` 的 HTTP、业务码和设备身份 3 条断言全部通过；主体仍在调试 `914733b1-cbbd-4b09-b570-850448c48fec` 返回 HTTP 200、业务码 `60101004`、“非启迪设备”。这证明该接口本身限定启迪设备，不能用当前固定设备建立正向基线；没有为了通过而改回旧设备或放宽业务码。
- 当前有效基线由 304 条增至 306 条。每天 08:00 的家用任务精确加入上述两条已通过用例并改名为“家用业务每日回归（294条）”；固定生产环境 v31、失败重试 0、任务超时 1800 秒、启用、飞书通知和一次性基线允许执行均保持不变，下一次执行为 2026/09/16 08:00。共享每日 10 条继续启用，因此每天正式执行 304 条；164 条旧设备依赖试验任务继续停用。
- Safari 手动执行更新后的 294 条完整任务 `33912c24-4ca8-4e65-a21d-a1a723eb42a0`：294/294、100%，失败/异常/跳过均为 0；执行历时 161.86 秒，用例累计耗时 154.80 秒。两条新增用例在完整顺序中分别为 98 ms、102 ms并通过。定时任务页最终显示任务已启用、目标 294 项、最近手动 294/294、下次 2026/09/16 08:00。
- 本轮只修改线上 API 测试基线、定时任务目标和本交接记录，没有修改或部署服务代码、配置、数据库结构，也没有重启服务。根目录三份 Word 与 `docs/career/` 仍为未跟踪文件，未修改、未纳入提交。

## 2026-09-14 家用有效基线扩充至 304 条、每日稳定范围调整为 292 条（main）

- 接续 291 条家用每日任务继续复核未覆盖资产。本轮净新增 3 条有效基线：`GET /print3d/api/v1/userCadComponentCategory/list`“查询CAD组件分类 - 基础正向流程”，两次 Safari 调试分别为 60 ms、49 ms，要求 HTTP 200、业务码 0、`data` 存在；`GET /print3d/api/v1/userCadComponent/page?pageNum=1&pageSize=20`“分页搜索CAD组件 - 基础正向流程 - 真实响应数据联动补跑”，加强后两次调试分别为 59 ms、52 ms，要求 HTTP 200、业务码 0、`data.records` 存在；`GET /print3d/api/v1/printTaskQueue/itemDetail?queueItemSn=0`“合盘分盘详情 - 不存在队列项错误契约”，两次调试分别为 69 ms、49 ms，精确断言 HTTP 200、业务码 `60101002` 和“待打印盘不存在或已失效”。三条均在真实响应通过后采纳；当前有效基线为家用 294 条、共享 10 条，合计 304 条。
- 另外更新了两条已有有效基线，不能重复计为新增。`advancedSliceParamConfig` 改用当前允许设备的业务 `deviceSn=1385270626257522688`，两次调试 69 ms、60 ms，4/4 通过；`currentSliceJob` 增加 `hpDevice` 前置，先断言外部序列号严格等于 `9888E0094F2A`，再提取业务 SN 调用主体接口，两次完整调试 104 ms、103 ms 均通过。没有使用 `18CEDF5BA7B2`，也没有自动切换其他设备。
- `GET /print3d/api/v1/slice/queryViewCnt` 实跑为 HTTP 404，调试草稿 `98b91a76-d53f-45ae-a85f-220da0a8795e`，未采纳；没有把资产存在、HTTP 可达或草稿保存冒充基线通过。
- 首次 294 条全量执行 `b81c8f10-18c6-40c5-8045-e3985357a3cb` 为 293/294、99.7%，失败 0、运行异常 1、跳过 0；执行历时 306.04 秒，用例累计耗时 298.83 秒。唯一异常来自原有“重试文字建模 - 创建取消后重试并取消”：前置 `POST /createTextModel/textV2` 在 30053 ms 后读超时，主步骤未执行，缺少任务 SN 时清理按设计跳过。仅重跑该项 `b07dbad5-d673-46a9-89f7-a3e545711306` 为 1/1，通过且四个步骤均完成，执行历时 14.12 秒、累计 14.09 秒；这证明接口可用，但不能推翻它连续全量中的超时风险。
- 紧接前两轮再次发起的 294 条执行 `a60abab1-5091-4ebd-b93a-000b1f501982` 在 134 条通过、6 条运行异常时主动取消，执行历时 305.36 秒、累计 302.34 秒。该轮与上轮和单项重跑无冷却连续执行，已经形成建模服务请求堆积，不能作为每日独立运行的失败率结论；保留执行记录作为连续重跑会放大外部服务延迟的证据，不再继续制造无效任务。
- 冷却后的 293 条完整执行 `cfaf901e-de03-4b8a-b652-a74bf888a39b` 为 272/293，失败 20、运行异常 1、跳过 0；执行历时 402.87 秒、用例累计耗时 395.85 秒。20 条业务断言失败集中在模型浏览、搜索、系列、详情和打印队列等模型库依赖接口，真实响应均为 HTTP 200、业务码 1001、“调用模型库服务失败”，单次约 10 秒；这是生产下游模型库故障，不能通过放宽断言或移出每日任务隐藏。代表性详情用例随后定向重跑 `e8cd9753-a6d3-4608-9b27-dd52004b5b77` 为 1/1，通过，执行历时 281 ms、累计 244 ms，证明下游已恢复。唯一运行异常“消息已读 - 动态真实消息”的列表前置当时返回空 `messageCategories` / `messageConfigs`，没有可提取的动态消息，属于每日执行顺序和数据前置不稳定。
- 每日任务最终保存为“家用业务每日回归（292条）”，精确移出“重试文字建模 - 创建取消后重试并取消”和“消息已读 - 动态真实消息”；两条仍保留为按需正式基线。每天 08:00、固定生产环境 v31、失败重试 0、任务超时 1800 秒、启用、飞书通知和一次性基线允许执行保持不变，下一次执行为 2026/09/15 08:00。共享每日 10 条继续启用，因此每天正式执行 302 条；164 条旧设备依赖试验任务继续停用。
- Safari 最终手动执行精确的 292 条任务 `ea502dab-bba1-48b8-984e-da602e4e058b`：292/292、100%，失败/异常/跳过均为 0；执行历时 186.10 秒，用例累计耗时 178.95 秒。该结果覆盖本轮 3 条净新增基线和 2 条设备关联修订；整轮未使用 `18CEDF5BA7B2`，也没有自动切换其他设备。
- 本轮只修改线上 API 测试数据和本交接记录，没有修改或部署服务代码、配置、数据库结构，也没有重启服务。根目录三份 Word 与 `docs/career/` 仍为未跟踪文件，未修改、未纳入提交。

## 2026-09-14 家用 API 未覆盖差集复核、基线扩充至 301 条与每日 291 条复验（main）

- 先在华为 qa 服务端进行只读核对：仅使用 `pwd`、`ls`、`find`、`ps`、Python 模型结构查看和 SQLAlchemy 查询，将接口 v12 中家用 `GET` 资产与当前有效基线按“方法 + 规范化路径”比对，确认有 34 个路径差集。服务端没有修改数据库、配置或代码，没有重启、部署或发起压测；线上数据变更均由 Safari 的 API 测试页面完成。
- 差集不是可直接入库清单。Safari 对其中可在当前账号下安全探测的候选实跑：`GET /print3d/api/v1/users/reportAll` 在 30047 ms 后读超时，调试草稿 `71a97ec0-aacf-46c6-8f7a-3282cade7f13`；`GET /print3d/api/v1/bizKit/content/page` 在 173 ms 返回 HTTP 404，调试草稿 `5e7a6999-ad58-4311-b8ba-be0b22b842b4`。两者均未采纳，也没有用 HTTP 可达、资产存在或草稿保存冒充基线通过。
- `GET /print3d/api/v1/learningAdventure/report/history` 的旧草稿原按正向成功设计，实跑 `87304722-d2b1-4132-9d08-2cfe69906372` 得到 HTTP 200、业务码 1001 和“历史报告不存在”。继续调用 `historyWeeks` 探查动态正向前置，调试 `3e9c7b20-0ceb-43c3-8022-cfccef2a4a13` 为 8/8 通过，但真实数据 `records=[]`、`total=0`，当前账号没有可用历史报告。因此将原用例明确改为“获取历史成长报告 - 无记录错误契约”，断言 HTTP 200、`code=1001`、`msg=历史报告不存在`；Safari 两次调试 `34ee161d-1881-4642-bd22-dc49337f407d`（62 ms）和 `7da3945b-bd0d-4ded-ada0-eec3aacf54eb`（69 ms）均为 3/3，通过后采纳。它覆盖稳定的无数据错误契约，不表示历史报告正向业务已通过。
- 本轮还修复“核心链路-中文搜索结果与模型详情”，两次调试 `b3271900-759b-468c-8414-5a8859a38d6e`（284 ms）和 `e67ad2cd-d652-4f59-b80c-2b71e43673b4`（283 ms）均通过；完整 290 条执行 `94745b9b-e6a9-46ef-a2f8-a81149a06a32` 为 290/290。重新验证的“查询科目类别”“查询科目下年级”“模型制作指导”都是已有有效基线的版本刷新，其中模型制作指导调试 `b25d943f-4d3c-4c6c-bf5a-6f1bd17f5a9e` 与 `a3aacca6-0e45-4b5d-a806-ebd67c508058` 均为 5/5；不能重复计为新增。
- 新错误契约使当前有效基线由 300 条增至 301 条，家用业务分组由 50 条增至 51 条。每日任务从 290 项精确加入这一项，保存为“家用业务每日回归（已复验291条）”；每天 08:00、固定生产环境 v31、失败重试 0、超时 1800 秒、启用、飞书通知和一次性基线允许执行均保持不变，下一次执行为 2026/09/15 08:00。共享业务每日 10 条仍启用，因此当前每天正式执行 301 条；164 条设备依赖试验任务继续停用。
- Safari 手动执行更新后的精确任务 `034f163c-26ba-49e3-8494-06f94beaf9ef`：291/291、100%，失败/异常/跳过均为 0；执行历时 176.61 秒，用例累计耗时 169.41 秒，两个时间口径分开展示。本轮没有执行设备写操作，也没有使用 `18CEDF5BA7B2` 或自动切换其他设备；唯一允许的真实设备仍为 `9888E0094F2A`。
- 本轮只修改线上 API 测试数据和本交接记录。根目录三份 Word 与 `docs/career/` 仍为未跟踪文件，未修改、未纳入提交。

## 2026-09-14 家用 API 再审计、基线扩充至 300 条与每日 286 条复验（main）

- 在接口 v12 的 1053 个资产范围内继续按“真实响应 + 业务断言 + 可重复前置/清理”复核。真正新增 `GET /print3d/api/v1/badge/task/detail?taskCode=WEEKLY_PRINT_CHALLENGE`，用例名“获取勋章任务详情”；该接口使用稳定 `taskCode`，不需要动态任务 ID，修正了上一节对此接口前置条件的保守判断。两次 Safari 在线调试分别为 124 ms、116 ms，最终要求 HTTP 200、业务码 0、`data.taskCode=WEEKLY_PRINT_CHALLENGE`，3/3 断言通过并采纳 v2；当前有效基线由 299 条增至 300 条，净新增只有这一条。
- 另外重新读取线上真实响应并刷新了 6 条原本已经有效的基线版本，不能把它们重复计为新增：文字建模推荐提示词 89 ms（4/4）、体素最新生成记录 341 ms（5/5）、体素生成记录分页 665 ms（8/8）、积分兑换商品分页 192 ms（8/8）、叠叠乐像素模型库分页 116 ms（6/6）、勋章总览 61 ms（4/4，新增精确校验首项任务码 `MONTHLY_PRINT_CHALLENGE`）。
- 明确检查但未采纳新的每日目标：`GET /print3d/api/v1/model/blindBox` 和 `GET /print3d/api/v1/createTextModel/queryCurrentModel` 在资产中均标记“开发中”，后者还依赖瞬时任务状态；没有用 HTTP 可达或已有旧基线冒充当前可稳定新增。其他未覆盖资产仍包含专用账号/设备、动态生命周期、破坏性清理或未证明前置，需逐条取得证据，不能把 1053-300 直接视为可安全定时执行数量。
- 每日任务只加入本轮真正新增的勋章任务详情，从 285 项精确更新为“家用业务每日回归（已复验286条）”。Safari 编辑页确认家用当前有效目标 290 条、该任务选中 286 条；其余按需/风险隔离目标没有为凑数全选。每天 08:00、固定生产环境 v31、失败重试 0、超时 1800 秒、启用、飞书通知和 7 条一次性基线允许执行均保持不变；下一次执行为 2026/09/15 08:00。共享业务 10 条任务仍启用，因此当前每天正式执行 296 条；164 条设备依赖试验任务继续停用。
- Safari 手动执行更新后的精确任务 `64394243-e459-4ba0-94ee-92cea488429f`：286/286、100%，失败/异常/跳过均 0；执行历时 152.92 秒，用例累计耗时 146.03 秒。结果中按路径筛选确认新接口 `GET /print3d/api/v1/badge/task/detail` 通过，耗时 89 ms。
- 完整回归后再次只读执行设备列表 `bf09fbf8-61a3-4ec6-abcb-1302c5dff0eb`：1/1、100%，894 ms。原始响应确认唯一选中设备为 `9888E0094F2A`（业务 `deviceSn=1385270626257522688`），`selected=1`、`deviceStatus=ready`、`printStatus=standingBy`，两个中文状态均为“空闲”；未使用 `18CEDF5BA7B2`，也未切换其他设备。
- 本轮只修改线上 API 测试数据和本交接记录，没有改运行代码。根目录三份 Word 与 `docs/career/` 仍为未跟踪文件，未修改、未纳入提交。

## 2026-09-14 家用 API 每日基线扩充至 285 条（main）

- 接续 283 条稳定任务，仅从未覆盖资产中选择当前账号可直接调用、无真实设备写入、无业务数据清理依赖的只读接口。新增 `GET /print3d/api/v1/aiModeling/cad/hasGeneratingTask`，用例名“CAD生成状态 - 当前账号”，要求 HTTP 200、业务码 0，且 `data` 必须为布尔值；Safari 在线调试实读 `{"code":0,"msg":"success","data":false}`，三条断言通过，服务请求耗时 28 ms，已采纳 v1。
- 新增 `GET /print3d/api/v1/aiModeling/cad/page?pageNum=1&pageSize=12`，用例名“CAD任务分页查询 - 当前账号”。先以 HTTP/业务码探测真实响应，确认 `data.records` 后补结构断言并再次调试；最终 HTTP 200、业务码 0、`data.records` 数组结构三条断言通过，在线调试 62 ms，已采纳 v2。两条新用例使当前有效基线从 297 条增至 299 条。
- 每日任务从 283 项精确增加上述两项，改名为“家用业务每日回归（已复验285条）”；每天 08:00、固定生产环境 v31、失败重试 0、超时 1800 秒、启用、飞书通知和“一次性基线也执行”均保持不变，下一次执行为 2026/09/15 08:00。共享业务 10 条任务保持启用，因此当前每天正式执行 295 条；未核实设备依赖的 164 条试验任务继续停用。
- Safari 手动执行更新后的精确任务 `4e326c12-0474-4454-a13a-07b4bd9a6193`：285/285、100%，失败/异常/跳过均 0；执行历时 155.53 秒，用例累计耗时 148.34 秒。结果内单独筛选确认新 CAD 状态用例 56 ms、CAD 分页用例 53 ms，均通过。
- 完整回归结束后再次只读执行设备列表 `bd1d33f1-a9e2-4f66-861d-83288c963239`：1/1 通过，原始响应中唯一允许设备 `9888E0094F2A` 对应业务 `deviceSn=1385270626257522688`，仍为 `selected=1`、`deviceStatus=ready`、`printStatus=standingBy`，两个中文状态均为“空闲”。未使用 `18CEDF5BA7B2`，也未自动切换其他设备。
- 本轮没有为凑数量执行需要动态勋章任务 ID、京东之家专用模型/设备、EPOne/Camera、打印删除、ES 全量同步等接口；这些仍需各自可重复前置、权限和清理证据后才能进入每日基线。根目录三份 Word 与 `docs/career/` 均保持未跟踪、未修改、未纳入提交。

## 2026-09-14 家用 API 基线、唯一设备与每日回归恢复（main）

- 本轮开始前重新核对本地 `main`、`origin/main` 和线上健康接口，三者运行代码均为 `72c5470f412daa1b4f0c8fcaa78a571418b5bd78`；平台仍部署在华为 qa `10.130.4.31`（外部 `101.34.197.12:8088`）。本轮只更新线上 API 测试数据和本交接记录，没有改运行代码。
- 按接口 v12 的 1053 个资产重新对账当前有效基线。新增并实跑采纳 `GET /print3d/api/v1/badge/overview`（HTTP 200、业务码 0、三项断言通过）；`GET /api/v1/menu/get` 和 `POST /business/v1/shopManager/list` 实际返回业务码 1001，未把 HTTP 可达误采纳为基线。当前有效基线共 297 条，其中家用及跨业务 289 条、共享 8 条；按方法和规范化路径统计覆盖 283 个资产，尚有 770 个资产没有有效基线。该数量是接口覆盖盘点，不等于未覆盖接口都适合当前账号或可安全自动化。
- DeepSeek CAD 创建/轮询/删除链路最初因等待不足失败，改为最多 30 次轮询且保留删除清理后，在线调试 `fc58db01-98ab-43ee-a4c1-bb1eb078a61d` 通过并采纳 v7。其后完整 283 条执行 `f4d4c982-8089-4c48-94e2-7e7ae7f45aa5` 为 283/283；CAD 本身仍按用例边界保留为按需正式基线，不进入每日稳定回归。
- Safari 实跑发现旧“选中设备”基线虽然接口断言为绿，却把当前设备切到了 `681268D090B7`。立即停用当时的每日任务，将账号恢复选择到唯一允许设备：外部序列号 `9888E0094F2A`、业务 `deviceSn=1385270626257522688`。`POST /print3d/api/v1/devices/changeSelected` 固定请求体为 `{"userSn":"0","deviceSn":"1385270626257522688"}`，在线调试 `27bff56d-7e30-4246-8cc3-f00bea0228bc` 通过并采纳 v5，替换旧正式基线。不得使用 `18CEDF5BA7B2`，也不得自动切换到其他在线设备。
- 用户明确一次性用例可以作为基线。7 条 `API Test / 一次性` 均保留为正式基线并纳入每日任务；定时任务已命名为“家用业务每日回归（已复验283条）”，每天 08:00、固定生产环境 v31、失败重试 0、超时 1800 秒、飞书通知和“一次性基线也执行”均开启。按需 CAD 不进每日任务，避免其长轮询影响每日稳定性口径。
- 最终用定时任务的精确 283 条目标在 Safari 手动执行 `1ff40cb0-dbf4-4048-a1c2-024baeaedce9`：283/283、100%，失败/异常/跳过均 0；执行历时 150.42 秒，用例累计耗时 143.35 秒，两种口径分开展示。执行结束后只读复查 `0b33fa4b-fb36-4116-9d0f-3134226ba81f` 的原始响应：`9888E0094F2A` 仍为 `selected=1`、`deviceStatus=ready`、`printStatus=standingBy`，页面中文状态均为“空闲”。任务保持启用，下一次执行为 2026/09/15 08:00。
- 共享业务每日回归保持启用，最近 10/10；两套任务合计每天执行 293 条。试验性的“家用安全接口每日回归（设备用例待复核）”在 66/164 时取消，因隐藏设备依赖未证明，继续保持停用，不能用其替代已复验 283 条任务。
- 根目录三份 Word 与 `docs/career/` 均保持未跟踪、未修改、未纳入提交。

## 2026-09-14 阶梯吞吐单变量线上复验与回放验收（main）

- 接续核对确认平台仍为华为qa `10.130.4.31`（外部 `101.34.197.12:8088`），腾讯云 `43.157.42.206` 只是独立演示服务所在机器。回放静态发布修复 `0c09bd6` 与AI资源结论约束 `d8474ad` 已依次推送main并部署；本轮开始时线上health为 `d8474ad39d7af003d12e3daf2536c434b941b9c8`。
- 回放已真实读取 `metric.series[].points`，支持冻结负载快照、同一按钮播放/暂停、Safari切后台暂停和缺失P95不计恢复。CPU/内存绝对用量保留cores/MiB，只有百分比或同点配额换算结果参加高占用判断。Safari发现采样桶比执行开始早数秒时首窗显示“计划—”；`858b4bc`已把计划曲线对齐到包含执行起点的首个真实采样桶。线上Safari复验首窗为“计划1次/秒”，播放后同一按钮变为“暂停”，再次点击恢复“播放”，时间轴同步推进到第二阶段。
- Safari重新诊断旧报告 `7731abc4-4781-4f97-a9dd-8f9aa027fd18`，v10模型输出因未通过证据校验被拒绝，页面明确显示平台规则备用建议；旧v9“资源充足”没有被继续展示为有效诊断。平台保留确定性报告，并记录具体拒绝原因为模型结论复述测量数值。
- 09:24:17 从旧报告的下一轮入口创建单变量复验 `94d14030-f408-407b-b57a-1ce5dad41182`。冻结原场景版本、隔离演示环境v5、1→2→4→2次/秒各15秒、四项阈值、停止策略和1项服务监控；只把 `pre_allocated_vus` 从10改为20，`max_vus` 保持20，节点只选“本机（备用）”。监控检查、目标连通性、单用户请求/业务断言/清理均在Safari通过后才正式启动。
- 新运行三段计划/实际发起为22/22、45/45、45/45，112条完整链路、336个HTTP请求，HTTP/业务/完整链路失败率均0；全程P95 954.03ms（阈值1200ms），4/4必选阈值通过，执行历时口径为配置45秒、含调度收尾观测50.193436秒。上一轮第三段44/45和P95 1532.649ms在本轮没有复现；这支持继续验证VU调度影响，但单次对照仍不能把差异唯一归因于预分配VU。
- 被测服务监控来源仍为 `http://172.22.0.1:19090` 的演示服务自身容器：35个有效样本，CPU峰值0.11核、内存working set峰值80.71MiB。发压Agent另计：CPU峰值0.19核/6.25%、内存103.3MiB。当前压力下没有出现持续P95超线、HTTP或业务错误，也没有CPU/内存打满证据；结论只限“本轮未发现明确瓶颈”，不扩写为容量充足。
- v10对新报告的AI输出同样未通过证据校验并回退平台规则；下一轮保持原曲线复验是允许的，升压仍需先补业务容量目标、单轮增长上限和观察时长。未操作任何真实设备，家用定时继续停用，唯一允许设备仍为 `9888E0094F2A`。
- 用户指出报告“测试条件”直接显示冻结英文。报告层已将本轮历史值翻译为中文可读信息：`demo-1.0.1`显示为“演示服务1.0.1”，隔离令牌/无业务账号及原曲线复验、非业务接口/人工智能/设备命令边界均显示中文；底层冻结证据不改写。首次Safari发布复核发现线上历史数据为空格分词，补充空格/下划线统一归一化后以 `7ccc7ed` 再次发布，Safari已确认管理摘要和测试条件均为中文。
- 尚未关闭：专用节点到受限演示服务的双节点路径、演示机本地响应与隧道对照、可控HTTP/业务错误及保护停止、找到最后稳定与首次持续失败区间并验证降压恢复。回放应作为报告内的证据复核工具保留；无持续异常的运行必须只展示开始/阶段/降压/完成，不能编造“瓶颈时刻”。

## 2026-09-10 12:20 服务事实v9部署与原阶梯吞吐复验进行中

- 实现提交 `86dccd0de4aed5b1b4e2ab8f2b80526ef6cec038` 已推送main并回读远端一致；Safari平台终端部署完成，12:07:35外部健康接口确认同版本，前端 `index-B-XFUIv2.js`。平台为华为qa/10.130.4.31（外部101.34.197.12:8088），43.157.42.206仅为用户指定的独立演示机，不混用两者。
- 生产第二次只读审计 `/tmp/home-device-reference-audit-20260910-after.jsonl` 确认有效基线296，静态关联旧设备的有效基线列表为空，pending/pending_cases均0；这不覆盖动态值或直接运行历史用例。家用定时继续停用。
- Safari保存隔离演示环境v5（12:13:03）：数据库/下游不存在、CPU墙钟工作150ms仅适用/demo/cpu，来源均为86dccd0源码核查，显式注明演示机运行源码哈希仍待确认；其余行为/GC保持未知。服务地址、内网授权和凭证保留。服务与监控页回读成功，监控连接检查时间12:13:33。
- 原183d044c负载从生产记录只读核对：1→2→4→2次/秒，各15秒，pre_allocated_vus10/max_vus20，P95<=1200、HTTP<=1%、业务/链路<=0%，stop HTTP20%/grace10秒；purpose stress、demo-1.0.1、warm。Safari新建12:20:02草稿，选v5、本机、同曲线/阈值/停止；监控前后各60秒。连通及单用户预检通过后已启动，终态结果待后续追加。
- Safari另开演示机OrcaTerm并尝试已显示微信快捷登录，仍0会话；已请求用户完成演示机认证。没有切到Chrome、没有复制凭证或放宽受限隧道。

## 2026-09-10 继续验收：原曲线草稿、生产设备盘点与服务事实v9

- 用户明确要求继续原压测待办，新增动画回放设计不应阻断已授权验收。回放草案仍待设计审阅，本批不实现回放、不改用户Word或晋升资料。
- Safari已恢复并创建原曲线复验草稿 `2ac96437-eb0f-4e75-8827-0ea286d5cee2`（11:30:26）：1→2→4→1 VU、每段15秒、总45秒，原场景d9f96b20、环境v4、4项阈值与1项监控保留，选本机节点并通过目标连通；未重复正式发压。向导摘要“阶梯 秒/仅起始或峰值”另已修为完整曲线与总时长。
- 用户在Safari平台终端部署完成5f46bb1；11:38:11健康接口实读5f46bb1。平台网络只读核查：ssh进程1040744监听172.22.0.1:18080/19090；到192.168.8.131走10.130.4.1默认路由。专用节点TCP仍失败、本机通过，现有Docker本地网桥地址不能作为双节点共用目标。未开公网端口或修改密钥/FRP。
- 生产只读设备审计已实际执行，输出 `/tmp/home-device-reference-audit-20260910.jsonl`：1116用例/2671版本/548基线/33环境版本/2定时，pending及pending_cases为0。发现7条基线历史结构关联旧设备，其中3条仍有效；Safari逐条移出“固件更新接口 - 基础正向流程 - 响应数据驱动”“设备默认用户信息 - 基础正向流程”“我的模型收藏 - 基础正向流程 - 响应数据驱动2 - 真实响应数据联动补跑”，有效数299→296。全部是隔离旧设备引用，不把GET信息接口误称为已发送固件升级命令。家用定时仍停用；777个动态引用、750个设备字段审查标记和29个未读取秘密值不能当已经证明运行安全。
- 本地完成服务事实v1→冻结环境快照→逐条建议v9：三态组件、四类有界行为与人工来源；按实际服务/端点绑定事实；AI选择动作、优先级、证据与完整下一轮曲线，平台按域/服务/端点和事实校验并生成动作/验证方式。unknown仅确认/补证据，absent不建议不存在组件专属实验；无服务绑定监控仅作环境级观察。旧运行不回填新事实，旧分析明确历史未逐条校验。
- 独立审查发现并关闭行为引用同服务其他端点的问题；自由AI解释不能机械证明根因，页面改为“AI解释（根因待验证）”，参数校验标记限定下一轮参数。原始诊断文字、报告和历史记录保留。
- 根审最终：PostgreSQL专项240项无跳过、前端全量、生产构建、静态后端63/前端84/Gateway46、diff检查通过。服务事实/建议本批尚需提交部署、Safari录入事实并用新运行验证v9。平台终端随后出现T_514连接超时，正在正常重连；演示本地/隧道对照、双节点路径、故障/保护停止、真正瓶颈与恢复仍继续，不宣称完成。

## 2026-09-10 11:15 部署复核与AI证据约束补强

- 本批实现已提交推送main `9f4eee0`；远端回读一致。线上前端资源为 `index-9YTisDtw.js` / `index-P_MWXx87.css`，与807aa0c构建对应。现有报告d2d00833的重新诊断 `83e2edda-185e-451c-ba27-9c1e6f2a63ff`、证据哈希06d53011d88e，在Safari中无需手动刷新自动完成，显示低置信度证据不足、原曲线复验与入口。
- Safari跨应用进入下一轮被正确阻断；随后已逐项选演示项目/v1接口/v4环境并点击保存查看范围，回到预填路由时粘贴超时，接着AX仅返回空窗口、截图不可用，重连和重置后仍如此。未盲发Return、未创建新草稿或新压测。已请求恢复可见Safari及其中平台/演示终端。完整预填、最新guard线上诊断、数据库全量盘点和原多场景待办仍未完成。
- 用户通知部署完成后，11:15:00 `/api/health` 已确认 `ecff3d3c932659767caa4ee7b69be24896384dad`，与本地/远端main一致；覆盖下方11:00尚未部署的结论，包含807aa0c。Safari已重新载入并打开隔离演示项目阶梯报告，对已有d2d00833重新诊断；未启动新压测。
- 本批新增AI完整性共用校验：执行完整、目标达到、采样一致与正整数节点完成计数必须明确满足。未知/缺失或不完整时仅允许insufficient_evidence/mixed及low/medium；模型纠正和规则回退均受约束，避免回退把未知写成“无瓶颈”。这不是组件根因证明，也尚未约束每条自由文本实验；服务能力事实、逐条建议证据及单变量实测仍待补。
- 新增 `scripts/audit_api_device_references.py`：项目范围、全部case/env版本、前置/清理/数据行/依赖反向闭包、定时引用、未完成执行的只读静态盘点。数据库只读可重复读事务和查询超时；秘密变量值在SQL层置空，不解密凭据；仅输出标识/路径/固定标记到新建0600 JSONL。历史值保留，不能把静态盘点当运行时全局设备拦截。
- 根审验证：PostgreSQL AI/策略/瓶颈/报告和Agent证据158项、审计脚本8项、后端静态63项通过；Python编译及diff检查通过。本批代码尚未部署，不能与已在线的ecff3d3混算。生产全量只读盘点仍待平台终端恢复；家用定时继续停用。

## 2026-09-10 11:00 续接：家用定时停用、线上仍为c98cc3a

- Safari保存3D家用查看范围后核实2条定时任务。家用288条任务每天08:00且允许一次性基线；移出旧打印/取消baseline后，已被目标解析阻断（2个目标已删除、手动执行禁用）。现进一步关闭家用任务启用开关，页面确认已停用；共享10条任务保留。
- 今天08:00自动执行 `ffd40cf0-98fc-44d3-92ba-96c0399d8e57` 的打印/取消快照仍指向旧设备，两条响应均为北京时间08:00:25；审计已确认至少5组请求，不是仅昨天4组，见原审计文档追加节。没有启动新设备测试。
- 11:00:58实时 `/api/health` 确认线上仍为 `c98cc3a5e8c72da4aedfcf734a223c759c1a347f`。`807aa0c`的AI自动刷新/原配置复验入口仍未部署，不能宣称已完成线上闭环验收。本地main在`ecff3d3`，其后是本次交接更新。
- Safari平台堡垒机各终端已退回登录页，已异步请求恢复平台10.130.4.31与演示43.157.42.206会话，不索取密码令牌；在线UI仍可用。独立只读审查正在确认设备引用路径与AI架构证据缺口，后续更新覆盖本节进度。

## 2026-09-10 用户指定唯一测试设备及旧设备基线移出

- 用户明确要求“以后不要用这台设备了，用9888E0094F2A”“以后先都用这一个”。后续真实设备测试统一只用 `9888E0094F2A`，不得使用 `18CEDF5BA7B2` 或自动替换其他设备；已写入 AGENTS.md 固定规则。
- Safari 已将“打印任务下发 - 基础正向流程”和“任务指令下发 - 基础正向流程”分别保存为 v3 草稿，请求体与查询参数的 deviceId 改为 `9888E0094F2A`。没有点击“保存并调试”、没有发送设备指令，也没有把新草稿采纳为已通过基线。打印草稿明确记录文件兼容、空闲状态、本轮取消和恢复终态待验证。
- Safari 逐条移出上述仍指向旧设备的 v2 正式基线，页面确认成功：当前有效数301→300→299，deviceCmd筛选4→3→2，仅余两条状态查询。保留用例草稿与历史执行证据。源v2 caseVersionId：打印 `6e73b631-4979-4fc8-8ad0-b1853abd880e`，取消 `9bb81872-9b1c-41f4-909b-4826dae3cb7e`。
- **范围边界**：本次已处理确认的两条旧设备写操作，不代表所有环境/历史版本/直接用例任务已完成全量替换。定时任务页面仍在演示项目范围显示0条，不能据此断言家用定时任务不存在；家用定时引用和其他设备用例需继续盘点，盘点前不执行家用全量回归。此规则是Codex工作约束与两条基线隔离，不是平台全局设备拦截器已上线。

## 2026-09-10 启迪真实设备指令审计：原 288 条基线不可直接复跑

- Safari 只读核实设备 `18CEDF5BA7B2`：9月9日四轮家用基线 `45ac741e`、`f08274df`、`0f9c838a`、`9caee20a` 均发送了打印 `323456345` 与取消 `3234564126`。服务端响应北京时间分别为13:33:54、14:37:08、17:36:31、17:49:00，共确认4次打印请求和4次取消请求。中断轮请求在Worker中断之前，非恢复重放。
- 两条用例为“打印任务下发 - 基础正向流程”和“任务指令下发 - 基础正向流程”。请求body包含真实设备号，URL query却为示例设备号；不能只看URL判断无设备副作用。八条请求HTTP200/code0/data true，断言只有HTTP200和code0，未验证设备执行/取消终态。
- **修正此前验收范围**：288/288仅代表当时接口断言通过，不代表真实设备安全或取消成功。原“稳定性问题已关闭”仅指Worker与图生图超时，设备动作隔离和终态门禁缺口未关闭。后续不要直接复跑原288条，应先盘点设备写操作及引用、落实专用设备与状态门禁。
- 用户截图中四组固定request_id与上述请求相同，但缺数据库时间，未逐行匹配行ID；UUID打印 `91986139-5612-4b35-b75c-aea3d5ddcfe7` 来源未定。不能据此认定用户本人手动操作，不能把取消接口受理成功当设备取消成功。
- 本次只读审计，未发送新设备指令、未修改线上用例/基线/定时计划。完整证据见 `docs/evidence/qidi-device-command-audit-2026-09-10.md`。Word文档与晋升资料保留。原演示服务压测任务仍有待验收项，不能因本审计视为完成。

## 2026-09-09 API 执行 Worker 中断安全收敛（main，待部署）

- 线上执行 `f08274df-b04c-4874-9634-29bf9da879da` 从 14:37:07 起停在 219/288；日志确认 API Worker 于 15:30:22 Warm shutdown、15:30:24 重启。执行器只接受 `QUEUED → RUNNING`，重启后的 Worker 不会接管已处于 `RUNNING` 的记录，这是执行可靠性缺口，不是第 220 条接口失败。
- 新增 Scheduler 自动恢复：默认将 5 分钟没有执行或子用例进度的 `RUNNING` 记录列为候选，实际恢复时加锁再次核对；窗口可通过 `API_TESTING_EXECUTION_STALE_SECONDS` 配置为 120–3600 秒。重复派发幂等，近期仍有进展的执行不处理。
- 为避免基线中的创建、切换设备、发验证码等请求被重复触发，恢复不会自动重放未完成请求。已完成用例和尝试证据原样保留；剩余 `RUNNING/QUEUED` 用例写为 `BROKEN/worker`，固定说明“执行 Worker 中断；已完成结果已保留，未完成用例未自动重放”，随后生成终态汇总、刷新关联任务并沿用原通知策略。
- 无需数据库迁移或 Agent 升级。TDD 先复现缺少恢复 API 的 2 项失败及缺少配置、Scheduler、Worker 入口的 7 项失败；实现后相关 97 项通过。完整 API testing gate 为后端 1203 通过/2 跳过、前端 617 项、生产构建、视觉检查和 Chromium E2E 1 项通过；后端静态 63 项、Python 编译和 diff 检查通过。待提交、推送和部署后，确认旧 219/288 记录自动进入终态，并用 Safari 核对执行列表、详情和报告。

## 2026-09-09 四类压测实跑、统计证据与页面切换优化（main，待部署）

- 隔离演示服务已覆盖四种 k6 模型：固定吞吐、固定并发、阶梯吞吐、阶梯并发。固定吞吐 2 链路/秒 30 秒完成 61 条链路/183 个 HTTP 请求；固定并发 4 VU 30 秒完成 45 条链路/135 个请求；阶梯吞吐 1→2→4→2 链路/秒出现 1 个 HTTP 失败、2 个断言失败和 1 条链路失败；阶梯并发 1→2→4→1 VU 完成 39 条链路/117 个请求。均为受控非业务服务，不代表生产容量。
- 修复阶梯并发证据：用连续 VU gauge 与每个节点自己的阶段轨迹对齐，报告展示计划起止 VU、计划平均并发、实测平均并发和阶段结论；阶梯吞吐仍使用实际发起量，二者不再混用。固定并发仅对不少于 20 秒的运行允许首尾一个五秒上传桶加采样 tick 的边界容差，内部采样缺口仍失败关闭。
- 领导摘要的“实际压力”区分完整业务链路吞吐、HTTP RPS 和请求总数，不再用 HTTP 请求数冒充业务完成量。请求头包含无法按 HTTP 传输的中文等字符时，执行器返回可理解的环境配置错误。编辑用例草稿后清除上一次校验提示，避免旧错误继续显示。
- Safari 自动化曾把并发输入错误追加为 `30444`；该值未启动执行，已改为先覆盖再回读确认。后续不得复用页面更新前的 AX 索引操作数值框。
- 页面切换慢的代码侧原因是工作台、资产、用例、基线、执行、报告和配置页面被一次性打入主入口包。现全部按路由异步加载；构建后主 JS 从约 594.10 kB 降到 124.71 kB，各页面形成独立 chunk。所有性能页轮询和 AI 计时器均已确认在卸载时清理，没有发现切页后累积定时器。
- 家用基线当前 288 条首次复跑 `45ac741e-c8b4-4f26-a847-3ad6b0fab999` 为 288/288、100%、194.41 秒，证明本轮基线内容可完整通过。第二次复跑 `f08274df-b04c-4874-9634-29bf9da879da` 从 14:37:07 起停在 219/288，Safari 15:20 仍为 RUNNING；这是执行 worker/异步恢复问题，不能记成接口用例失败，也不能把首次通过掩盖该可靠性缺口。未自动重发剩余可能有副作用的请求。
- 本地检查：前端 87 个文件、617 项通过；执行器/统计/阶段报告 83 项通过；前后端静态检查 84/63、Python 编译、生产构建和 diff 检查通过。新版本尚未部署，Safari 线上仍是上一版本；部署后需复测首次切换、重复切换、浏览器前进后退和报告深链，并继续排查卡住执行的 worker 日志与安全恢复策略。

## 2026-09-09 受控内网压测目标页面授权

- Safari 真实验收隔离演示服务时，节点连通性检查已通过，但平台单用户预检被默认公网地址策略拒绝，错误为 `host resolves to a non-public address`；这不是 Agent、令牌或接口断言问题。
- 环境编辑页现在按单个服务提供“允许访问受控内网地址”开关，并随不可变环境版本保存。默认关闭，未勾选的服务和历史环境版本不受影响；只读详情会标记已授权的服务地址组。
- HTTP 执行器仅对当前请求引用且显式授权的服务放行私网地址，同时保留 DNS 结果校验和固定、服务基础地址、Host 请求头及同源重定向边界。即使授权，回环地址、链路本地地址、云元数据地址、未指定、组播和保留地址仍会拒绝。
- 本地验收：执行器测试 56 项、环境设置页 11 项、TypeScript 检查和生产构建通过；后端/前端静态检查分别通过。全仓串行门禁另有两条既有 AI 提示词契约失败，与本批文件无关，未作为本批通过项。
- 部署本批后，需在“压测演示（非业务）→ 隔离演示环境”的演示服务上开启该开关并保存新版本，再用本机节点执行 2 次/秒、30 秒的小流量验证，核对预检、三接口断言、Prometheus CPU/内存、节点证据和领导摘要。现有两条失败执行保留用于问题追溯。

本文件记录当前 Codex 交接状态，目的是减少长对话上下文依赖。每次完成一轮重要修改后更新本文件。

## 当前项目状态

### 2026-09-08 演示服务部署就绪竞态修复（main）

- 43.157.42.206实机确认容器RestartCount=0、cgroup v2启动成功，等待后health、demo/ok和metrics均为200；首次ConnectionResetError是compose启动后立即检查的就绪竞态，不是容器、cgroup或网络故障。
- check.py改为最多20次、每秒一次等待连接拒绝/重置/超时，HTTP应用错误不重试；脚本可导入测试且主程序行为不变。修正版独立标记1.0.1，避免同名1.0.0包产生不同校验值；运行中的1.0.0服务无需为此重启。
- 新增启动重置重试和HTTP错误不重试测试，load-demo 8项通过。修正版包为Downloads/midscene-load-demo-1.0.1.tar.gz，SHA256为e4b1b99adf6d625b147afe5595690ae1f9f602ac17a19f74ca35d1a05495f5f4，仍仅含11个白名单源文件且不含令牌。

### 2026-09-08 性能报告五项领导摘要（main，待部署）

- 报告第一屏由三项技术状态改为五项固定摘要：测了什么、怎么测的、结果怎么样、主要风险、下一步做什么。应用、场景、业务版本、环境名称、被测服务、配置压力、实际吞吐、持续时间、阈值和证据完整性均来自执行快照或确定性报告；环境无名称时明确显示未记录，不暴露内部环境版本ID。
- 风险按证据不完整、目标未达到、失败阈值、错误率、最慢步骤和服务资源缺口排序。通过只表述本次已配置阈值，禁止扩写为容量足够或系统无风险。下一步只有在既有策略允许预填时采用已校验建议，否则按补证据、同条件复验或接入服务监控给出动作，不自动发压。
- TDD验证过摘要入口缺失和环境ID泄漏两次预期失败；报告页8项通过。Safari用含真实容器CPU/内存采样的fixture验收：宽屏五项同排，约1000px窗口两列布局、最后一项整行，无横向挤压；fixture其余性能数据不是线上结论。
- 前端全量613项、生产构建、前后端静态检查及diff已通过；最终视觉回归与提交以本节后续提交记录为准。

### 2026-09-08 独立演示服务与真实容器资源验证（main）

- 目标是用户的43.157.42.206 Ubuntu 2核2GB，不是平台服务器；目前仅本地Docker验证，远端未部署。新增deploy/load-demo及白名单打包脚本，包不含令牌；默认127.0.0.1:18080/19090，不修改FRP 80/7000，不自动开放公网。远端安全联网仍需确认私网或TLS网关。
- 提供正常/延迟/HTTP错误/业务错误/有界CPU/共享64MiB TTL内存及可切换故障接口。3个独立凭据，非root、cgroup v2、0.5核256MiB服务及0.25核384MiB Prometheus。真实读取自身cgroup，不用压力机数据冒充业务。固定故障用于预检；正式失败演示在switchable通过预检后管理员短暂切换，不绕过门禁。
- 补齐容器内存working set相对新鲜有限memory limit的占比，未知或无限额保持未知；页面与导出解释分母。
- 本地真实k6 0.52.0：1次/秒30秒，30请求/30迭代、60断言通过、零HTTP错误/丢弃；P95 2.72ms仅代表本地演示。真实Prometheus查询：CPU峰值0.02897核（2分钟rate），内存峰值78.72MiB，配额256MiB；采集暂停后up=0且API正常。实际容器故障切换/恢复通过。
- Safari用实际采集数据验证资源组件/配额/占比及展开表；其余报告内容是fixture，不能宣称线上正式报告或AI诊断已完成。视觉回归通过。后端相关131、演示6、前端613项通过，生产构建及前后端静态、diff通过。
- 原始交付包/sha256在Downloads/midscene-load-demo-1.0.0.tar.gz；包11个源文件，不含镜像，需要目标机器拉取镜像。平台本批内存占比修复需要部署，Agent无需因此升级。用户Word/晋升资料未修改或提交。


### 2026-09-08 脑图重启任务漏显修复（main）

- 线上只读查明记录未丢：gen_1788858769663_00010（17:12:49）和gen_1788858816771_00013（17:13:36）在17:15部署重启后标记failed/服务重启中断。三条稍早任务success且右侧脑图在，共37份脑图文件。
- 根因：generationJobs先用isGenerateBackgroundJob按“生成/YAML”等文字过滤；mindmap_only中断后无生成关键字且无result，被过滤后到不了脑图中心的显式类型判断。新增generate/mindmap_only/agent_mindmap_plan类型识别，不依赖错误文案；更新静态缓存键。
- 回归用线上同型中断记录验证：修复前只识别yaml，修复后中断/排队脑图均保留、Runner/APK安装不混入。node回归/语法、前端静态84、diff通过。没有删除记录或自动重试AI任务；需部署前端后实页复验。


### 2026-09-08 场景与服务证据驱动下一轮建议（main，待部署）

- 新增独立 next-run-policy.v1 / AI v6：用不可变场景副作用、测试目的、原始负载、阈值/采样容差、每实例 CPU/内存/Pod 信号和压力机运行资源决定下一步。AI负责证据解释，最终配置由平台覆盖；未通过校验回退也沿用策略，不再任意升压。
- 网页测试条件新增可选业务目标、最大增长比例、观察时长；未填写不自动猜值。固定吞吐在完整只读资源证据和观察时间满足后按用户步长（最多50%）递增且不超过目标；85%是平台保守资源提示线，不是行业标准/验收阈值。
- 缺监控先接入、已有失败同压力复验、压力机异常先排查发压资源；写入/未知副作用、阶梯或突发、固定VU增长要求人工设计。长时观察完成不无限重复。样本允许原平台容差；监控至少3样本/95%覆盖，不拿整机冒充单服务，未知配额不能判断容器余量。
- 页面/下一轮向导/Word与Excel增加建议依据、验证目标和限制；历史AI无新策略需重新诊断。节点与连接/预检/启动门禁不变，不自动发压。缺监控不能直接复制仍缺监控的旧快照。
- 验证：后端策略/AI/PostgreSQL报告/导出/执行条件68项通过，前端613项通过，生产构建、前后端静态63/84及Python编译通过。保留Vite包体积提示。Safari本地构建夹具实点策略展示、配置跳转、网页选项展开、缺监控隐藏入口；不是新版本线上或真实AI v6验收。线上只读看到仍为v5，真实服务监控未接入。
- 本轮只需部署平台前端、后端和worker，不改Agent。用户Word与career文件未修改/提交。

### 2026-09-08 7e256b4线上复验

- 前后端与定时计划接口已确认7e256b4。API执行双节点只读短跑aad5742b-8d26-4762-b469-6b2d22b2f68c：全局2次秒/20秒/2VU，41请求，2.05次秒，2/2完成、计数一致、无缺失窗口、各5个运行资源样本，AI completed。不是容量验证。
- Safari实点AI阶梯建议跳转和门禁、停用计划创建/修改/归档/显示归档；AuditDone始终未启用，无通知。真实业务监控与自动计划运行仍未线上验收。
- 修复实看发现的忙碌节点被误提示需校准；前端17项、静态84、构建/diff通过。仅此提示修复待部署，Agent不变。用户Word/career未改。

### 2026-09-08 部署后双节点验收及剩余能力（main）

- 线上确认 c788b49、两台 Agent 0.1.5；Safari 同时双节点只读短跑 68b02f82-6d57-4615-8816-6b31d155c91d，全局2次秒/20秒/2VU，各节点1/1，42请求、2/2完成、3/3阈值通过、两台均有运行资源样本、AI v5正常无回退。仅小流量流程证据，不代表业务容量。
- 新增独立性能定时计划（0013）：默认停用、固定快照、只读准入、每轮权限/连通性/预检、无重叠与过期补发、启动/运行超时恢复、终态可恢复报告收尾及显式通知。页面应用筛选、5秒刷新、停用编辑与归档；没有线上启用或发送。
- Pod状态配置/查询/报告：精确instance/namespace/Pod/UID；重启为前2分钟滚动估计不可累加；最后观测状态带时间，缺失不作0。没有真实业务源，需用户提供只读URL和标签（凭据网页填写）。
- v3每轮一个临时资源finally清理，断言失败停止后续请求，清理失败计入链路失败；ID安全白名单、数字规范化、独立owner提取，禁止历史ID/重发创建/不安全清理。保留v1/v2字节，旧写执行加门禁。不支持通用polling、多资源、全局setup或强杀持久补偿。
- AI阶梯建议可跳转配置，但必须明确补充阶段后才能建草稿，不自动发压。Safari本地实点定时页来源/保存/刷新/归档/显示历史；修复全局panel最小高度空白，用户指出弹窗过大后改随内容撑高与折叠配置并截图复核。
- 检查：前端611项；最后相关数据库/后端208项，含真实k6/编译/预检专项68项通过；0013迁移往返、静态63/84、构建及性能响应式专项通过。独立审查发现的5项问题均修复复验。Vite体积提醒保留。新代码尚未部署；详情见docs/性能测试剩余能力验收-2026-09-08.md。
- 本轮不改Agent，无须再次升级0.1.5。main推送后部署平台前后端、迁移0013与worker/scheduler。用户根目录3份Word与docs/career未动。

### 2026-09-08 下一轮建议配置、多节点分配与AI纠正（main）

- 本轮实现报告“按建议配置下一轮”→配置向导；重读原执行与当前诊断并核对关联/应用/场景/环境，冻结原场景版本、环境、阈值和监控版本/必选性/窗口，沿用停止策略及测试条件。节点与生产确认重新选择，不自动发压；固定吞吐/并发可预填，阶梯建议缺少阶段时明确提示，不编造阶段。低吞吐可精确转换到每分钟，禁止取整扩大压力。
- 固定吞吐新增all_selected策略：所有选中节点共同分担全局目标与最大VU，保持优先级模式为默认；至少每台1个目标单位和1VU，无可分配容量/失效节点明确拒绝。数据库验证两节点总2次秒/2VU分成各1/1；修复审查发现的奇数目标取整误拒绝。
- AI已升级提示词v5；证据引用/结论数值/确定性结论冲突最多做一次定向纠正，再次失败或超时安全回退。仍对原证据校验，基础报告不受影响；没有声称真实模型纠正已成功。
- 线上Safari与API已确认平台88517c5、两台Agent0.1.5在线校准有效，各800VU/2400次秒可分配。升级后本机实际运行资源、同时两节点验收待新版部署后继续，不能用校准代替。
- 本地检查：Vue85文件602项通过；隔离PostgreSQL/分配/AI共51项通过；静态后端63/前端84、Python编译、生产构建、diff通过（保留Vite主包体积提示）。独立只读审查发现的两项P2已修复。性能响应式专项通过。
- Safari本地生产构建夹具实点报告建议→2次秒60秒配置、原版本阈值、节点选择与生产确认门禁、刷新深链恢复、返回清理参数；未向业务发压。修复实看发现的复选框换行、锁定环境仍标可切换，阈值中文化。模拟页已关闭，不作为线上端到端证据。
- 本轮不改Agent，无须再次升级节点。部署平台前后端和worker后继续线上验收。其他未关闭项仍见docs/superpowers/plans/2026-09-08-load-next-verification.md：真实业务监控、定时性能发压通知、Pod统计/跨Pod聚合、通用异步补偿。用户Word与docs/career未动。


### 2026-09-08 部署后Safari点检与修复（main）

- 已核对线上1d28c3b。专用Agent0.1.5、本机仍0.1.2；不能把平台升级算成本机Agent升级。
- Safari真实只读短跑：cea199a1-5181-4194-862d-c65feec4f8ea专用41请求/20秒，失败率0，5个运行资源样本；be6e5d00-b80a-4703-b6ff-05dcab9c9875本机20请求/20秒，失败率0但容差内计数差异、无运行资源。是分别单节点，不是同时多节点或业务容量验收。
- 验证连通性/预检反馈、自动完成、报告深链、Word/Excel下载（ZIP完整）、真实多轮条件差异、监控空态及表单取消；暂无真实业务监控配置，AI触发安全回退。
- 本轮修复低流量默认、禁用/失效节点选择、候选池/峰值说明、节点版本可见、校准自动更新文案、零请求全局伪步骤、AI回退可读性、多轮JSON折叠摘要、监控新增按钮换行。前端构建产物同步提交，不修改Agent代码。
- 本轮检查：隔离PostgreSQL报告16项通过；Vue84文件592项通过；后端静态63/前端84、Python编译、生产构建与diff检查通过（保留Vite体积提示）；响应式专项通过。Safari另开本地模拟数据页复核最新构建1次/秒10秒默认值、候选池解释和返回，不混作线上验收。
- 完整证据与未关闭项：docs/性能测试部署后验收-2026-09-08.md。新修复尚未部署；本机升级、真实监控接通、并行多节点、AI输出契约、定时性能发压/通知、Pod就绪重启、异步补偿等没有冒称完成。

### 2026-09-08 性能测试剩余能力集中交付（本地完成项，线上未部署）

- 用户要求先提交旧修改，再连续完成剩余项；旧修改已提交并推送 `f00f8a3` 到 `origin/codex/load-monitoring-report`。本段覆盖下方旧“未提交/仅主机/待实现”等阶段状态，但不覆盖业务监控源仍待接通的事实。
- 监控扩展：主机网络、磁盘、空间、平均I/O延迟；cAdvisor容器及精确Pod内各容器CPU/working set；指定PostgreSQL连接和提交/回滚速率。各口径、分母、缺失与实例分开，未提供的模板不伪装可用。`deploy/load-monitoring/` 提供Linux只读采集与网关部署资料，未远程安装。
- Agent 0.1.5 增加真实运行时cgroup/进程CPU与内存；首样本及无限额度为未知。实时及结束批次校验、合并、配额、截断说明进入报告和AI。修复停止时丢尾部指标、自动停止误标人工取消、并发锁顺序；旧v1冻结脚本保留原字节，新脚本v2记录阶段开始数。每分片分别核对阶梯吞吐，边界外请求独立保留；阶梯VU缺实际分阶段证据仍不能冒充达标。
- 页面加入目的/版本/缓存/数据规模、可编辑阶段、默认HTTP及链路错误标准、可选累计HTTP错误保护停止。场景高级编辑保留原请求/提取/断言，数据集可选或导入；非法或未应用编辑不会被回退流程吞掉。没有通用异步补偿或混合业务权重。
- 同一授权报告快照导出Word汇报稿与Excel数值明细；曲线含时间/单位/缺口、CPU口径及风险/AI依据，中文排版逐页渲染检查。按分钟到达率显式保留时间单位，避免放大60倍。Safari实际下载两个排版样例并验证文件ZIP完整性；样例不代表业务容量。
- 独立“性能分析”页面支持2–5轮同项目对比与已请求接口覆盖；不同条件只列原值。新增0012人工性能参考基线、冻结采纳理由/证据/阈值、手动退化核对；同场景/环境当前项唯一，来源执行删除不丢参考。未开启定时性能发压或通知，API功能基线不与其混用。
- 验收：最终相关后端443项通过，前端83文件587项通过；最后审查修复到达率时间单位及旧AI诊断复用：GET与request均匹配当前证据哈希和提示词v4。实际本地k6 v1/v2→Agent→隔离PostgreSQL对账及停止并发检查通过。0011→0012→0011隔离迁移往返通过；后端静态63/前端84、生产构建与变更Python编译通过。性能响应式视觉专项通过（桌面/宽屏/平板/手机/短屏）；Safari原生操作新建监控、勾选/取消、多轮选择、参考表单、报告资源明细与Word/Excel下载。网页API使用明示交互夹具，不是生产端到端压力验收。
- 本次根目录 `npm test` 的AI提示词评估仍有两项既存文案期望失败（mobile_print_record_generation、nameplate_print_boundary_generation）；本轮未改AI生成提示词，不能写全仓npm test通过。Vite仍有主包体积提醒。
- 升级包：`/Users/adouceshi/Downloads/midscene-load-agent-0.1.5-upgrade.tar.gz` 及同名 `.sha256`；最小源码20文件，不含.env/凭据/数据卷/镜像。`deploy/package-load-agent.py` 白名单可重复打包。先更新平台依赖、数据库至0012及API/worker/beat，再用 `LOAD_AGENT_IMAGE=midscene-load-agent:0.1.5 bash deploy/load-agent/upgrade.sh` 升级节点。没有部署平台或Agent、发送飞书或创建真实业务高压。
- 使用与未完成边界：`docs/性能测试完整使用与交付说明-2026-09-08.md`、`docs/性能参考基线与退化预警说明.md`、`docs/性能测试多轮对比与覆盖说明.md`、`docs/superpowers/plans/2026-09-08-load-completion-ledger.md`。真实业务监控URL/标签/只读授权和部署后多节点验收仍缺，不能凭旧附件规格替代；定时性能发压/通知、Pod就绪重启、跨Pod聚合及通用异步补偿未实现，明确保留。
- 用户根目录3份Word与docs/career/未修改或提交。

### 2026-09-08 监控表单 Safari 布局修复

- 用户截图发现全局 input width:100% 污染监控复选框，导致文字竖排和授权区域撑高。监控配置与执行选择两处都显式限定18px复选框；表单宽屏双列、窄屏单列，指标fieldset与授权区独立排列，按钮右对齐。
- Safari刷新本地生产构建后实际打开新增表单、切换CPU和授权勾选；窗口截图 /tmp/monitoring-form-safari-only.png 确认文字横排、复选框尺寸正常、底部空白消除。仅本地夹具，不是线上部署验收；窄屏尚未实机截图。
- 最新验证：相关前端28 tests passed、监控/运行/报告/统计/AI/任务后端166 passed、frontend_static_checks 84通过、生产构建通过（保留chunk体积提示）。仍未部署/提交。原计划的业务监控源对账及后续批次继续保留，不算完成。

### 2026-09-08 服务监控配置到报告第二批（代码已实现，业务源未验收）

- 新增环境监控目录、不可变配置版本、加密只读令牌与迁移0011。网页首次地址授权要求 platform.configure；配置维护 api.environment，执行人员可检查已授权监控。私网需显式授权，元数据/回环/重定向等防护保留。
- 环境“服务与监控”、向导勾选/就地新增/必选性/观察窗口、执行卡片检查已冻结版本已接入。启动门禁查最近五分钟同版本新鲜采样；监控源版本修改不改变已有草稿或历史报告。
- Celery 后台采集、去重锁、重启恢复、运行中回传、结束后观察+30秒补采、最终冻结接入。必选资源证据缺失时报告不能判完整达标，可选缺失只告警；AI等待最终监控快照，基础报告独立可读。
- 报告区新增整机CPU/内存图与明细，包含实例、来源、分母、采样缺口。当前仅node_exporter主机口径，容器/Pod/数据库/磁盘/网络及压力机运行资源尚未实现，不把整机冒充单服务资源。
- 实际Prometheus2.54.1 + 本地确定性exporter验收：CPU37.50004%对预设37.5%，2核/约0.75已用核；内存25%，1GiB总/256MiB已用。修复时间精度、内存时间戳重复标签、CPU缺核心/历史不足误判。冷启动CPU现在缺失而非误报96%。模拟exporter不是业务主机，源进程已清理。
- Safari本地生产构建已打开CPU/内存报告，查看37.5%/25%并展开明细；已操作执行向导勾选、检查和就地新增表单。配置API使用明确标记的交互夹具，不是业务环境端到端验收。
- 新增 docs/性能测试服务监控使用说明.md。真实监控地址/实例待用户提供或后续部署采集；源数据到线上报告验收未完成，未发送飞书、未发业务压力、未部署。本分支 codex/load-monitoring-report 尚未提交。

### 2026-09-08 用户追加：数量对账允许容差（本地）

- 按用户“数量上可以放宽”覆盖之前零容差门禁：同节点/步骤全程差值同时不超过2条及较大计数10%时，仅警告，不单独阻止性能阈值判定。21/19放行，5/3或1000/997仍阻断。consistent保持精确含义，新增acceptable和显式容差策略，原始数量不修补。
- 容差内报告主解释与明细仍注明耗时基于已收样本、尾部可能受影响；AI保留原始差异且不得无条件声称没有瓶颈。节点失败、损坏JSON、缺失窗口不放宽。
- 后端报告/统计/AI共48项检查通过。本地修改未部署；这不是行业标准或线上新结果。

### 2026-09-08 统计可信性修复与监控适配基础（本地，未部署）

- 已建立 `codex/load-monitoring-report` 分支，按路线图先处理统计可信性。真实本地 k6 0.52.0 对回环服务复现 console 与 JSON stdout 混流损坏；另修复 TextIO 预读与 select 组合漏读缓冲行。Agent 改用安静输出与字节流读行，对损坏指标明确失败，不再静默丢弃。版本准备为 0.1.4。
- 报告按同一 shard/step 全运行对账 HTTP 请求与耗时样本；允许窗口间偏移，不比较请求与业务链路次数。差异保留原值并判证据不足，不判业务失败。Safari 本地生产构建夹具实际展开计数明细，看到请求21/耗时19及证据不足；这不是线上新版验收。
- AI 新增采样一致性证据，精确允许 P95/k6 等名称但仍禁止编造数值；实际调用提示词 v3，拒绝无完整证据的 no_bottleneck 结论。复审发现的问题已补回归验证。
- Prometheus 只读适配器及安全测试已落地，但尚未接入网页配置、持久化和执行采集，不能称被测服务 CPU/内存功能完成；下一批继续此集成。旧报告原始流没有保留，不能反推历史每条差异原因或补造样本。
- 本轮没有发业务压测或飞书通知。用户原有未跟踪 Word 文档保持不动。尚未提交、打包或部署，不能要求用户按已发布版本升级。


### 2026-09-08 可配置服务监控与报告规划（未实现）

- 用户要求按公司报告维度设计，网页每次执行勾选服务、填写/复用配置并生成报告；随后明确“好好规划一下”。本轮只新增 `docs/superpowers/plans/2026-09-08-load-monitoring-and-report-roadmap.md`，不改运行代码、不发压。
- 重新读取两份 Word 正文和覆盖表共享文本，确认 cms-qa、wps-linux/WPS 核心服务、多实例规格、数据库与磁盘等参考维度；旧服务和配置不作为当前真实目标。优先复用 Prometheus 只读源，无监控时提供标准采集方案，附件只作历史证据。
- 交付顺序调整为统计可信性 → 单次多服务监控/CPU内存报告 → 采集部署与扩展维度 → 正式 Word/Excel → 多轮统计和性能回归。网页配置放环境“服务与监控”，执行时勾选；报告无词典入口。每批有真实 Safari/源数据对账验收，不把表单完成当成采集完成。
- 近期真实小流量的 20 k6迭代/21请求/19链路与耗时样本差异仍未定位；需区分阶段和范围再对账，不能解释为天然边界误差或宣布容量验证通过。被测服务运行期 CPU/内存和 AI 诊断契约修复仍未完成。

### 2026-09-08 最新部署压测真实验收与报告修复

- 用户追加要求：完整词典移到工作台，报告页连帮助入口也不展示。现工作台工具栏进入独立“压测使用帮助”，报告移除词典及入口；帮助默认展开可搜索，支持返回工作台。25 项相关 Vue 测试、生产构建、前端静态84项通过；构建产物已重新生成，尚未部署。
- 用户指定后续使用 Safari：实际切换 Safari 平台标签页，当前仍在登录页且无登录态，已请求用户在浏览器登录，不索要聊天明文密码。之前本轮在线真实压测证据来自 Chrome，不能写成 Safari 验收通过。
- 随后用户完成 Safari 登录；已在 Safari 实际创建、连通性检查、预检、启动并打开实时报告，执行 `67d6629c-2b4f-458a-987d-1fb0be00f002` 使用本机节点、1次/秒、20秒、预分配/最大2VU，自动回传21请求/1.05RPS、三类失败率0、节点1/1完成，当前门槛判通过。观测22.215993秒；P95=10ms高于实测最大6.18ms、AI数值校验退回均再次复现。20目标/21请求的边界计数仍待核对，不能称严格20次计数验收完成。未做高压或双节点运行。Safari曾打开无查询参数的入口出现404，实际from=login入口恢复；shell两入口均200，未确认是缓存或瞬时部署问题。

- health 实测部署为 0fe6264；Chrome 重新加载后确认新指标词典和统计口径页面已生效。两个节点校准有效、心跳正常，各可分配 800 VU / 2400 次每秒。未修改容量策略。
- 实际通过网页创建专用节点只读执行 `797e3776-79bf-4d5d-9ebc-2c57e8386c52`：公开网页配置接口、固定吞吐 1 次/秒、20 秒、1 VU；连通性与预检均显示通过，启动后状态自动更新、最终报告自动出现。19 请求、0.95 RPS、HTTP/业务/链路失败率均 0，节点 1/1 完成、正常退出。报告判定负载不足，不能算容量验收通过；未发飞书报告。
- 新词典展开与中文“失败”搜索实测成功。页面吞吐分母为配置 20 秒，观测时间 53.966028 秒；19/20 差额尚需进一步核对 Agent 原始点与 dropped iterations，不能推断业务容量不足或擅自降低门槛。固定 VU 实测及双节点分片仍未验收。
- 发现 P95=500ms 大于实测最大值 292.47ms；原因是分桶上界没有受实际最大值约束。本地修复取二者最小值，仍保留分桶估算口径。另修复未达标时仍固定显示“达到99%”文案，现显示实际迭代率、完成比例和真实达标状态。新增两项复现测试，修复前两项失败、修复后通过。
- AI 诊断本轮因“结论不能复述数值”被安全校验退回，未采纳模型根因；此问题未修复。统计及 AI 纯单元检查 12 通过/6 数据库检查跳过；统计与报告检查 10 通过/12 数据库检查跳过（本轮未配置 TEST_DATABASE_URL）；后端静态 63 项、差异检查通过。上述两个报告修复尚需部署后复验，不覆盖之前保留的真实运行证据。

### 2026-09-08 短信基线号码替换与真实复验

- 按用户指定将家用 `/print3d/api/v1/auth/sendSms`、共享 `/share/open/v1/BLogin/sendSms` 收件号码改为用户指定的尾号 9531；明确授权后各实际请求一次，09:13:41 / 09:15:16 返回 HTTP 200、code=0、data=null。两条 v4 已采纳为当前基线；这证明接口接受请求，不代表已确认手机实际收到短信。
- Chrome 实际打开两条每天 08:00 定时任务编辑页，家用已选目标含 sendSms v4，共享已选 10/10 含 sendSms v4；只读核对后取消编辑。采纳逻辑会重指向原定时基线目标，本次无需重复保存任务或部署代码。
- `auth/checkCode` 新号码 v3 调试使用示例验证码，实际返回 data=false；现有“data 存在”断言误显示通过，不能证明验证码有效。未由本次操作采纳该版本；定时任务当前仍选择旧 v2。此项需真实验证码与正确业务断言后再完成正向验证，不算本轮已修复。
- 本轮没有重新执行整套基线，没有重复发送短信。今早 qidi/component/info 失败修复及压测统计真实验收仍是独立未完事项。
...
### 2026-09-08 升级复验及早间单条基线失败调查（未完成）

线上health仍返回64640e2，不含统计修复3afcea1；网页报告缺少新词典/统计口径。节点页两节点心跳正常但都在校准中，可调度0，未发起压测，不能记为升级验收通过。
今早08:00:12家用基线任务ff5c59fa-e7bf-4a9b-ab08-3bc9119f68fb共290条，289通过/1失败；共享10条通过。唯一失败为GET /print3d/api/v1/qidi/component/info，componentSn=1380568721287430144_EP3，HTTP200但code60101002、msg=model component does not exist、data=null。用例无前置步骤、固定组件号。通过用例管理单独实际只读复验，仍HTTP200/code60101002，139ms（调试页面草稿版本bf079012-6790-48d0-a03b-a9e9220c7f77）。未改断言、未采纳新基线、未整批重跑。
同组件号的普通GET /print3d/api/v1/model/detailByComponentSn在今早基线通过，不能直接认定该模型全局删除；后续需核对启迪接口数据范围/映射及有效组件来源，不能换错码断言掩盖正向失败。执行记录“编辑用例”深链还复现“执行记录对应的接口不在当前接口版本中”，从用例管理搜索能正常查看，是另一入口缺陷，未修复。



### 2026-09-07 网页验收部署与登录阻断

按用户要求开始线上网页验收。Chrome 扩展连接两次超时后改用原生 Chrome，打开带 verify=3afcea1 的性能报告页，实际被重定向到登录页，未进入报告。健康接口返回部署版本752fbf9；Git祖先检查确认不包含3afcea1。本轮未发压、未修改线上数据，不标记统计功能线上验收通过。待平台部署该批修改、Agent升级0.1.3及浏览器恢复登录后，继续报告词典/搜索、统计口径、无样本与真实最小负载完整验收。

### 2026-09-07 统计分析计划第一批 P0

用户授权开始执行 docs/superpowers/plans/2026-09-07-performance-statistical-analysis.md。本批仅完成第一阶段中的采样证据门禁与解释入口，不是五阶段全部完成。Agent版本升至0.1.3，新增实际VU汇总和最大采样间隔，服务端严格校验、兼容旧数据缺字段。固定并发要求所有节点共同时间段持续达到分配压力，缺失/稀疏/不同时峰值不再误判达标；阶梯目标按积分并换算分钟单位，缺少阶段对齐证据时保持证据不足。必选阈值无对应样本时 actual=null、不通过。报告公开统计时长与口径，新增可折叠搜索的中文指标词典。指标聚合保留当前及前一个窗口吸收乱序数据，已完成窗口再次收到数据会明确失败，防止部分桶覆盖；运行时验证失败状态、进程终止和清理。

后端/Agent/真实PostgreSQL聚合102项、前端专项10项、生产构建、后端静态63项和前端静态84项通过。独立审查先发现稀疏采样与分钟单位错误，已补反例修复并复审。本地浏览器固定样本验证与线上真实k6验收分开；本机没有k6或现成镜像，未真实发压。

待续高优先级：阶段精确对齐、实际发起量采集、稳定期统计分母、真实节点验收。阶段二多轮计划、阶段三监控对比、阶段四覆盖/基线、阶段五正式导出尚未开始，详细台账在计划末尾。部署必须先平台后端，再各Agent0.1.3；旧执行无VU证据不补造通过。

### 2026-09-07 性能报告可读性与排版优化

按用户要求参考 JMeter Dashboard 的汇总/分位数/趋势分区和 Locust 的统计明细组织方式，延续既有领导汇报设计方向。性能报告页调整为清晰的标题、核心数字、解释、明细字号层级；摘要采用高对比文字；补充 P50/P90/P95/P99 中文解释和毫秒单位，业务断言或链路无样本时显示“—”。接口与步骤统计接入已有报告数据，按后端 P95 排序展示，不新增未经采集的服务端指标。

P95 图新增时间轴、毫秒刻度、网格线、键盘/鼠标/点击数据点详情；缺失和零请求窗口不再画作 0 毫秒，时间缺口不跨越连线。仅对已配置的 p95_ms less_than_or_equal 标准绘制全程参考线，并明确单个时段不能替代全程判定。补充耗时分桶估算解释，数据表入口改为中文业务描述。主平台功能测试报告和飞书通知卡片不在此轮修改范围。

验证：报告/图表专项 8 项、生产前端构建、前端静态 84 项通过；本地固定数据浏览器回归覆盖桌面、平板、手机、矮屏与历史切换，包含无横向溢出和数据点键盘交互检查。使用的是本地报告样本，未对线上重新发起压测。构建仍有原有主包大于 500 kB 提示。发布包含 api-test 构建产物；线上需要部署后查看，不能把本地截图当成线上已生效。

### 2026-09-07 最近三天家用 API 基线固定失败修复

线上核对 2026-09-05 至 2026-09-07 的每日基线后确认，共享任务连续三天均为 `10/10`；家用任务连续三天均为 `282/290`，而且每天都是同一组 8 条失败，并非随机网络抖动。根因分为三类：4 条用例把“当前默认设备顶层 `printStatus` 必须为 `standingBy`”作为与目标无关的固定前置条件，但账号当前默认设备长期保留已完成切片任务的顶层 `slicing` 状态；3 条模型用例直接使用未按设备类型筛选的 `model/page` 第一条数据，列表首条详情实际已下架；1 条把“当前设备一定没有切片任务”当作每日固定断言。

8 条用例均创建不可变新版本并先做单条或小批调试，全部通过后才采纳为活动基线。模型详情、队列和收藏链路调整为先读取当前设备类型，再携带 `deviceType` 查询可用模型；今日推荐与设备状态链路移除与目标无关的待机限制；设备设置更新改为检查设备内部真实待机状态；切片任务接口改为稳定契约，同时覆盖 `data=null` 和完整任务对象；重新打印链路记录原选中设备，切换到用户授权的专用测试设备 `1385270626257522688`，创建后立即取消、删除记录并恢复原设备。设备切换恢复先用独立临时用例验证通过，临时用例随后归档。

家用任务仍保持 290 条且自动替换了 8 个新活动基线；家用和共享每日 08:00 任务均从旧接口版本 `ef46b8fd-2868-459d-8695-fc5daba853ed` / 旧环境 `26eb31f3-8f34-431e-9e39-3d33add597e2` 更新到当前接口版本 `0a64dd01-ecdb-428d-adbe-7c4b2fd4f7de` / 当前环境 `46271e34-35c2-4a19-bb84-dc949dc7be82`，飞书通知和原目标范围不变。完整家用回归执行 `893deea4-f1aa-40ed-9a72-210996283e4b` 最终为 `290/290`，0 失败、0 阻塞，飞书发送成功；重新打印用例的切换、创建、取消、删除、待机确认和原设备恢复步骤全部通过。

### 2026-09-03 API性能测试 Task 14 Docker Agent 部署与运维

独立 k6 Agent 已补齐可交付的 Docker Compose、安装、升级、检查和卸载脚本。Compose 使用固定 k6/Python 镜像版本、只读根文件系统、受限 tmpfs、命名凭据卷、CPU/内存/PID 上限、健康检查、`no-new-privileges` 和全部 capability 移除，并且不挂载 Docker Socket。首次安装等待注册凭据写入后会清空宿主机 `.env` 中的一次性令牌并强制重建容器，避免令牌继续存在于容器环境；重复安装、平台发布和 Agent 升级均保留凭据卷，普通卸载也保留，只有显式 `--purge` 才永久删除。

节点页复制命令已指向真实的 `deploy/load-agent/install.sh`。发布包包含 `load_agent` 构建上下文、Compose、运维脚本和中文说明，同时排除 `.env`、Agent 凭据、身份数据库和运行数据集。安装与运维支持多个可配置镜像候选；本机真实构建仍在读取 Dockerfile 基础镜像元数据时被 `docker.mirrors.ustc.edu.cn` EOF 阻断，尚未进入 Dockerfile 构建阶段，不能声称本机镜像构建成功。7 项部署脚本测试、14 项 Agent 配置/客户端聚合、节点页 4 项交互、Compose 实际渲染、生产前端构建、前端静态 84 项和 shell 语法均已通过。下一步是 Task 15 受控目标端到端门禁及真实单/双 Agent 验收；真实 Docker 验收需要在可拉取镜像的服务器执行。

### 2026-09-03 API性能测试 Task 11 节点管理与校准闭环

API平台已新增权限感知的“性能测试”导航组，包含性能场景、压测执行、性能报告和压测节点四个独立入口；当前节点页已经可用，其余三个入口将在下一批向导和报告页替换临时引导页。节点页支持中文搜索输入法、加载/空/失败状态、一次性注册令牌、可复制的Docker启动命令、首选/普通/备用/停用四级调度、完整硬上限与软上限、当前占用、校准容量和六种校准状态。所有禁用按钮均显示原因，本机备用节点不会自动参与的规则在页面常驻说明；当前HTTP传输风险仍明确展示，未标记为已解决。

“校准节点”不再是前端假按钮：服务端持久化一个幂等校准命令，普通心跳不会覆盖；空闲Agent在心跳响应收到命令后运行本地k6校准，将关联命令ID的成功或失败结果写入私有文件并在后续心跳回传。离线、执行中、无管理权限或正在校准的节点均会被阻止并给出中文处理方法。协议/服务/HTTP专项35项、API前端全量507项、生产构建、前端静态84项已通过。真实Docker Agent构建和单/双节点校准仍属于Task 14/15服务端验收，当前不能声称线上节点已校准。

### 2026-09-03 API性能测试 Task 10 用户API与飞书报告

压测用户侧HTTP边界已经实现：场景及不可变版本、数据集、运行、事件增量读取、确定性报告、AI诊断、节点与一次性注册令牌均通过`/api/api-testing/v1/load-*`提供。实现使用独立`load_testing_http.py`并只在现有API测试分发器中挂载一次，没有继续扩张大路由。读取、编辑、执行和节点管理四类权限分开校验，项目/环境范围在资源读取和写入时再次验证；注册令牌只在创建响应显示一次，后续列表不返回令牌。重复启动已在HTTP边界幂等返回当前运行，重复停止沿用服务层幂等行为。

性能飞书卡片与功能回归卡片分开：包含场景名、环境、目标与实际负载、确定性结论、P95/P99、HTTP错误率、节点证据、AI状态和性能报告链接，不读取或发送原始样本与凭据。最后一个分片结束后只派发一次确定性报告归档任务；报告结论先写回运行记录，再排队AI诊断，诊断完成或失败后尝试发送飞书，飞书未配置不会破坏报告。专项HTTP、Agent完成、通知共35项通过；扩展压测服务聚合74项、主平台访问控制52项和后端静态63项通过。

当前仍有一个必须在Task 11/12继续收口的真实缺口：预检要求每个Agent上报目标环境连通性，但现有Agent协议还没有用户可触发的目标连通性命令。当前后端会明确阻止缺少连通性证据的预检，不会假定节点可用；节点页、Agent命令和预检轮询必须形成闭环后，才能进行真实单节点压测。
...
用户确认环境由平台自由配置，不做仅限压测环境的硬限制；生产环境继续要求现有`api.production`权限和风险确认。主平台机器默认`fallback`且受Docker和Agent硬上限保护，第二台专用服务器可设为`preferred`。远程Agent只访问平台Agent API，不直连Redis/PostgreSQL；当前平台HTTP传输风险仍未解决，跨公网传递环境密钥必须先配置HTTPS，或使用受控私网/VPN并显式启用私网传输。

第一阶段设计已经包含多Agent协议和真实双节点验收，但先不加入定时压测、混合权重、多地域和自动扩容。设计已经由用户确认并进入测试驱动实现；当前仍未完成镜像、部署脚本、用户页面和真实节点验收，不能部署为可用压测功能。

用户已明确批准按设计实施。详细实施计划已写入`docs/superpowers/plans/2026-09-03-api-load-testing.md`，按15个测试驱动任务拆为持久化域、Agent控制面、场景编译与分片、Docker Agent、指标报告、AI诊断、用户API、Vue体验、部署和单双节点真实验收。

Task 1 持久化基础已经完成：新增11张性能测试表、`0010`迁移和事务仓库，支持场景不可变版本、运行/分片、五秒指标桶幂等写入、有序事件及有限样本。测试先以缺失模块失败；实现后又真实发现并修复PostgreSQL 63字符约束名上限。隔离PostgreSQL schema内4项仓库测试与16项迁移测试通过，迁移覆盖空库升级、重复升级和降级到base；后端静态63项及`git diff --check`通过。用户要求所有压测术语同时提供中文名称和用途/风险提示，该要求已加入前端测试与验收。Task 2权限与Agent注册已经完成：新增四项性能测试权限、旧身份库内置角色一次性升级、一次性注册令牌、Agent凭据哈希、心跳、四级调度状态和软/硬容量校验；停用节点凭据不能再认证，也不能原地重新启用。92项后端聚合测试、40项Chromium身份交互、后端静态63项和前端静态84项通过。Task 3独立Agent HTTP协议已经完成：`/api/api-testing/load-agent/v1`只接受Agent凭据，浏览器Bearer不能冒充节点；支持注册、心跳、领取分片、启动、停止命令读取、指标/样本/事件和完成上报。分片归属、1MB请求体、批次数量和重复指标均有门禁；整批内容先校验后写入，避免坏数据造成半批成功。201项后端聚合测试通过，含59项既有HTTP契约；测试身份库必须位于服务账号拥有的0700私有目录。Task 4场景准入和k6安全编译已经完成：严格限定HTTP请求、五种步骤作用域、有限等待、断言、变量提取和数据引用，拒绝任意JavaScript；真实打印、设备控制、支付、短信、高成本AI及缺少归属/清理的写操作返回中文原因和修复办法。四种k6模型均可确定性生成，密钥只保留`__ENV`引用，数据按VU或全局迭代序号读取，全局初始化/清理由控制服务负责而不会在每台Agent重复执行。所有负载模型、场景模式、数据方式、任务优先级、节点级别和阈值均提供中文名称、用途与风险提示。26项场景专项测试、Node语法检查、后端静态63项通过；连同既有迁移、Agent、HTTP、权限与身份的PostgreSQL聚合回归为243项通过。Task 5数据集与分片已经完成：CSV/JSON使用UTF-8严格校验，拒绝重复/不一致字段、路径穿越、超限文件和密码/Token/密钥列，标准化内容以0600权限写入0700私有目录，数据库提交失败会删除孤立文件；预览只展示最多三行并遮盖手机号、邮箱。节点调度按首选、普通、明确允许的备用顺序分配，硬上限、软上限、有效校准容量和当前占用共同限制VU/迭代率；未校准、过期、版本不匹配、无进程槽或不健康节点不能参与。多节点整数分片总量精确，固定数据按VU分区、独占数据范围不重叠、循环数据明确标记复用，容量不足返回差额而不突破节点。27项数据与分片测试含真实PostgreSQL持久化通过，后端静态63项通过。用户新增要求正式运行前必须校准Agent，设计和Task 6/7/11/12已补充本地校准、七天过期、签名失效、逐节点目标连通性与中文状态。Task 6预检与运行编排已经完成：每个选中节点必须具备有效本地校准，控制端执行恰好一轮功能请求且主体失败仍尝试清理，每台节点独立上报目标DNS/连接/TLS结果；按实测链路耗时反推到达率所需VU，容量不足只有明确选择后才允许排队且结论固定为证据不足。任务冻结场景、环境、负载、阈值、编译器、节点及校准快照；Agent在草稿/预检阶段无法领取分片，全部节点就绪后才收到统一开始指令。停止状态持久化供Agent轮询，启动屏障或运行分片失联会恢复为失败/证据不足且不会自动迁移压力。108项压测域聚合测试和后端静态63项通过。Task 7 Docker Agent运行时已经实现：严格配置、一次性注册凭据私有持久化、本地CPU型k6校准、七天有效期及版本/硬件签名失效、五秒有界指标桶、私有脚本/数据文件、运行中低频心跳、停止后SIGINT到SIGKILL收敛、密钥脱敏和文件覆写清理。107项压测域与Agent聚合测试、后端静态63项和Python语法检查通过。Docker镜像构建在本机基础镜像清单拉取阶段被已配置镜像源EOF阻断，尚未进入Dockerfile执行，因此镜像本身仍需在服务端构建并检查；服务端部署说明将提供多registry mirror及直连回退。Task 8 指标与确定性报告已经实现：Agent同步保留固定边界延迟直方图、业务断言和完整链路总数/失败数；服务端校验分片归属、运行状态、已知步骤、五秒窗口、时间顺序、有限非负数和直方图兼容性，并以批次ID和唯一时间桶双重保证重复上报不翻倍。报告分开呈现负载目标、HTTP传输、业务失败、链路失败、丢弃迭代、合并百分位、步骤排行、节点、脱敏样本、严格可比历史和证据完整性；目标未达到、窗口缺失、节点丢失或停止均为证据不足，HTTP 200下的业务失败可单独使必选阈值不通过。123项压测域与Agent聚合测试、后端静态63项和Python语法检查通过。Task 9 AI诊断已经实现：只把负载目标、阈值、结构化时间窗、步骤、节点资源、分类样本和证据完整性送入模型，丢弃样本原文与其中的提示注入内容；Authorization等敏感值再次脱敏。AI输出必须使用固定瓶颈分类、引用真实evidence_id、提供有界中文建议和下一轮参数，证据不足不能伪造根因。相同证据默认复用诊断，人工强制重诊只新增诊断记录且不会重跑压测；超时或格式错误单独记为AI失败，不影响确定性统计报告。27项AI/指标/任务专项测试、AI Gateway静态46项、后端静态63项和语法/Schema检查通过。Task 10起的用户API/UI、部署和真实节点验收仍未实现，当前版本仍不能从页面发起压测。

### 2026-09-03 3D API 核心链路统一与补充实跑

线上版本为`f4012f905be795a0c861e51c761804a7150ace4a`，PyYAML可用。当前12条核心基线已经统一为精确分组`核心链路`，任务`2975548b-8fa8-43fd-b92f-3b019ca17211`改名为`3D核心链路回归（v11）`并包含12条可执行目标。Safari真实点击执行`117f0e97-1a43-4da4-82fe-3f2e14896b67`通过12/12、耗时122.16秒，产品失败、脚本/数据和环境异常均为0；任务报告、全部状态筛选、新增用例阶段证据与完整轨迹均已点击，飞书通知发送成功且任务名正确。

新增真实打印、CAD产物导入清理和中文搜索详情三条家用核心链路。真实打印使用用户授权的Mac`9888E0094F2A`、SN`1385270626257522688`，用例v2`c2d727eb-3de0-4abf-99ca-7f26fc369f29`完成创建、查询、取消及设备状态复核；统一任务后设备为`ready / standingBy`、`printingFlag=false`。模型导入v1发现`/createModel/delete`错误传入模型SN仍返回HTTP 200/业务码0，但模型没有删除；首次产物已用数据库ID`2111033634`补删。v2`471eb365-a361-421d-ba3c-3c2c75ff45b8`先从`createModel/page`提取数据库ID，再删除并断言详情返回1001、`模型不存在`、`data=null`，连续调试`dc7b8861...`、`08d57071...`及统一任务均通过并清理。中文搜索用例`44d9550e-2f91-47e5-b530-2d8cec6af30e`以`收纳盒`完成结果到同一模型详情断言，连续调试`17b25df7...`、`106906ef...`及统一任务均通过。

当前活动基线303条（家用293、共享10），其中12条属于核心集合；每日08:00任务仍为家用290和共享10，本轮没有替换调度或执行300条全量回归。旧共享、CAD和硬件独立任务作为诊断入口保留，不删除历史。用户明确排除设备首次绑定/解绑、共享商城下单支付、EPOne/EPONEPro及京东之家；现有共享只读契约保留。在该范围内核心链路已无待补数据，但这不代表Apifox 1031个接口全部回归。

Safari登录成功后返回裸`/api-test/#/...`仍能命中nginx 404；本地`js/auth.js`在API登录回跳增加`?from=login`并保留hash路由，`tests/identity_frontend_check.js`新增目标回归，待本次提交部署后再次用Safari验证。健康检查时进程RSS与峰值均为232.0MiB，无活动大请求、重任务或后台执行。全平台逐按钮和多角色深链点检仍未全部结束，HTTPS、受限成员按应用安全使用Agent、公开静态HTML报告和外部Sonic权限隔离仍是未解决边界。

### 2026-09-03 A112-A116 配置、报告与多成员权限续检

线上版本仍为`1aac3f31ec67b34204f655fa0a126033ac065b88`，PyYAML可用。本轮在Safari继续实点环境体检、执行环境、群通知、应用配置、模型配置、报告、成员权限和API配置/定时任务。应用配置保存后弹窗没有关闭，A112保存成功即返回管理页；操作记录中API工作流预览、任务执行、基线批量分组和用例分组仍显示泛化HTTP动作，A113补为中文业务动作。

管理员已把每日08:00任务更新到当前全部活动基线：家用`290/290`、共享`10/10`，共300条；家用开启一次性用例，共享关闭一次性用例，两条任务均启用飞书。卡片会区分“当前目标”和历史最近结果，历史仍分别为283/283和8/8。本轮只保存目标，没有触发300条全量执行，不能把历史结果当成新配置的执行证据。

真实账号矩阵覆盖管理员、普通测试成员、只读成员和受限范围测试成员。只读成员只能查看资产、运行记录和报告；受限成员只看到授权UI模块和`3D家用`API项目，Agent入口因安全边界禁用。线上API设置和定时任务虽然最终由后端拒绝写入，但仍允许无权限成员进入编辑、切换和保存，反馈过晚。A114-A116在会话校验后保留当前权限快照，对API定时任务、生产环境、环境新建/编辑/归档/恢复及飞书配置提前禁用并解释原因；归档单独要求`api.delete`，恢复只要求环境配置权限，服务端仍是最终授权边界。报告中心初次读取时曾短暂显示0条和空态，A115改为明确加载态，真实数据返回后再显示250条、成功227、失败23。

本轮临时账号`codexflowtester0903`、`codexflowreadonly0903`、`codexflowlimited0903`均已停用。受限成员停用后，Safari下一次API页面请求立即跳回登录页并保留API返回地址，证明会话撤销即时生效。仍未完成全平台全部按钮：主平台其他报告与多成员深链、API其余写入口的权限前置反馈及完整写入/清理闭环继续点检。HTTPS、受限成员按应用安全使用Agent、公开静态HTML报告和外部Sonic权限隔离仍未解决。

A112-A116本地验证包括API前端65个文件502项、主平台专项62项、前端静态84项、API静态5项、主平台26张及API平台25张响应式视觉截图、TypeScript/Vite生产构建、JavaScript语法和`git diff --check`。新构建为`index-Czt7xvbQ.js`/`index-BbxEKnv5.css`，主包约571KB的拆包提醒继续保留。

### 2026-09-03 3D API v11 核心业务链路

线上产品版本为`1aac3f31ec67b34204f655fa0a126033ac065b88`，PyYAML可用。已从Apifox默认模块v11的1031个接口中建立9条当前版本核心链路基线：家用稳定6条、共享稳定2条、DeepSeek CAD按需1条。新数据没有覆盖或移动旧291条活动基线；当前活动基线共300条（家用290、共享10）。每日08:00调度随后已在A112-A116续检中更新为家用290加共享10，但尚无300条新执行结果。

稳定组合任务`3D核心业务链路回归（v11）`为8条，Safari执行`bc99d003-7137-47a5-be46-a7cf3f1aa0ad`通过8/8、4.52秒；API执行`39252585-904c-4065-82e5-37bbf9e79f15`也通过8/8并成功发送飞书。共享独立任务`3D核心链路-共享稳定回归（v11）`执行`1e766f8a-f2c4-4c67-a9f8-131ca0fb2a6f`通过2/2。

CAD用例v3`2ee68542-3050-4f06-9f97-7ef6783b494c`按真实响应从`$.data`提取整数任务ID，有界轮询`SUCCESS`及OBJ/预览产物，使用10字以内名称完成改名和列表验证，再删除本次任务并要求列表总数归零。两次调试`f4966182...`、`ad9ea292...`及Safari任务执行`c2189c08...`均通过1/1；失败版本创建的临时任务也已经精确删除。该用例位于`核心链路 / 扩展 / 按需执行`，不进入固定日常任务。

Safari已逐一打开9条用例，核对v11、分组、调试/基线/回归状态与结构化编排；稳定和CAD任务均点击生产确认、用例明细、阶段证据、完整轨迹、报告与全部状态筛选。首次执行列表曾约22秒才出现，随后三次50条列表读取为367/2174/1578毫秒，暂未稳定复现，继续作为冷启动或前端渲染性能项观察。

扩展链路边界：用户明确本轮不做京东之家及EPOne/EPONEPro，因此这些接口不建用例、不进入核心任务，也不再索取设备或组件数据；其只读发现结果仍保留，其中京东之家为业务码1001、`404:null`，EPOne设置查询路径为HTTP404。真实打印仍需要专用空闲打印机及允许创建后立即取消。完整证据见`docs/evidence/3d-api-core-flow-run-2026-09-03.md`和`docs/evidence/3d-api-core-flow-inventory-2026-09-03.json`。

### 2026-09-02 A111 定时任务加载反馈与开发中接口门禁

线上健康接口仍返回`eca2b0d`，未包含已经推送的A104-A110；因此本节界面修复仍需部署后强制刷新复验。当前进程PyYAML可用、无后台或重任务，RSS约436MiB、峰值695.6MiB。

API定时任务实际配置已核对并更新为家用283条、共享8条，共291条活动基线；两条任务均启用、允许一次性基线、开启飞书、每天CST 08:00执行，下次运行是9月3日08:00。Safari已打开家用任务的283条手动执行确认并取消，编辑页也显示283/283及7条一次性基线；这证明当前配置范围，不代表291条已经完成一次新的定时执行。

退出编辑后目标列表重新加载时，旧页面把正常配置短暂显示成红色“执行已阻断”。A111改为中性的“正在校验目标，配置未变”，读取期间禁用手动执行，只有目标读取完成且确有问题时才显示阻断。目标测试先在旧实现复现红色误报，再验证新反馈。

同步新增的“查询京东之家共享设备状态”已建立动态前置：先调用`hpDevice`取得当前设备SN，再带入主体请求。Safari试运行前置通过并取得SN，但完整调试`9011fdbd-f4d1-46ed-addb-51b4ac7507af`仍返回HTTP 200、业务码1001、`404:null`。接口契约同时标记`x-apifox-status=developing`，说明是开发中的临时联调接口；不能为进基线而把成功断言放宽到1001。A111在接口详情和“从接口创建用例”动作区醒目标出开发中，说明当前环境可能尚未上线，候选必须真实调试通过后才能采纳。

本轮仍未完成全平台所有入口和多成员矩阵。HTTPS、受限成员按应用安全使用Agent、公开静态HTML报告和外部Sonic权限隔离继续保持未解决。

A111完整API前端65个文件497项、TypeScript/Vite生产构建、前端静态84项、25张桌面/平板/手机/短屏截图和`git diff --check`通过；新构建为`index-BK6nwGdn.js`/`index-BSo82qYr.css`。

### 2026-09-02 A110 页面知识库业务模块筛选

Safari在线A103将页面知识应用从智小白3D切到小白学习时，模块仍停留3D打印基线并显示误导性空结果；打开模块下拉还看到两个应用的业务模块、`cache`和多条`AI_Agent_修复重跑_*`历史混在一起。A110按当前应用只展示其绑定的业务模块，切换应用时重建模块选项并清除不兼容选择，同时排除缓存和自动修复历史，保持这些记录与固定业务基线分离。

浏览器回归先在旧代码复现不兼容模块残留，再验证当前应用范围和系统历史排除；26张主平台响应式截图、JavaScript语法与差异检查通过。线上仍为A103，本轮只清除了页面筛选，没有删除知识或修改基线；A110待部署后用Safari复验。

### 2026-09-02 A109 页面知识库表单上下文

Safari在线A103页面知识库已逐项点击刷新、新建、导入Figma、首条编辑和测试库/基线库切换；新建、导入与编辑均没有出现可操作表单，只有库类型切换真实生效。本轮已把首条资料从测试库切到基线库后立即移回测试库，没有留下数据变更。根因之一是三个表单入口都会重新切换到通用资料中心，丢失页面知识库上下文；A109保留当前管理器，弹窗打开和关闭后仍停留页面知识库。

继续按完整流程检查发现，管理器切换到小白学习后仍保留3D打印基线模块，页面显示误导性空结果；新建、导入或编辑还会被当前YAML模块反向覆盖为另一应用，存在知识资料写入错误应用的风险。A109在切换应用时清除不兼容模块，并让管理器选中的应用显式传入表单。浏览器回归先复现两个问题，再验证联动筛选、新建与Figma表单、应用保持、管理器上下文和返回AI生成均正确；26张主平台响应式截图通过。线上health仍为`eca2b0d`，PID4030533、RSS408.9MiB/峰695.6MiB、PyYAML可用且没有后台或重任务，说明A104-A109均未进入当前进程。页面知识删除确认已打开并取消，其余筛选与主平台/API入口继续实点，尚未完成全平台验收。

### 2026-09-02 A108 应用编辑上下文、生成反馈与操作记录可读性

Safari继续点击应用配置、群通知、成员权限和AI生成。应用配置先编辑智小白3D，再关闭并点“新增应用”时，旧版把名称、包名、Sonic、飞书和模块完整带入新增表单，保存会有误覆盖现有应用的风险；A108进入新增时先清空全部草稿。四步弹窗原来始终只写“应用分组”，从群通知进入也看不到正在配置哪个应用；现在标题和状态区明确区分新增、编辑、未归属模块分配，并显示目标应用、包名和当前步骤。

成员权限的操作记录实点到第2页，工作台、用例、任务、基础正向预览和UI报告等请求仍显示`POST平台操作`或`PUT平台操作`；A108补齐中文业务动作和对象。AI生成实际完成空提交、应用/模块/业务选择、Figma高级范围、页面知识范围、刷新、全选和清空；字段已经修正后，全局错误提示仍残留。A108在清除表单校验状态时同步移除这条旧Toast，新错误仍会在下一次提交时重新校验。

用户表示部署后，health在强制刷新前后仍返回`eca2b0d`，PID4030533、RSS408.9MiB/峰695.6MiB、PyYAML可用且没有后台或重任务，因此线上实际仍停在A103，A104-A108均不能记为已部署。A108应用配置21项、身份前端38项、前端静态84项、26张主平台响应式截图、JavaScript语法和差异检查通过。页面知识库已打开并读取智小白3D的20条历史资料；本轮没有保存应用、修改知识库、生成YAML、执行Runner、删除历史或发送飞书。全平台其余入口和API定时任务继续逐项实点，尚未完成全平台验收。

### 2026-09-02 A107 Agent终态、报告统计与重跑范围

Safari从一条既有失败记录逐项打开用例、质量、YAML、校验、日志、报告、失败、修复、未产出缺陷和总结。页面能够切换全部产物，但终态仍显示“当前步骤/当前阶段”，阶段卡写成“失败：执行中/失败：安全重跑”，失败对象直接显示`[object Object]`。A107按任务状态切换“当前步骤/结束步骤”和“当前阶段/结束于”，把Runner执行与安全重跑分别写为“未通过/未恢复”，结构化失败只提取可读结论并保留下方完整诊断。

同一逻辑用例的原始失败和修复重跑在报告中被写成“失败用例2条”，下方又重复列出两条“未知”；A107把它们标为失败执行记录并注明逻辑用例数，已有结构化执行卡时不再重复旧失败块。总结页三个Runner动作均已真实点击并在确认框取消；“执行全部”错误把3条可执行与9条人工用例合计为12条，“继续下一批”把2条显示成2/3且仍提示首批冒烟。A107只按可执行YAML计数：首批1/1、剩余2/2、全部3，继续批次提示不会重复首批，人工用例不会进入Runner计数。本轮没有创建Runner任务、删除历史或发送飞书。

用户表示已部署后health仍返回`eca2b0d`，PID4030533、RSS408.7MiB/峰695.6MiB、PyYAML可用且无后台/重任务；因此A104-A107均仍待线上部署复验。A107新增4项终态专项与既有7项Agent专项全部通过，前端静态84项、JavaScript语法、26张主平台视觉截图及差异检查通过。全平台其余主平台与API入口继续实点，不能作全部完成结论。

### 2026-09-02 A106 执行中心Trace与YAML批量移动流程

Safari继续逐项点击执行中心的Trace、Runner状态、失败重跑、待处理和Sonic入口。顶部动作在非Trace页写“刷新 Trace”却实际切页，Trace内部`window.open`入口在Safari没有出现新页；A106让顶部按当前页显示“查看 Trace/刷新 Trace”，三个Viewer入口改为带Trace ID的真实新页链接。Sonic未选YAML禁用、去资产选择、5条当前模块同步确认、1条已选同步确认、批量删除确认和清空选择均有反馈，所有确认已取消。

批量移动真实下拉仍包含唯一源模块、`cache`及`AI_Agent_修复重跑_*`自动历史，会产生无意义同目录移动或把正式资产混入系统历史。A106排除唯一源模块和内部历史，目标按“业务与固定基线/其他模块”分组；自动修复重跑记录继续与固定基线分离。Runner状态为1个在线Runner、1个可执行设备和2条绑定记录；失败重跑筛选、空态、分页、定位，以及待处理选择、批量确认/拒绝取消、显示更多均已实点。本轮没有同步Sonic、删除数据、保存Trace快照、Runner执行或飞书通知。

线上health仍为`eca2b0d`，未包含A104-A106；PID4030533、RSS约397.5MiB/峰695.6MiB、PyYAML可用且无后台或重任务。A106新增19项执行导航回归中的2项先红后绿；部署后需Safari逐点三个Trace链接、动态顶部文案和批量移动目标分组。全平台其余主平台与API入口仍继续点检，不能作全部完成结论。

### 2026-09-02 A105 Agent范围真实性与可选安装返回链路

Safari在Agent工作台逐项打开执行范围和模式后发现两个主链路问题：选择“指定模块”没有出现模块选择器，选择“只分析”后页面仍展示Runner设备、App安装和“启动Agent”，提交数据还会把模式改回`AUTO_SAFE`，存在只想分析却实际生成YAML并执行的风险。A105为指定模块增加仅来自当前应用配置的选择器，前端和直接API都阻止空模块，服务端只在精确同名目录中检索，避免`家用基线`误包含`家用基线旧`。只分析模式现在明确显示“分析配置/开始分析”，隐藏设备和安装，关闭YAML生成、校验、Runner、修复、重跑与缺陷策略；服务端再次以步骤白名单强制只执行资料准备、规划、影响分析、用例检索和匹配。

继续点击“加载更多模型”成功读取204个模型；点击“安装/更新App”能进入执行中心，但“暂不安装”原来只能去调试执行，无法回到Agent，割断了刚填写的目标和配置。A105保存Agent表单来源，安装页按来源显示“暂不安装，返回Agent”，空态也统一返回上一步；从执行中心直接进入安装仍回调试执行。本轮未启动Agent、创建安装任务或触发Runner。

用户表示部署后线上health实际为`eca2b0d`，PID4030533、RSS214.4MiB/峰472.9MiB、PyYAML可用且无后台或重任务，只包含A103，A104/A105仍待部署复验。A105目标前端24项、后端2项、后端静态63项、前端静态84项、Python/JavaScript语法、26张主平台视觉截图及`git diff --check`通过；部署后必须在Safari重做指定模块、只分析预览/启动边界、安装返回Agent和普通执行中心安装返回路径。全平台逐按钮点检继续进行。

### 2026-09-02 A104 生成评审状态、Safari下载与上传键盘操作

Safari在历史生成结果评审中实际点击复制检查点、重新生成确认取消、下载脑图、重建脑图、删除确认取消和“生成 YAML”。下载脑图连续点击两次均没有进入Safari下载记录，本机也没有新增`.mm`文件；原因是认证下载点击后立即撤销Blob URL，Safari尚未接管就被取消。A104把对象URL保留60秒再回收，其他浏览器仍走同一认证和文件名逻辑。脑图报告的测试人员、版本、飞书需求和用例平台历史都为空，但旧页面仍给出可点下拉框和删除按钮；现在显示“暂无历史记录”并禁用无意义操作。

“生成 YAML”弹窗已点击模块/应用、家用共享、页面知识刷新/全选/清空、文件选择与无资料提交；没有创建后台任务。文件上传区原来是只支持鼠标的`div`，A104为生成、脑图、页面知识、YAML上传和Agent资料上传统一增加Enter/空格激活及可访问名称。生成结果中的单项采纳/不考虑会清空状态文字，批量操作仍显示“待处理”；A104统一显示“已采纳 / 本轮不考虑”，批量操作同步每一项状态，避免界面与提交内容矛盾。

线上health在用户表示部署后仍为`269d51c`，PyYAML可用，RSS318.1MiB/峰484.5MiB且无后台或重任务；A101-A104都不能记为线上通过。A104目标测试21项、前端静态84项、26张主平台响应式截图、JavaScript语法和`git diff --check`已通过；部署后必须用Safari复验真实脑图下载、空历史禁用、批量状态和上传键盘操作。全平台逐按钮点检继续进行。

### 2026-09-02 A103 脑图报告反馈与结论真实性

Safari在脑图中心真实点击37条历史记录的选择、清空、合并报告空态和首条“生成报告”，报告页显示2条自动化、1条人工；清空后点击“预览报告/生成报告”实际产生了必填错误，但状态组件移除了`show`类，页面没有任何反馈。A103保留状态可见类，错误、忙碌和成功反馈都会留在报告页。进一步点击“仅选冒烟→包含人工用例→预览报告”后发现，旧服务把没有执行证据的自动化用例全部写成通过，并在1条人工项未确认时仍显示100%、质量通过、建议发布。A103改为只按真实Midscene报告或显式执行结果统计；无证据为未执行，人工项单列待确认，失败、阻塞、未执行、人工待确认或缺陷未闭环时均不建议发布。前端统计区把“总计”明确为“自动化总计”，并展示待人工确认数。

验证已通过：脑图报告服务12项、主平台交互反馈14项、后端静态63项、前端静态84项、26张主平台视觉截图、JS/Python语法和`git diff --check`。线上health在用户表示部署后仍为`269d51c`，RSS320.5MiB/峰441.2MiB，无后台或重任务；因此A101、A102和A103仍需重新部署并用Safari复验，不能记为线上通过。全平台逐按钮点检继续进行，脑图报告本轮没有持久化正式报告、删除历史或触发生成任务。

### 2026-09-02 A102 生成记录长读取即时反馈

Safari从AI生成点击“查看生成记录”后的前约3秒仍停在原引导页，按钮和页面没有任何加载状态；继续等待后才出现75条生成记录。页面实际能加载，但用户无法区分读取中和点击失效，刷新按钮也在请求期间保持原样，与此前“按钮无响应”的体验一致。A102在请求前立即切换到生成记录加载页，说明任务可离开弹窗后查看，并保留返回AI生成；读取失败留在页面显示中文错误和重试路径。记录页刷新改为按钮禁用及“刷新中”，成功或失败都有明确反馈。浏览器回归用受控未完成请求先复现无加载提示，再验证即时提示和请求完成后的列表；26张视觉流程截图、前端静态84项、JS语法和`git diff --check`通过。待部署后须Safari再次实点首次进入与刷新时的即时反馈。

### 2026-09-02 A101 AI生成内Figma导入保持生成上下文

A100部署后，Safari继续从AI生成真实点击“导入 Figma”：工具栏切成“资料中心”，导入表单没有成为可操作前景；在当前资料页再次点击也没有给出可见表单。代码路径中`showKnowledge()`会无条件把当前工作流改成资产管理，因此即使从生成流程打开临时导入弹窗，关闭后也会落入资料上下文，生成链路被打断。A101让AI生成入口显式调用`openKnowledgeFigmaImport("generate")`，只在该入口保留AI生成工作流；页面知识管理器和资料中心自己的Figma入口仍按资产管理处理。新增浏览器回归验证导入表单可见、活动流程仍是generate以及关闭弹窗后生成上下文不丢失，旧代码先失败、修改后通过；26张视觉流程截图、前端静态84项、相关JS语法和`git diff --check`通过。待部署后须Safari再次实点导入、空解析提示、关闭和返回生成页。

### 2026-09-02 A100 AI生成到页面知识库的真实导航

Safari从“AI生成用例”真实点击“页面知识库”后，标题短暂变化但最终落到YAML用例资产表，“页面知识/Figma”入口与按钮承诺不符。根因是页面知识管理器创建后会刷新左侧YAML目录，目录渲染逻辑看到当前工作流仍是资产管理便再次调用`showAssetsCenter()`，覆盖刚渲染的知识库。A100在目录刷新时识别已经打开的知识管理器，不再覆盖；从AI生成进入时增加“返回 AI 生成”，保持生成资料准备的上下游关系。浏览器回归先复现“找不到知识管理器”再通过，并实际点击返回入口恢复AI生成引导；主平台前端静态84项、JS语法、26张视觉流程截图及`git diff --check`通过。待部署后须用Safari重新点击“AI生成用例→页面知识库→返回 AI 生成”完成线上复验。

### 2026-09-02 A99 AI生成表单旧错误随输入清理

Safari在新建自动化测试弹窗真实点击空“生成YAML”后，正确提示“请选择已启用应用”；随后选择智小白3D会加载20个页面知识，选择业务也能生效，但旧错误仍固定显示在资料区，容易让用户误判选择失败。A99为生成弹窗的输入和选择变更统一清理已经失效的错误状态，保留下一次提交时的完整必填校验。浏览器回归先复现失败再通过；主平台前端静态84项、JS语法、26张视觉流程截图和`git diff --check`通过。待部署后用Safari再次执行“空提交→选应用→选业务”，确认旧错误即时消失。

### 2026-09-02 A98 API源码与发布构建包一致性

用户部署`7a304da`后，health已确认线上revision正确，PID3973973、RSS/峰值108.0/205.7MiB，后台队列和重任务为0。Safari普通刷新后，282条历史执行的单用例证据按钮仍显示旧文案“重跑失败项”；线上`/api-test/index.html`继续引用旧`index-BrCGj6Wo.js`，旧包不含A97的“仅重跑当前失败项/重跑全部失败项”。部署脚本默认`BUILD_API_TEST=0`，所以revision更新并不等于Vue源码已重新构建。未点击旧按钮，没有触发生产回归。

A98重新生成并跟踪`api-test`哈希构建产物，入口改为`index-D2azL2v4.js`；新增静态回归直接读取待发布包并要求包含两种明确重跑范围，测试先在旧包失败、构建后5项通过。待提交部署后必须再次普通刷新，并实际点击单用例按钮确认环境、用例名和数量1后取消；顶层“重跑全部失败项”也需单独确认范围后取消。

### 2026-09-02 A97 失败项重跑范围、动作下一步与审计可读性

用户部署后已确认线上 revision 为 `c6d33dfff83c4766314bab2510775a0b09bb951b`，PyYAML可用，主平台HTML返回`no-cache, must-revalidate`，A96普通刷新策略和统一静态版本线上生效。Safari继续实点模型策略测试、账号菜单/改密/会话确认、环境快速/深度体检、Sonic旧步骤扫描、失败处理、缺陷草稿、API工作台五个详情标签、执行记录、报告和定时任务；所有删除、撤销、推荐配置、刷新桥接、282条手动执行确认均取消，没有改写历史或触发大任务。

当前定时任务为家用282+共享8，共290条；家用编辑器显示282/283，额外1条活动基线未选。家用任务删除和282条手动执行确认均能响应，最近执行可进入282条历史的分页、证据和失败报告。共享在9月2日08:00有8/8自动调度证据；家用卡片只有9月1日最近手动结果，需继续判断是恢复调度晚于当天08:00还是实际漏调度，不能用“明日08:00”代替执行证据。

真实点击发现严重范围错误：282条历史报告的单用例证据按钮写“重跑失败项”，确认框却准备重跑全部282条生产用例。A97让抽屉和嵌入证据只重跑当前失败版本，按钮明确为“仅重跑当前失败项”，确认数量固定1；诊断报告顶层另设“重跑全部失败项”，两类操作都显示环境、范围并确认。子集重跑的新执行名称增加“部分重跑 9/279 条”等范围，保留来源任务ID但不再伪装为完整固定基线；执行历史仍与活动基线、定时任务目标分离。

主平台同步Sonic的四个无YAML动作此前只闪现提示并停在原页，A97统一引导到用例资产后再显示长提示。操作记录为常见Apifox、项目、用例、基线、执行/重跑/恢复、任务改名和YAML模块维护补充中文业务动作名。API工作台共享域、店铺分组、查询省份详情五标签、结构化/原始JSON和接口执行筛选均实点可达。Safari自动化可输入英文，但直接Unicode不能替代真实中文输入法候选验证；用户报告的全局中文输入仍需部署后人工拼音逐页复验，不能宣称关闭。

A97完整验证已通过：API前端65个文件495项、执行服务14项（另40项因本机未接测试库跳过）、身份与审计38项、主平台前端静态84项、后端静态63项、API生产构建、后端语法、26张主平台视觉交互截图及`git diff --check`。API生产包仍有565.61kB单块提示，作为后续首屏性能点检项保留，不能误写成构建失败。全平台剩余按钮、写入/清理闭环和多成员矩阵继续点检，不能作完成结论。

### 2026-09-02 A96 普通刷新、配置向导与执行/失败处理衔接

用户部署后公开 health 仍为 `3f51c195af03c65e9d0a8f484b6ab5be98ac0254`，PyYAML 可用；进程 PID 3915344、RSS/峰值 230.4/447.0MiB，无后台或重任务。公开 `task-manager.html` 和带 A96 查询参数的 JS 仍没有 `Cache-Control`，证明此前只改应用响应头未穿透实际 Nginx。Safari 普通刷新因此同时加载了新 `agent-workbench.js` 和旧 `execution.js`：历史 Agent 的用例/YAML 中文展示已生效，但“编辑 YAML”仍打开占位脚本；忽略缓存后同一按钮才正确停留原页并提示文件已清理。A96 新增真实 Nginx 静态缓存片段，并让安装/同步脚本复制、校验、失败回滚和重载；主平台相互依赖的五份脚本统一使用同一版本标识，避免再次混用。

继续真实点击应用配置、群通知、执行环境、缺陷草稿、失败处理、执行中心、Sonic 同步和用例资产后，A96 修复：应用向导切换时清掉上一步错误；未选应用时业务提示先选应用；执行环境“编辑绑定”直接进入设备/Sonic 步骤；`cache`及所有`AI_Agent_*`生成/修复记录不进入应用业务归属；历史 Agent 侧栏状态使用最终“部分通过”等业务结论，YAML 按实际文件清单计数；脑图已清理提示给出“AI生成用例→脑图中心→刷新脑图文件”的真实路径。缺陷草稿“从失败任务生成”首次进入曾错误显示空列表，手动刷新才出现23条历史失败；报告和失败处理现在进入即强制加载完整管理历史。失败任务分页、返回执行页和定位提示已实点可用。

执行/资产链路中，选中 YAML 后批量 Sonic 同步、批量移动和批量删除均能打开确认并取消，未改线上数据；批量移动原来默认选择源模块，现改为必须显式选择目标。Sonic 子页原来平铺最多60条最近任务，现只显示12条并明确当前文件入口与“用例资产”批量入口。A96 专项43项、前端静态84项、后端静态63项、Nginx配置4项、JavaScript与部署脚本语法、diff检查及26张响应式视觉回归通过；本机没有`nginx`命令，真实`nginx -t`必须由部署脚本在服务器/容器执行。A96 待提交、部署及普通刷新复验；主平台其余配置、多成员矩阵和 API 全平台按钮继续点检，不能作全平台完成结论。

### 2026-09-02 A95 历史 Agent 产物动作与用例/YAML 口径

Safari 在强制忽略缓存后已加载 A93，并继续实际点击历史任务 `agent-1785757359037-e05447ad` 的用例、YAML、日志、修复、缺陷、总结、复制、下载 YAML、下载脑图和编辑 YAML。结构化产物与总结零值已生效，复制显示“当前 Agent 产物已复制”。继续点击发现：用例页把 16 条设计用例和 10 个 YAML 引用混成约 26 张卡片；YAML 状态仍显示英文 `executable`；已清理的历史 YAML 会打开一个看似“已保存”的兜底模板；下载 YAML 在脚本全被清理时会静默下载 JSON 索引；下载脑图会打开没有 Authorization 的新标签并显示“登录已失效”。这些动作表面可点，但结果与按钮承诺不符，不能计为流程通过。

A95将用例设计与 YAML 执行门禁分开计数，YAML 引用不再伪装成额外用例，YAML 状态显示中文。`openFile`先读取并验证真实 YAML，成功后才切换编辑上下文；历史文件已清理时停留原页并说明从运行记录查看，不再生成可误保存的占位脚本。Agent 下载只输出真实 YAML，全部丢失时明确阻断，部分丢失时说明实际下载和缺失数量。主平台所有脑图下载入口统一使用当前会话认证请求并下载 Blob，不再打开未认证标签页；相关调试、执行、历史跳转只在文件真实打开后继续。

15:59 再次核对线上 health 仍为 `51910a9d03d31af4c5b334ddd41d21b7d12a1128`，首页和 JS 响应仍没有 A94 的 `Cache-Control: no-cache, must-revalidate`，所以用户本次部署不能记为 `b000803` 已上线。进程 PID 3879002、RSS/峰值 458.3/698.1MiB，后台队列和重任务为 0；本轮没有触发 Runner、生成、删除、归档或飞书。A95 的 Agent/资产/历史动作专项 23 项、前端静态 84 项、后端静态 63 项、5 份 JS 语法、diff 检查和 26 张主平台响应式截图全部通过；用例桌面/移动、YAML、日志及移动整页截图已人工复核。单元素截图透明区域在查看器中显示白色，移动端整页背景实际连续，因此没有误记为平台缺陷。A95 待提交推送及用户部署后 Safari 复验；全平台逐入口点检继续进行。

### 2026-09-02 A94 部署后静态资源缓存失效

用户部署`51910a9`后，公开health确认线上revision正确、PyYAML可用，PID3879002、RSS/峰值306.8/325.3MiB且无重任务；Safari重新载入并打开同一历史Agent任务后，“用例”仍显示旧版整块JSON。直接读取服务器`js/agent-workbench.js`已存在A93结构化渲染，证明问题不是代码漏部署，而是主平台JS/CSS统一返回`Cache-Control: public, max-age=3600`，同时HTML中的静态资源版本参数长期未更新，浏览器会在一小时内合法复用旧脚本。

A94把主平台HTML及`/assets`、`/css`、`/js`改为`no-cache, must-revalidate`，允许浏览器保存但每次载入必须向服务器确认；API测试平台带内容哈希的构建资产继续保留原缓存策略。`round5.css`、`agent-workbench.js`、`agent-status.js`和`app.js`同步更新资源版本，确保本次部署立即绕开已有的一小时缓存。新增静态服务回归先复现旧`max-age=3600`后通过。待提交推送并由用户部署后，再用Safari普通刷新确认A93结构化用例/YAML/日志和0值显示生效，同时核对响应头；这不是全平台点检完成结论。

### 2026-09-02 A93 Agent产物逐标签与结果可读性

在线仍为`c532647adf4e86c234bbcb26302429e76f906b66`时，Safari继续打开历史任务`agent-1785757359037-e05447ad`，逐项点击计划、用例、质量、YAML、校验、日志、报告、失败、修复、缺陷和总结。计划、质量、校验、报告、失败、总结已经有结构化界面；用例、YAML和日志仍直接输出大段原始JSON。修复页同时显示“当前无可应用YAML”和同目标重复的“已生成YAML草稿”，旧尝试与当前门禁结果混在一起；脚本问题的缺陷空态只说“当前无需”，没有解释为何不生成产品缺陷；总结中0值被通用HTML转义函数转为空白，指标只剩标题。

A93为用例增加业务目标、入口/需求范围、自动化/可执行/需确认/草稿/人工一次性分层和可展开步骤卡片；为YAML增加文件、分数、冒烟、校验/隔离状态、编辑入口和明确下一步；为日志增加中文阶段、终态计数、耗时、摘要和工具/产物数量。三类原始JSON均折叠为技术详情。修复尝试按目标合并，顶部单列“当前可应用YAML”，历史候选明确不代表门禁通过，长失败日志默认折叠；脚本/环境/待归因失败的缺陷空态解释不会自动创建产品缺陷。通用`escapeHtml`保留数值0和布尔值，Agent阶段`SUCCESS/PARTIAL_FAILED/SKIPPED`统一显示中文。

新增6项专项回归，并为用例桌面/移动端、YAML和日志补充4张截图；专项连同Agent历史/报告/资产/失败处理共19项、前端静态84项、JS语法和diff检查通过，主平台视觉脚本26张通过，新增界面已人工查看。A93已以`51910a9`提交推送并部署；Safari复验发现旧静态缓存仍使页面运行上一版脚本，转A94修复缓存策略后再逐标签复验。还需继续检查顶部复制/下载、总结中的安全跳转和其余主平台入口。全平台、多成员及完整执行清理闭环仍未结束。

### 2026-09-02 A92 Agent报告按逻辑用例去重统计

Safari从运行记录“部分通过”任务点击“查看报告”，正确回到同一Agent轨迹的Runner报告产物，但运行记录显示最终逻辑结果8条、通过5、失败3，报告摘要却显示执行用例11、通过5、失败6。报告摘要直接统计Runner任务行，把同一逻辑用例的修复重跑尝试重复算成新用例。A92优先复用Agent持久化总结中的逻辑用例口径，摘要改为“逻辑用例8、通过5、未通过3”；原始11次Runner执行和其中3次修复重跑作为补充说明保留，明细仍展示全部尝试，既不重复计数也不丢诊断证据。新回归先复现11/6覆盖8/3，修复后通过；连同A90/A91相关16项、前端静态84项、JS语法、diff检查和22张响应式截图全部通过。A91/A92已以`0d4b653`提交推送，待用户部署后以同一历史任务Safari复验。

### 2026-09-02 A91取消任务与部分通过筛选分离

Safari在Agent运行记录实际点击刷新、首条输入详情展开、选择当前页、批量删除确认后取消、清空选择和状态筛选。“部分通过”筛选返回14条，其中一条卡片同时显示“执行结果”和“Agent 已取消”：它在取消前完成5/8条，旧前端先按阶段性通过率分类，覆盖了最终取消状态，导致筛选统计和卡片主状态不可信。A91让最终`CANCELLED`优先于执行比例：卡片主标签固定显示“已取消”，颜色和边框使用取消状态，进度仍保留阶段性5/8证据；筛选新增独立“已取消”，取消记录不再进入“部分通过”。已完成的`DONE`任务仍按通过/部分通过/失败结果分类。专项1项、与资产/运行相关15项、前端静态84项和diff检查通过，待提交部署后Safari复验。

### 2026-09-02 A90自动生成/修复历史与固定基线隔离

用户部署`c532647`后，公开health确认线上revision为`c532647adf4e86c234bbcb26302429e76f906b66`、PyYAML=true，PID3823047，RSS/峰值402.6/701.0MiB，后台队列和重任务均为0。Safari强制刷新后继续主平台逐控件点检：执行中心空安装表单能给出设备选择提示，“不安装，直接执行”可进入调试；同步Sonic未选YAML时按钮正确禁用并能跳到资产选择；Runner待处理仍有127条，批量操作在未选择时禁用。用例资产真实选择一条YAML后，批量移动和批量删除确认均能打开并取消，未修改线上数据；Safari原生拼音输入“模型”能够进入资产搜索并筛出3条，证明该输入框的真实输入法可用。新建YAML和上传YAML后来重新逐项点击，并用页面可见截图及辅助访问树确认两个弹窗均正常打开后取消；最初判断来自弹窗刚出现时辅助访问树未及时更新，已更正，不能把控制层盲区记成产品故障。

资产目录同时暴露固定业务/基线模块、多个`AI_Agent_*`自动生成/修复目录以及空`cache`，自动修复重跑记录会混入长期维护目录，空目录也只能逐个处理。A90把资产默认范围改为“业务与基线模块”，将`AI_Agent_*`和`cache`集中到独立的“AI 生成/修复历史”；主资产表与左侧YAML目录均不再把系统历史逐模块平铺，新建、上传及AI生成目标下拉禁止选入系统历史。历史区显示历史模块、YAML和空模块数量，并提供带删除清单确认的“清理空历史模块”；实现再次校验只删除零YAML目录，单个失败会保留并明确反馈，任何含YAML的修复记录都不会进入批量清理候选。专项4项、相关主平台/身份45项、前端静态84项、JS语法检查和22张响应式截图通过；资产业务区与历史区截图已人工复核。A90尚未提交部署；线上仍会混排，部署后必须用Safari点击业务区、历史区、空目录清理确认取消，以及新建/上传模块下拉复验。

### 2026-09-02 A89前置/清理步骤保留完整接口契约

继续在Safari线上`99f7592bbe774760fb7864c075e9bc58dd99d20a`使用未保存的`GET /api/v1/jdHome/device/isSharedDevice`手工用例点检编排。主体请求正确带有查询参数`deviceSn`，但同一接口分别加入前置步骤和清理步骤后都显示“参数 0”，原始JSON中的`query`也是空对象；这会让新人以为从接口选择即可运行，实际调试才发现缺少必填参数。根因是步骤选择器只拿轻量接口清单，创建步骤时丢弃了OpenAPI参数和请求体示例，没有像主体用例创建那样读取完整接口定义。

A89在添加或重新选择前置/清理接口时先读取`/endpoints/{id}`完整定义，再复用主体用例的参数、Cookie及请求体示例构造逻辑；读取期间禁用重复选择并显示“正在读取完整接口定义”，读取失败保留选择器和搜索条件，以中文说明失败原因并允许重试，不再静默生成残缺步骤。成功和失败回归分别验证`deviceSn=sn-001`被带入以及详情失败时不提交步骤。

本轮还真实点击了前置/清理接口选择、复制、排序、启停、变量选择、高级配置、轮询、原始JSON、断言和提取增删、清理手工配置、测试数据行/变量增删、主体输出变量增删、共享用例依赖以及前置/后置处理增删；全部停留在未保存草稿并已离开页面丢弃，没有执行生产接口。步骤“删除”按钮多次点击未取得确认框或删除结果，不能计为已验证，部署后继续复查。完整API前端65个文件491项、前端静态84项、TypeScript/Vite生产构建、25张响应式截图和`git diff --check`通过；新产物为`index-BrCGj6Wo.js`/`index-DeaUqoo1.css`，构建仍提示主JS约564.7KB的拆包风险。

A89已以`58c36a1`提交并推送，并随`c532647`部署上线；本轮已核对release revision，但尚未再次保存一条带前置/清理参数的真实用例，因此“代码已部署”和“Safari业务闭环复验”仍分开记录。全平台逐按钮、多成员矩阵和完整执行/报告/失败修复/重跑/清理闭环仍未全部结束；HTTPS、受限成员按应用安全使用Agent、公开HTML报告和外部Sonic权限隔离继续保留为未解决边界。
...
## 2026-09-02 主平台 Agent 历史与 YAML 编辑器继续点检

- 线上强制刷新后仍为 `eca2b0d4b4546038d0ed7f853cf55ae26c735cd7`；本节改动尚未部署。health 48.0ms，PyYAML 可用，RSS 433.8MiB、峰值 728.1MiB，无大请求或后台任务。
- Agent 运行记录改为进入页面即加载；顶层遗留 DONE 但步骤失败且无报告的任务按失败展示，避免成功假象。
- YAML 编辑器两侧面板同时收起时改为单列满宽；未改内容不再发送保存请求，服务端也不会创建重复历史版本。
- 取消 AI 修复输入或确认后保持当前页面和操作区，不再误切到修复工作流。
- 单文件、批量和整模块物理删除确认补充永久删除、不可恢复及受影响范围；后端删除失败不再从前端假移除，批量失败项保留选择并显示成功/失败数量；Safari 只打开并取消确认，没有删除历史数据。
- 已点击 Agent 历史筛选/选择/删除确认/重跑确认/报告/轨迹，以及编辑器刷新、保存、侧栏、历史、复制、移动、重命名、状态、基线参考、AI 修复、模块选择、批量移动和批量删除；全平台其余入口和四类成员权限矩阵继续点检，不能标记完成。
### 2026-09-03 性能场景与压测执行向导

- 新增性能场景三步向导和压测执行向导，四种 k6 负载模型均按后端编译器的精确字段生成；P95 使用确定性报告支持的结构化阈值。
- 节点容量按硬上限、软上限、有效校准上限的最小值扣除当前占用；容量不足默认阻断，显式继续时报告标记证据不足。
- 新增 Agent 到环境服务的 DNS/TCP/TLS 连通性命令。执行页逐节点展示结果，所有节点通过前不允许单用户预检。
- 生产环境同时要求 `api.production` 权限和页面风险确认；服务端仍做最终权限校验。
- 场景版本安全校验失败时自动归档刚创建的空场景，避免场景列表产生不可编辑的垃圾壳记录。
- 本地验证：PostgreSQL 真实契约 29 项、API 前端 70 个文件 518 项、生产构建、后端静态 63 项、前端静态 84 项、Python 语法和 `git diff --check` 通过。
### 2026-09-03 性能实时控制台、确定性报告与 AI 诊断

- 性能报告页按“确定性结论 → 吞吐/错误/时延/阈值 → 趋势与节点证据 → AI 诊断”的顺序展示，负载达到与阈值通过分开判断。
- 新增压测事件 SSE：浏览器先用登录会话换取短期不透明票据，URL 不携带会话令牌；断线后从最近事件序号切换为有界轮询。
- P95 趋势图提供等价数据表并明确缺失窗口；节点证据支持展开，环境/模型/参数不一致时阻止误导性的历史对比。
- AI 诊断显示模型、提示词版本、证据哈希、引用和置信度；AI 失败不影响确定性报告，重新诊断不会重跑压测。
- 完成后仍按项目飞书配置自动通知，并新增报告页手动发送入口；消息包含场景名称、环境、负载目标、错误率、时延和证据完整性，不发送原始样本。
- 本地验证：报告组件 14 项、PostgreSQL 报告/运行/AI/HTTP 契约 32 项、生产构建、Python 语法和四种视口真实 Chromium 视觉门禁通过。

### 2026-09-03 性能测试受控集成、N 节点拆分与节点状态页

- 受控集成测试首次实际打通任务创建、双 Agent 领取、指标、完成、报告和 AI 超时链路，发现并修复 Agent 领取结果缺少 k6 脚本、环境、默认请求头和数据集的主链阻断。
- 每个 Agent 现在拿到按自身分片重新编译的负载。固定并发、固定到达率、阶梯并发和阶梯到达率均保持所有分片之和等于全局目标，避免 N 台机器把总压力放大 N 倍。
- 领取阶段会重新核对全局编译哈希、环境服务地址和私有数据集哈希；环境缺少场景所需服务时直接中文阻断，不等 k6 启动后才失败。
- 双节点测试证明私有数据范围不重叠、重复指标不放大总数、HTTP 失败与业务失败分开统计；五节点非均匀容量测试证明 137 VU 和固定数据可完整拆分，节点数量没有写死为两台。
- 节点页增加实时汇总和带文字的心跳灯：在线为脉冲绿灯，离线为红灯，停用为灰色，并继续显示最后心跳时间。桌面、手机和矮屏截图已人工查看，无横向溢出。
- 运维说明已改为 N 节点扩缩：每台机器独立注册即可，增加节点无需修改平台配置；同级节点参与均衡，首选容量足够时不会无意义占用普通或备用节点。
- 性能专项门禁 94 项通过；完整 API 门禁在本批最终代码上为 PostgreSQL 后端 845 项、Vue 75 个文件共 532 项、生产构建、32 张响应式截图和 Chromium 业务闭环 1 项全部通过。后端、前端和 AI 静态检查及差异检查通过。
- 真实 Docker 镜像仍因本机 Docker Desktop 强制 USTC 镜像源 EOF 未进入 Dockerfile 构建阶段；真实一台/N 台 Agent、飞书送达和资源峰值须等用户部署本批代码后继续验收，不能标记为完成。

### 2026-09-03 真实节点校准失败与 Safari 首次试跑点检

- 线上版本为 `a91444900355afd5a763b678b400dd1b5629fab8`。Safari 强制新开页面后登录，真实点击压测节点刷新、专用节点校准、性能场景三步创建、四种负载模型入口、压测执行和性能报告。
- 两台节点心跳正常但均校准失败。接口中的真实错误均为“校准结果缺少迭代率或虚拟用户数”；专用节点点击后约 50 秒回到失败状态，证明命令已被 Agent 领取并执行，不是心跳或机器内存故障。
- 根因是 Agent 按新版 `handleSummary` 的 `metrics.*.values` 读取 k6 0.52 的 `--summary-export`，但 0.52 把 `iterations.rate` 和 `vus_max.max` 直接写在指标对象上。Agent 已升到 `0.1.1`，解析兼容两种结构，并增加 k6 0.52 真实格式回归；失败详情现在会直接显示在节点卡片，不再只给泛化建议。
- Safari 创建并保留了受控临时场景 `Codex--20260903-1953`（`161f2ab5-0e02-4f7c-8f5e-02a779e1a4ab`，版本 `b01be885-980a-4d71-b91e-8db32d207a65`），内容为只读 `GET /user/v1/userModel/active`、cycle 数据、HTTP 200 断言。因 0 个有效校准节点，执行向导正确禁选节点，本轮没有创建执行、没有向生产环境发请求，也没有生成报告或 AI 诊断；场景留作 Agent 更新后的 1 次/秒、10 秒真实闭环，完成后归档。
- 固定并发/阶梯并发原把 `VU × 秒数`展示成预计迭代数，这与 k6 闭合模型实际会按响应速度持续循环的语义不符，可能让新人低估流量。页面已改为明确“实际次数取决于接口响应时间”；固定吞吐/阶梯吞吐才按目标速率展示预计次数，并新增首次试跑建议。
- AI 诊断后端已经强制引用真实 evidence id、限制瓶颈分类、给出置信度和带验证方式的建议；页面原来漏掉瓶颈分类、建议优先级和下一轮参数，现已补齐中文展示。真实 AI 建议质量仍需本批部署、校准和小流量执行完成后复验，不能用组件测试代替。
- Safari 自动化 `typeText` 写中文时仅保留 ASCII 和标点；该方式不经过原生中文输入法，不能作为人工 IME 已修复或仍失败的结论。中文输入专项仍需部署后以真实拼音组合输入复验。

### 2026-09-03 API 性能测试中文使用说明

- 新增 `docs/api-load-testing-user-guide.md` 和排版后的 `docs/API性能测试使用说明.docx`，面向首次接触压测的同事说明压测目的、四个页面入口、完整操作流程、VU/Iteration/RPS、可用容量计算、硬上限/软上限/校准容量、多节点调度、四种负载模型、指标与 AI 诊断边界。
- 文档提供一套可直接执行的最小安全演练：无副作用只读接口、固定吞吐 1 次完整链路/秒、10 秒、低优先级、专用首选节点；要求连通性检查和单链路预检通过后才能启动，并明确生产环境、真实打印/支付/短信/删除/高成本 AI 等风险边界。
- 文档记录 Agent 0.1.1 对 k6 0.52 校准结果解析的修复及排查命令；仍要求部署后重新校准并完成真实执行、报告和 AI 建议复验，未把本地代码测试写成线上验收。
- Word 文档采用 `compact_reference_guide` 设计预设和 `editorial_cover` 首页面式；已渲染为 11 页并逐页检查中文字体、原生编号、表格、公式块、命令块、页眉页脚和长链接，无截断或续号错误。DOCX 压缩包结构检查通过，包含 145 个正文段落和 21 张表格。

### 2026-09-03 本机备用节点最小压测阻断定位与修复

- 线上版本 `da2f20517cdb680ee5e616c49c1c6e524994a01f` 下，Google Chrome 真实选择已校准的“本机（备用）”节点，配置固定吞吐 1 次/秒、预分配 1 VU、10 秒、P95 不高于 1000 毫秒和低优先级。页面点击“创建压测草稿”无任何反馈。
- 服务端复现确认向导把用户可见的 1 VU 隐式编译为 `max_vus=100`，而本机校准容量为 80 VU，创建返回 `409 capacity_unavailable / 压测节点没有剩余容量`；`createRun` 又没有把异常写入 `runError`，视图 catch 后形成假性“按钮没反应”。
- 前端已新增可见的“最大并发（VU）”及中文用途说明，固定/阶梯吞吐的节点容量校验与提交均使用同一个最大并发值，不再隐藏 100 VU 下限；创建失败会原样显示服务端中文原因。组件和 Store 回归共 11 项先失败后通过。
- 为继续验证后端和 Agent，使用同一安全参数经 API 创建执行 `179edbe2-4216-48ed-af8d-63aa65532fbe`。Chrome 真实点击连通性通过；单用户预检返回 HTTP 200、业务码 1001、`404:null`，平台正确阻断，证明原场景依赖的生产 `ZXBToken` 已失效或不适用，未进入正式压测。
- 新建公开只读场景“本机备用节点最小压测-公开只读配置”（场景 `ec4cd083-2205-441c-8920-8ff6a51e5d6a`、版本 `f0eafdef-937d-4457-bdba-804edfbbf008`），请求 `GET /print3d/api/v1/open/webPage`，精确断言 HTTP 200 和 `$.code=0`。执行 `0d4a14ac-fc59-4080-beda-6a6977c77208` 已通过目标连通性及单用户预检。
- 正式启动后执行一直停在 `starting`。平台服务器容器 `load-agent-load-agent-1` 实际为 `Restarting (1)`；日志确认 Agent 领取分片收到 `409`，原因是环境中 9 个与当前场景无关的 Apifox 服务未解析。预检只使用已解析的 `default` 服务并通过，分片下发却错误要求环境内所有服务都必须解析。
- 服务端已改为只检查场景步骤实际引用的服务地址；仍会阻断场景真正需要的缺失服务。Agent 领取分片遇到平台配置类错误时记录中文告警并继续心跳/重试，不再退出形成容器重启循环。PostgreSQL 正反契约和 Agent 存活测试共 3 项通过；完整性能门禁 97 项、API 前端相关 12 项、生产构建、前端静态 84 项、Python 语法和差异检查通过。
- 当前正式执行仍是线上旧代码下的 `starting`，不能记为压测成功。部署本批并重建 Agent 后先停止或恢复该执行，再从页面创建新执行，完成正式运行、确定性报告、AI 诊断和资源峰值复验。

## 2026-09-03 本机备用节点真实压测闭环

- 线上最终验收执行 `734e7699-b64a-424f-81f3-5c92aba720e3`：固定吞吐 1 次/秒、1 VU、10 秒，10 请求/10 链路，HTTP/业务/链路失败均为 0，P95 10 ms，节点正常退出，证据 1/1 完整。
- 实跑先后定位并修复 k6 `??`/可选链语法、VU 状态变量名、控制台非 UTF-8 字节解码、吞吐错误使用编排总时长、历史 `inconclusive` 锁住新报告、五秒图表重复 0 行、节点原始 JSON 和 AI 引用不存在证据等问题。
- 当前代码将 AI 数值与确定性报告分离：诊断结论只允许定性描述；无效证据引用或结论复述数字时降级为低置信度安全建议。专有名词、容量计算、校准、四种模型、指标、报告顺序和渐进压测方法见 `docs/api-load-testing-user-guide.md`。
- 本轮压测专项门禁最新为 104 项通过，前端静态 84 项通过，TypeScript 与生产构建通过。主服务器发布时 RSS 约 132 MB；这不是正式容量数据。
- 仍需：专用服务器 Agent 安装与校准、多节点分片真实压测、固定并发模型、服务端资源采集、飞书性能报告送达、故障/停止链路，以及全平台剩余逐按钮点检。HTTP、受限 Agent 数据权限和公开报告权限边界保持未解决。

## 2026-09-04 两台压测节点扩容策略与容器 CPU 上报修复

- Safari 在线复读确认：专用 2C8G 节点当前仍上报硬/软上限 500 VU、2000 次/秒、CPU 2 核、内存 2048 MB，校准容量 400 VU、2000 次/秒；本机 4C16G 节点仍上报 100 VU、200 次/秒、CPU 4 核、内存 1024 MB，校准容量 80 VU、200 次/秒。两台合计可用容量仍是 480 VU、2200 次/秒，尚未执行新资源参数，不能把建议值写成已生效容量。
- 用户确认专用节点不承担其他业务，本机压测并非每日运行且可作为常规节点。Safari 已把“本机（备用）”调度级别从备用改为普通，页面明确回显成功；名称仍是历史名称，调度行为以当前“普通节点”为准。专用节点继续保持首选，首选容量不足后才使用普通节点，总目标不会因两台参与而翻倍。
- 发现 Agent 0.1.1 的 CPU 硬上限来自 `os.cpu_count()`，Docker 即使设置 `cpus: 1.5` 仍可能按宿主机 4 核上报，导致页面和硬件签名虚高。Agent 0.1.2 改为优先读取 cgroup v2 `cpu.max`，兼容 cgroup v1 quota/period，并以宿主机可见核数封顶；Docker CPU 配额变化会使校准签名变化并要求重新校准。
- 节点页说明不再固定要求平台本机必须设为备用：资源充足且压测期间无其他重任务时可设普通；需要优先保护平台服务时再设备用。
- 推荐下一次升级参数：专用节点使用 2 CPU、6G 容器内存、1000 VU、3000 次/秒、最长 1800 秒；本机使用 3 CPU、8G 容器内存、1000 VU、3000 次/秒、最长 1800 秒。两台均保持单 k6 进程，升级 Agent 0.1.2 后分别重新校准；平台软上限必须等新的硬上限上报后再提高，并以校准结果作为最终可调度容量。
- 本地验证：性能测试门禁 109 项、API 前端 75 个文件 537 项、生产构建、后端静态 63 项、前端静态 84 项、Python/Shell 语法和差异检查通过。线上仍为 Agent 0.1.1，0.1.2 与新容量须由用户部署后复验。

### 2026-09-04 专用节点最小发布包与报告空错误修复

- 专用 Agent 服务器不需要 Git 或完整平台仓库；最小发布包只保留 `load_agent/` 和 `deploy/load-agent/`，解压后从包根目录执行节点页生成的安装命令。Agent 仅发起出站连接，不需要开放入站业务端口。
- 线上真实成功报告的节点错误字段为 `{\"message\": \"\"}`。报告页原按错误对象是否有键判断，导致正常退出的节点仍显示“节点执行失败”；前端现只在非空错误码或错误说明存在时显示错误。
- 该误报修复已用真实字段结构补前端回归。专用节点仍须完成上传、独立令牌注册、心跳、校准和真实小流量执行，不能因发布包生成就记为已部署。

### 2026-09-04 节点校准自动回显与心跳超时

- 线上专用节点虽保留 `online` 状态，但最后心跳为 09:31:59，校准请求于 09:38:50 才创建；平台错误接受了已经停止心跳的旧版 Agent `0.1.0`，导致页面长期停留“校准中”。
- 节点页改为每 3 秒静默刷新心跳、校准结果和容量，校准终态回传后直接更新，无需手工刷新且不会因加载状态清空现有卡片。
- 服务端以 45 秒心跳窗口判定真实在线状态。心跳超时节点在列表中显示离线；未完成校准显示明确失败原因；校准、连通性检查和压测创建不再接受数据库中残留的假在线节点。
- 性能专项 PostgreSQL 契约 105 项、API 前端 75 个文件 537 项、生产构建和前端静态 84 项通过。专用节点必须实际运行 Agent `0.1.1` 后重新校准，旧容器在线或仅上传文件都不算升级完成。
- 专用节点升级后真实日志暴露第二个恢复阻断：新容器上报的某项硬上限低于平台注册时保存的软上限，服务端返回 `平台软上限不能超过节点上报的本机硬上限`，使 Agent 重启循环。心跳现会把旧软上限逐项收紧到新硬上限；仍禁止管理员主动把软上限调到硬上限以上。新增缩容恢复契约后性能专项为 106 项通过。
- 部署 `31e3288` 后线上已确认专用节点恢复：Agent `0.1.1`、k6 `0.52.0`、校准有效，实测容量 400 VU / 2000 次每秒，校准峰值约 204.67% CPU、143.74 MB 内存；软硬上限均收紧为 2 核、2048 MB。`check.sh` 原样输出最近 80 行历史重启日志易被误读，现先输出容器内真实 Agent/k6 版本，并明确旧日志不代表当前失败；默认本地镜像标签同步更新为 `0.1.1`。

### 2026-09-04 脑图正式报告证据门禁与场景扩展

- 线上`合盘打印流程方案`是`mindmap_only`任务：5个需求点生成5条设计（2自动化、3人工），没有生成YAML；自动化和人工去重均为0，因此“只有2条”不是删除造成，而是3条因缺UI/路径/颜色数据/状态同步证据转为人工。
- 报告服务原只按`模块 + YAML文件名`关联Runner索引；该批没有YAML且线上Runner报告索引当前返回0条，所以把2条统一显示“未执行”。这只能说明缺少可关联证据，不能证明用户没有在真机或外部平台执行。
- 报告页现展示本批总数、自动化/人工构成、去重数和执行证据状态；支持逐条或批量补录通过/失败/阻塞，人工补录必须填写执行时间、设备或外部用例记录。证据未闭环时可预览，但正式执行报告按钮禁用且服务端再次拒绝；人工失败/阻塞会真实影响质量评估。
- 报告统计与正文将“未执行”改为“缺少执行证据”，并写入用例构成、关联结果和人工执行依据。旧报告不改写，部署后必须重新生成新报告复验。
- 脑图生成不再只按每个需求点一条正向场景压缩：当脑图模式有4个及以上独立需求点且分析已识别风险时，平台会从已识别风险中确定性补充最多3条异常、边界或状态一致性场景；合盘需求重新生成的目标为8条，Runner自动化候选上限仍为5条。自动化筛选必须把每个场景归入自动化或人工清单，不能静默省略。
- 本地验证：报告服务21项（连同脑图人工模式共25项）、主平台交互25项、前端静态84项、后端静态63项、Python/JavaScript语法和差异检查通过。线上仍是`6984961`，本批待提交、部署和Safari复验。

### 2026-09-04 脑图大列表入口与人工结果报告修复

- 脑图中心加载 37 条历史文件后，CSS Grid 自动行被可滚动容器压缩，页头高度缩到 38px，导致“新建脑图”、刷新和说明区被裁掉。`.generation-records` 现强制自动行按内容高度布局；视觉回归固定生成 37 条数据并校验页头至少 150px、新建入口可见。
- 页面原先在全部用例已人工标记后，仍额外要求“执行依据”文本；后端也执行同样拒绝，所以用户看到已通过仍无法生成报告。现在平台人工标记本身作为有效执行证据；备注改为选填，未填时服务端自动写入“平台人工标记”和报告生成时间。
- 批量标记后立即告知后续状态：全部闭环时显示“✅ 现在可以生成正式测试报告”；仍有未确认用例时显示具体剩余数量。所有自动化用例和已选人工用例仍必须有结论；失败或阻塞可生成真实报告，但质量结论不会被写成通过。
- 本批修改只在本地，待提交、部署后用 Safari 重新打开 37 条脑图列表，批量标记通过并实际生成一份新报告。

### 2026-09-04 性能测试容量配置与四入口体验整改

- 用户确认平台容量策略必须直接在页面配置，不通过服务器配置文件或 SSH。节点页现分开显示校准达到总量与当前可分配总量，逐节点写明 `min(本机硬上限, 平台容量策略, 校准达到值) - 当前占用` 及实际限制来源；管理员可在页面使用保守 25%、标准 60%、专用 80% 预设或六项自定义值，平台持久化并禁止超过 Agent 硬上限。
- 性能场景按当前应用/API 项目和接口版本资产组织，支持编辑并创建不可变新版本、归档且保留历史报告；创建和编辑均有清楚的返回入口。新建执行显示应用、可切换目标环境，并把节点选择放在负载目标之后，提供推荐节点和容量计算依据。
- 执行列表每 3 秒静默刷新，显示应用、环境、中文负载模型和四步证据。连通性/预检/启动/停止点击后立即在卡片内反馈；预检通过显示 `✓`。终态记录不再保留看似可点的无效按钮，改为使用当前场景再次压测、查看报告和删除；运行中删除由后端返回 409，草稿及终态删除会级联清理关联证据。
- 性能报告选择从原生下拉改为可搜索、可按中文状态筛选的历史执行卡片；顶部增加管理层摘要，按负载目标、质量阈值、证据完整性组织，再下钻 RED 指标、时延、阈值、趋势、节点证据、历史对比和 AI 诊断。设计依据已补到中文使用说明。
- Safari 强制刷新线上 `6984961` 并实际点击节点页刷新、注册、关闭和搜索，线上仍是旧版 600 VU 汇总且没有页面容量配置，故本批保持“本地完成、线上待部署”。Safari/CUA 不经过原生中文输入法，不能作为人工 IME 复验依据。
- 本地视觉脚本首次发现 390px 节点页横向溢出 60px，已定位断点覆盖并修复；新增容量弹窗、场景编辑返回、执行列表、新建执行返回的桌面/手机点击和截图，最终全套 39 张响应式视觉门禁通过。本批最终验证为性能专项 109 项、PostgreSQL HTTP 契约 10 项、Vue 75 个文件 543 项、性能页面专项 27 项、生产构建、后端静态 63 项、前端静态 84 项和差异检查全部通过。

### 2026-09-04 压测执行列表与领导版报告改版

- 压测执行改为紧凑证据卡，增加应用/API项目筛选；默认当前应用前8条，可按状态和关键字继续筛选，“显示更多”不再悬浮遮挡卡片。列表仍每3秒静默刷新。
- 跨应用历史执行接口允许不传`project_id`，但始终通过`api.loadtest.view`与资源范围谓词过滤；传项目时仍校验项目访问权。
- 性能报告的历史区默认收起，展开后按应用、中文状态和关键字筛选，首批8条并在限高区域内下钻，大量历史不再把当前报告推出首屏。
- 结合k6/Grafana、Azure Load Testing、BlazeMeter与Gatling的信息结构，顶部新设计深蓝科技风“性能决策简报”，先展示应用、场景、目标压力、性能门禁、证据完整度和上线风险，再展开响应时间、错误、趋势、节点证据和AI建议。确定性结论仍先于AI，不让AI改写平台数值。
- 视觉回归使用2个应用、14条执行记录真实点击筛选、展开、显示更多、新建返回；桌面、平板、手机和短屏无横向溢出，当前视觉脚本全套通过。代码尚未部署，线上仍需Safari强刷复验。
- 最终门禁：PostgreSQL后端858项、Vue 75个文件545项、生产构建、39张响应式截图及1条Chromium完整业务闭环通过；后端静态63项、前端静态84项、Python语法和差异检查通过。已有jsdom跳转提示和主包大小提醒不影响成功结果。

### 2026-09-04 压测小卡片与跨页操作闭环

- Safari 强制打开已部署的 `d1324c0`，真实点击压测执行的立即刷新、关键字/状态筛选、再次压测、四种负载模型、向导返回、报告历史展开/搜索/切换、场景刷新/编辑/归档确认/新建，以及节点刷新/中文搜索/容量预设/注册弹窗；归档只打开确认后取消，未生成令牌、未保存容量、未启动真实压测。
- 点检发现从“再次压测”返回列表后仍残留“请核对目标环境和负载”旧提示；现统一清理向导反馈和场景深链参数。性能场景卡新增“创建压测”，直接带场景进入执行向导，不需要用户再次查找。
- 报告历史切换原只替换页面内容，不同步地址栏 `run_id`，导致复制链接、刷新和浏览器前进后退可能打开错误证据；现切换报告时同步 URL，并监听地址变化恢复对应执行。
- 压测执行卡在 901px 以上改为 `400–520px` 的自动多列布局，2560px 一行四张、1440px 一行两张；场景卡限制为 `420–560px`。900px 以下保持单列，四步证据、状态和操作均完整可见。视觉门禁新增 2560px 宽屏卡片宽度及列数断言，防止退回整行大卡片。
- 本批最终验证：Vue 75 个文件 548 项、生产构建、42 张响应式截图和完整 Chromium 视觉交互通过；前端静态 84 项与 `git diff --check` 通过。构建仍有既存主包超过 500kB 提示；本批待部署后再用 Safari 复验线上宽屏卡片。

### 2026-09-04 性能场景历史脏说明与卡片二次收紧

- 部署 `bfd628a` 后截图确认历史场景 `Codex--20260903-1953` 的说明包含 Markdown 代码围栏和 `AI; 1 VU` 碎片，列表原样展示会误导使用者；场景卡现识别代码围栏并显示“场景说明待补充”，保留编辑入口，不擅自修改或归档历史数据。
- 性能场景卡宽屏上限从 560px 继续收紧到 480px，仍保留完整的创建压测、编辑新版本和归档入口；900px 以下继续单列。
- 新增历史脏说明组件回归及 2560px 卡片宽度视觉断言；Vue 75 个文件 549 项、生产构建、42 张响应式视觉截图、前端静态 84 项和差异检查全部通过，待部署后线上复验。

### 2026-09-07 性能场景真实脏数据修复

- 线上已部署 `c377291`，但真实历史说明并不是测试模拟的三个反引号，而是 Safari 输入转换后保存的 `、、、AI；，1 VU，。`（开头为三个 U+3001 中文顿号）；旧清洗只判断 Markdown 围栏，因此线上未命中。根因是回归数据与真实数据不一致，不能归咎于缓存或部署。
- 场景摘要校验现同时识别连续异常标点前缀、Markdown 围栏和 `AI/数字/VU` 技术碎片，统一提示补充压测目标、压力范围和风险边界；回归测试直接使用线上真实字符。
- 临时场景 `Codex--20260903-1953`（`161f2ab5-0e02-4f7c-8f5e-02a779e1a4ab`）只有一条失败历史执行，已按测试清理边界归档；执行 `179edbe2-4216-48ed-af8d-63aa65532fbe` 仍保留。线上活动场景现只剩正式的“本机备用节点最小压测-公开只读配置”。
- 本批最终验证：Vue 75 个文件 549 项、生产构建、42 张响应式视觉截图、前端静态 84 项和差异检查全部通过；待提交部署后验证代码层清洗，服务端临时场景归档已即时生效。

### 2026-09-07 性能场景卡片操作图标对齐修复

- 部署截图继续暴露场景卡底部操作区问题：`创建压测`、`编辑并创建新版本`、`归档` 的图标和文字被排成上下两行，按钮高度被撑大；这与场景说明数据无关。
- 根因是 `.load-scenario-list article div > *` 作用范围过宽，把操作区按钮也强制改为 Grid。现将选择器收窄到卡片直属标题区，并明确要求操作按钮横向 Flex、图标不收缩、单行文字和 32px 紧凑高度。
- 视觉回归新增操作按钮生产计算样式门禁，逐个校验非 Grid、横向排列、按钮高度和图标垂直位置；修复前会以 41px Grid 按钮失败，修复后桌面、宽屏和手机均通过。

### 2026-09-09 API Worker 中断恢复与家用基线稳定性线上复验

- 线上健康检查确认发布版本为 `438fbac1e69921fad553752d05e33c7a72cd8cf4`。旧执行 `f08274df-b04c-4874-9634-29bf9da879da` 在 Worker 重启后由恢复任务收敛为终态：219 条通过、69 条运行异常、0 条断言失败；已完成结果被保留，未完成用例没有自动重放。
- Safari 实际打开旧执行及中断用例，页面显示“执行 Worker 中断；已完成结果已保留，未完成用例未自动重放”，响应为空、耗时 0 ms，和服务端证据一致，不再长期停在运行中。
- 修复上线后的第一轮完整家用基线 `0f9c838a-c2d4-47dd-9762-f87225d9c67a` 正常结束为 287/288；唯一异常“图生图创建 - 真实图片并取消”不是 Worker 中断，而是业务服务读取 `loremflickr.com` 随机外部图片时超时。
- 将该用例升级为 v4 `74c85970-da3c-4d61-a89f-23fb392da61f`：改用接口文档中现存且可访问的功夫豆 CDN 图片，移除随机外部图片查询，保留创建后按真实任务号取消的清理步骤。v4 在相同生产环境连续 3 次在线调试均为 1/1 通过，随后按平台门禁采纳为正式基线 `bfb8eaba-7194-4f12-a7ee-2ce3c7e5492a`。
- 最终完整回归 `9caee20a-dfa1-42d8-ac43-ce0b6486d21d` 于 17:48:59 开始、17:52:06 结束，288/288 通过、0 失败、0 运行异常、100%，总耗时 187.11 秒。v4 在该轮主体请求与取消清理均返回 HTTP 200。Safari 执行列表、摘要、用例明细和分段日志均完成点检。
- 本轮稳定性问题已关闭。后续多场景性能压测需继续遵守小流量起步、逐级升压和压测节点容量上限，不复用 API 基线的 288 条串行回归作为容量结论。

### 2026-09-09 压测 AI 完整曲线与瓶颈实验设计续接

- 实现及前端构建已通过 `5def91a` 推送 main；本节与证据文档继续明确保留线上待部署边界。
- 进入时本地 main、远端 main 与线上 `/api/health` 均为 `dfa1126a36235c6be58f635a9e3760409be174a6`。已核对前一节 Worker 与 288/288 证据，本轮未重复家用回归。三份工作区 Word 文档及 `docs/career/` 保留。
- Safari 复读阶梯并发 `2e6407f4-aafd-4f2f-8e05-e230b7b33b75`：117 请求/39 链路、4/4 阈值通过，但未证明各阶段达压。阶梯吞吐 `183d044c-b3e6-4149-8827-d909e2cfa58d`：330 请求/110 链路、1/4 阈值通过、P95 1825.316 ms、HTTP/业务/链路失败率 0.30%/0.30%/0.91%。两份 v6 AI 分析都已完成，旧“人工配置”不代表完整下一轮建议能力完成。
- 报告中的服务监控版本为 `f411ab83-ce40-413a-a851-3ec395e47c88`，Prometheus 来源 `172.22.0.1:19090`，`/load-demo` 与 `demo:8080` 标签属于演示容器自身 cgroup。两轮服务 CPU/内存各有 17 样本；仅本机单节点，不能替代演示双节点证据。历史演示目标 `43.157.42.206` 的实时隧道/配额仍需服务器现场核对；NAT `106.54.190.138` 不能误作演示目标。
- 新代码区分执行历时与用例累计耗时，缺少时间戳不再用累计耗时冒充；v7 AI 建议保留起点、分钟单位、完整升压/保持/恢复曲线及 VU 预算。结构或引用错误先纠正一次，超时或再次失败展示规则备用；合法建议由平台按当前证据校验，越界保存原提案与调整原因，不自动发压。
- 下一轮草稿绑定来源执行/诊断，并保留原场景、环境、阈值、监控及停止策略。修复独立审查发现的回退绕过禁用状态、真实 Prometheus 标签误判缺监控、目标达到却未完成观察、历史缺失测试条件被默认值覆盖、观察计划偷偷改压等问题。
- 报告和导出新增九类瓶颈证据覆盖及补采建议；它是覆盖清单，不是九类根因已定位。完整设计见 `docs/superpowers/specs/2026-09-09-load-ai-closed-loop-design.md`：先找到最后稳定—首次持续失败区间，再用单变量实验确认限制因素；GC/连接池/SQL 锁等待/trace、跨轮实验与资源自动停止尚待建设。
- 本地验证：最终 PostgreSQL 压测专项 147 项通过；Vue 88 文件 636 项通过；生产构建、后端静态 63 项、前端静态 84 项、AI Gateway 静态 46 项、Python 语法与差异检查通过。扩大后端全量时先修正隔离身份库环境；1272 passed、2 skipped 后唯一失败是旧超时状态断言，已更新并在最终 147 项中复验通过。
- **未完成**：新版部署与 Safari 报告→完整建议→草稿→预检→执行闭环；演示双节点、HTTP/业务故障切换、人工与保护停止、长保持/恢复和瓶颈触达实验。本轮没有新启动压测、没有修改线上配置或发通知。直连平台 SSH 22 超时，Safari 堡垒机终端会话失效；已请求恢复可用会话。恢复后在 `/opt/midscene-task-platform-src` 执行既有 `bash deploy/update-main-server.sh`，确认健康检查发布版本后继续。完整记录见 `docs/evidence/load-ai-acceptance-2026-09-09.md`。

### 2026-09-10 部署后 Safari 压测、停止证据与实时链路修复

- 用户确认部署后，本地/远端 main 与线上健康版本均核实为 `89befcce082858675bf43761906c6d8c9edb8acf`，入口 JS/CSS 与本地构建逐字节一致。没有重跑 288/288 基线；三份 Word 与 `docs/career/` 保留。
- Safari 在“压测演示（非业务）/隔离演示环境 v4”完成三轮。原阶梯并发复验 `d2d00833-843e-44fb-89b8-5e4d99edd890`：1→2→4→1、各 15 秒，96 HTTP/32 链路，三类失败率 0，P95 1187.36 ms≤1200，达压、4/4 阈值通过，监控补齐后 passed。
- 新单接口 CPU 场景 `de374014-e20a-4715-92d6-189c3063f023`，版本 `6b5a56b1-faa0-4816-84ee-6a5e224c3d3f`，HTTP 200 与 $.code==0 双断言。固定吞吐 `19893aba-801c-4ad6-81df-6e3bbbcbf353`：2 次/秒×180 秒，360/360 链路、错误 0，但 P95 1142.547 ms>500，最终 failed；服务 CPU 0.22 核、内存 25.75 MiB，各 61 样本。只能证明端到端延迟不达标，尚未证明 CPU 饱和或定位根因。
- 人工停止 `841b9d0d-6161-4922-9bb9-313fe0de57b5`：2 VU/计划120秒，实际并发采样 30.999 秒，58 请求、14 指标窗口保留；节点 cancelled、退出码105和interrupt日志，报告inconclusive。服务监控完成，各32样本。09:02:56两台节点均0进程/0VU。
- 双节点草稿 `eb8dbd31-a7c4-451f-95ec-2f9e758d7ae3` 在全局2次/秒、30秒下检查：本机可达172.22.0.1:18080，专用节点TCP失败，正确阻止预检/启动。未放宽目标保护；不能算双节点通过。监控来源172.22.0.1:19090、/load-demo、自身cgroup标签；历史演示服务器43.157.42.206与实际隧道仍需服务器终端核验。
- 旧阶梯报告v7重新分析 `5d4e28c4-be72-4b2f-aaa7-0676753d1384` 因不存在的证据引用回退为规则备用，但原完整曲线成功贯通下一轮差异/草稿/门禁/执行；不能把它写成AI提案通过。停止轮v7诊断已完成且“AI建议已通过校验”，保留2VU/120秒、暂不升压；通用线程池/下游建议仍需架构约束，不能视为根因已证实。
- 本轮代码修复提交 `53d2c88`：执行卡正确显示完整阶梯曲线、总时长、零起点和分钟单位；SSE与JSON事件按内容协商分流、禁代理缓冲；10秒无事件降级串行轮询；旧ticket/事件/报告与卸载刷新隔离；JSON UUID/sequence游标适配、has_more末页补取、终态报告不提前切断事件；首次报告不重复建连接。
- 最终验证：Vue 88文件645项、PostgreSQL HTTP与load HTTP75项、生产构建、后端静态63、前端静态84、Python语法与diff检查通过；独立复核无未解决P1/P2。测试使用临时身份库。构建和源码随修复提交，线上仍为89befcc，53d2c88尚待部署后Safari复验。
- 后续仍待：双节点路径修复、阶梯吞吐原失败复验、可控HTTP/业务故障与保护停止、瓶颈触达及恢复实验。09:03后Safari完整重开报告出现 `The string did not match the expected pattern.`，重试会话仍失败，健康接口正常；不作已修复声明。堡垒机仍未进入可用终端，已请求恢复会话，不请求密码令牌。完整数据与下一步单变量实验见 `docs/evidence/load-live-acceptance-2026-09-10.md`。

### 2026-09-10 09:30 后 Safari 部署复核、隧道延迟与 AI 阶梯契约补正

- 用户恢复平台堡垒机后，Safari 执行既有部署脚本，03bdcea（含53d2c88）部署成功，健康/登录/资源检查通过；执行列表已真实显示完整阶梯曲线与45秒总时长。强制重载后Safari会话正常，先前pattern异常未复现但根因未定。两节点0进程/0VU。
- 用户截图确认演示机43.157.42.206、2核2GB；平台机10.130.4.31上的SSH进程现场确认18080/19090转发到该演示机回环端口。平台隧道健康检查5次HTTP200，total0.869–1.185秒；SSH TCP RTT279.113ms。端到端高延迟已有链路证据，服务本地耗时和容器实际配额仍需演示机终端；不能断言CPU瓶颈已触达。
- 补读最终AI：CPU稳态f0a967e66f7b为AI已调整（300秒建议调整回原180秒）；新阶梯b7b421eb5d79因缺完整workload回退。v7首个JSON示例缺workload，与正文矛盾；新增v8完整曲线示例和四模型字段/纠错契约，AI/策略PostgreSQL88项通过。v8尚待本次提交后部署和真实重新诊断，不能将本地测试当AI输出通过。
- 双节点、阶梯吞吐失败复验、受控故障与保护停止、瓶颈触达/恢复及新事件链路仍未全部验收。详见docs/evidence/load-live-acceptance-2026-09-10.md。Word和晋升资料继续保留。

### 2026-09-10 09:39 后阶梯 AI 契约线上通过与复验入口收口

- c98cc3a在Safari部署成功。d2d00833报告同证据b7b421eb5d79重新诊断得到v8完整1→2→4→1、15/15/15秒曲线，状态AI建议已通过校验；诊断为证据不足。自由文本仍有不符合演示架构的DB/池/OOM建议，未执行，不能把参数通过视为所有根因/实验通过。
- 修复终态监控完成后AI不自动刷新的页面问题；增加独立轻量AI轮询，终态即停；诊断世代隔离覆盖同run重新诊断前和POST期间旧GET迟到，POST切页/卸载均隔离。独立复核无剩余P1/P2。
- next-run-policy.v3：缺少增长目标仍可按原配置创建复验草稿，增长或延时继续拒绝，完整曲线/阈值/监控保留。AI/策略90项与前端报告17项通过；本次后续部署和Safari新执行结果见证据文档。

### 2026-09-21 执行手机实时画面与占用状态同步

- Agent 工作台的手机卡片支持勾选固定设备；“刷新设备”同时向空闲 Windows Runner 请求一次 ADB 实时截图，并同步电量、温度、亮/息屏、前台应用和 Sonic 当前占用状态。截图成功后替换默认手机图，失败保留上一张并显示中文原因。
- Sonic 状态按 ONLINE=空闲、DEBUGGING/TESTING=占用、其他=待确认处理；占用或待确认设备不能手工勾选，也不会进入自动分配。平台 pending/running 任务优先标记占用。
- 截图请求有 75 秒有效期，设备离线、任务开始或状态变忙时取消；服务端保存前再次检查任务，避免采集执行中画面。截图只接受 Runner 机器凭据、服务端生成的同源地址和不超过 12 MiB 的 PNG；心跳不能注入外部截图地址。
...
### 2026-09-21 任务结束后设备状态误报修复

- Sonic 实时设备列表显示手机空闲，平台卡片却短暂显示“设备状态采集失败”。根因是平台任务执行期间主动跳过截图所保存的说明，在任务结束后被错当成真实采集故障。
- `/api/runners` 现将“设备正在执行平台任务”识别为正常延后采集：任务完成后清除过期说明，并以 Sonic 当前空闲/占用状态为准；真实超时、ADB 或 Runner 错误仍显示采集失败。
- 页面可见且停留在 Agent 工作台或执行页时，每 10 秒轻量同步一次 Runner/Sonic 状态，更新空闲、占用人、电量和温度；页面转入后台或离开相关页时不发请求，回到页面立即同步。实时截图仍仅由“刷新设备”触发，避免频繁 ADB 截图干扰手机。
- YAML 编辑中心区支持在整个编辑面板上使用鼠标滚轮或触控板双指上下滑动，不必先把光标移到文本中；行号与文本同步滚动，到达顶部/底部后不锁死外层页面。
- 回归覆盖主动跳过、任务结束后清理过期说明和真实采集异常；Python 17 项、后端静态 63 项、前端静态 84 项、语法和差异检查通过。
# 2026-09-21 Sonic 原生操作录制生成 YAML

- 新增“操作录制”工作流：平台创建录制会话后打开 Sonic 原生设备中心，Sonic 继续负责画面和触控；浏览器旁路钩子先发送原控制消息，再异步镜像动作。平台不代理视频流，Windows Runner 不重复点击、滑动或输入。
- 首次发布后页面误把业务打印机编号 `9888E0094F2A` 当作 Sonic 手机，用户在页面验收时及时指出。本轮已纠正设备域：录制页只展示 Runner 实际在线上报的 Android 手机，由用户明确勾选；刷新不自动切换，后端再次校验手机确实属于该在线 Runner。`9888E0094F2A` 与 `18CEDF5BA7B2` 均被录制服务明确拒绝，继续仅用于各自业务设备历史语境。
- Windows Runner 只在录制动作后采集一张 PNG 关键帧和一次 UI XML，上传结果具备幂等处理。坐标只用于匹配页面节点，不写入最终 YAML；无法识别控件时必须在时间线补充中文控件说明。
- 录制结束后可添加检查点、确认歧义步骤、生成 YAML，并复用现有静态校验和可执行性评分；生成结果可保存到当前应用已经关联的模块。存在问题时明确标记为草稿，不允许当成可调试结果。
- 本地专项验证：Python 18 项通过，Node 3 项通过；`py_compile`、前端静态检查 84 项、后端静态检查 63 项和 `git diff --check` 均通过。Windows 整合包已重建，SHA-256 为 `bb01c3821f5a9f07eff610acff2de69a2772a9d1b742e585c80e34052b97db07`。
- 仍需完成线上发布证据：发布设备域纠正提交、替换并重启 Windows Runner、Safari 由用户明确选择一台空闲手机，完成安全只读录制到 YAML 保存的全链路验收。
- 设备域纠正提交 `91c1fe2` 已推送并部署，公网健康接口精确返回 `91c1fe2c1f43a26548faa1db3ebcdd38656ffa7d`。跨域预检从 Sonic `http://101.34.197.12:3000` 到录制动作接口返回 200，并明确允许该 Origin。
- Sonic Web 容器已在主 bundle 之前加载 `/sonic-recorder-hook.js`，线上脚本可访问且 SHA-256 为 `0d038492a8d6a1840d971012af0df52a7db69bc5f12972329f0f87035d13d151`。该容器没有宿主机挂载，脚本在当前容器重启时保留，但日后重新创建容器时需按 `deploy/sonic-recorder-hook.md` 重新注入。
- Safari 真实页面已确认“操作录制”入口和纠正后的手机卡片：荣耀 `ATLSVB3C23002676`、华为 `UQG0220513008845` 可选；OPPO `ecbfd645` 显示 `wangwc 占用中`且不可选；页面没有再显示打印机 `9888E0094F2A`。按用户要求手机必须由用户本人勾选，本轮没有替用户选择或自动切换，因此尚未创建录制会话、操作手机或生成线上 YAML。
- Windows Runner 仍需替换为本提交的 `recording-evidence-v1` 脚本并重启服务，随后才能完成关键帧/UI XML证据和最终全链路验收；当前不得把“页面已展示手机卡片”写成录制功能最终通过。

### 2026-09-21 Sonic 录制流程闭环修正（待部署验收）

- 线上截图复核确认首版流程不合格：平台先选手机、进入 Sonic 又选一次；会话仍显示等待桥接且 0 个动作时可以结束并生成仅含 `launch` 的空 YAML；页面缺少返回入口、手机状态色、步骤删除和通用编辑，用例名称不联动文件名，应用模块因前端目录缓存缺失可能显示空白。
- 录制手机现改为只在 Sonic 选择一次。平台仅展示手机在线/占用状态；Sonic 建立真实 Android 控制连接后回传设备编号，平台校验在线设备并绑定会话，再回执允许记录。绑定前动作不上传，另一手机的控制连接不会混入当前会话。
- 空会话和只有检查点的会话不能结束成有效录制，也不能生成 YAML；可明确取消本次录制。页面增加返回用例资产、红绿状态点、折叠的操作说明编辑/删除、删除后重排编号、用例名称到 YAML 文件名联动，以及生成失败的可见提示。
- 保存模块严格来自被测应用的已关联模块，但不再因当前 `modules` 前端缓存缺少条目而隐藏已保存关联；应用只决定 YAML 启动包和模块范围，与手机分配互不绑定。
- 本地验证：录制服务/YAML/路由/Runner 相关 Python 31 项，录制页面与 Sonic hook Node 7 项，前端静态 84 项、后端静态 63 项、Python/JavaScript 语法和 `git diff --check` 通过。全量 `visual_smoke_check.js` 被既有冷启动 Agent 应用名称断言阻断，与本轮录制页面无关；不得据此宣称线上录制已通过。

### 2026-09-22 Chrome 录制握手复验与加载竞态修复

- Safari 中 Sonic 远控画面出现大面积灰块，而同一手机、同一页面在 Chrome 可完整显示，现场证据确认这是 Sonic 远控的浏览器兼容问题，不是 Windows Agent 截图组件故障。录制验收改为平台与 Sonic 都使用 Chrome；平台其他 Safari 验收结论不外推到 Sonic 视频流。
...
### 2026-09-22 Chrome 真实录制、刷新恢复与 YAML 生成验收

- `7b014e8` 和 `9eb4c7b` 已推送并经华为堡垒机部署，公网健康版本最终核对为 `9eb4c7bc0acdeb307c3ab257b077658c815b9a63`。Sonic 新开远控页及站内路由都会保留独立 `midsceneRecorder` 缓存键；短期会话片段仅用于首个新页，令牌不进入查询参数。
- Chrome 新会话从平台默认应用“智小白3D”开始，在 Sonic 选择 PHM110 后，远控页显示“录制中，已同步 0 步”，平台绑定 `ecbfd645` 且确认真实点击前画面 ready。刷新远控页回到设备中心后，缓存键和录制任务仍保留；重新进入同一手机后再次显示“录制中，可以开始操作”。
- 真实点击一次后，Sonic 先显示“第 1 步已收到，正在采集截图并识别”，再显示“第 1 步记录成功”。平台时间线保存点击截图、原始坐标 `1001,176`、红点拖动、重新识别、重置和手工编辑入口；识别结果为资源标识 `com.heytap.browser:id/epm`。
- 结束录制后生成 YAML 成功，包含 `launch: com.kfb.model` 与 `aiTap: epm`，基础校验无 issue。该单点击测试缺少等待和断言，可执行性评分 25/draft，未保存到用例资产，也未当成完整业务用例通过。
- 在线 Windows Runner 为 `recording-evidence-v2`、Midscene `1.13.0`、模型族 `qwen3`；关键帧和步骤确认链路本轮可用，但仓库脚本当前为 `recording-evidence-v3`，后续替换 Windows 文件并重启后仍应复验版本一致性。
- 录制页与 Sonic 钩子 Node 34 项、前端静态 84 项、后端静态 63 项、JavaScript 语法和差异检查通过。测试会话及此前四条测试会话均已通过 API 删除，最终 GET 返回 404；未保存测试 YAML，关闭远控页后 Sonic 明确显示 PHM110 空闲。

# 2026-09-23 Chrome 录制现场复验：旧截图复用与视觉提示词偏置

- 用户完成 Windows Sonic Agent 替换后，Chrome 新录制会话绑定 PHM110 (`ecbfd645`)。平台与 Sonic Web 钩子均为线上 `37031ec` 版本，Agent 在线。Sonic 远控默认选中 Minicap；切至 Scrcpy 后画面可正常显示。
- 实际点击验证发现：极短浏览器点击被旁路录到，但手机未跳转；约 350ms 按下才进入“打印记录”页。该项先作为真实触控时长差异记录，不把收到 WebSocket 动作当成导航成功。
- 一次在证据尚未 ready 时执行的“打印记录”点击确实使手机跳转，但没有入库。随后重新进入同一手机，后端仍复用更早的“我的”页 `pre_action_frame`；点击打印记录中的“删除”后，手机弹出正确确认框，平台保存的步骤截图却仍是“我的”页，并错误识别为“底部导航「我的」”。这证明了缓存证据过期与固定示例词偏置，而非点击位置坐标错误。
- 本地修复：Sonic 远控新连接和未记录动作后向 bridge 明确请求更新当前手机截图/UI XML，旧帧立即进入 pending 并拒绝下一步入库；桥接旧响应不能覆盖新刷新状态。正在录制且已经绑定手机的会话可在 2 小时令牌有效期内刷新页面后重连。视觉识别提示词移除固定“我的”答案示例。
- 回归先红后绿：录制后端 26 项、Sonic 钩子 17 项、录制前端 21 项、Windows 证据 5 项通过；后端静态 63、前端静态 84、Python/JS 语法与 `git diff --check` 通过。现场错误录制会话 `840c7af9e23741d39de96dc4243dedb9` 已取消并删除，确认原打印记录删除弹窗点的是“取消”，未删手机数据。新修复尚待发布和完整业务实录/YAML 回放验收。

## 2026-09-23 部署后 PHM110 复验：转场帧、图片字段与时间线截图

- `5901591` 已由用户部署，线上健康版本和 Sonic hook SHA-256 与本地一致。Chrome 在 PHM110 上真实点击左上角返回与底部首页均使智小白3D 页面跳转；平台也收到了两个动作。第二步的前置截图却是“我的”与“打印记录”的转场混合画面，Android UI XML 同时采集失败，视觉识别因而无法命名首页。第一步 XML 命中的是 `android.widget.Image`，`text` 为 PNG Base64 尾段 `Fn+i0op0v4AAAAAElFTkSuQmCC`，被错误显示为控件名。录制页选中步骤图片的授权接口返回 200/774692 字节，但轮询重绘替换 `<img>` 后丢失异步加载结果。
- 本地修复：录制动作后等待 1.5 秒再向 Windows Runner 派发下一张前置截图请求，避免触控尚在执行或页面转场时采集；过滤伪 Base64 控件名，历史记录和 YAML 生成也拒绝此类名字；视觉模型同时接收整屏图和用红色十字标出点击点的 600px 局部图；时间线截图按会话与步骤缓存，加载结束后挂到当前 DOM 节点。Pillow 加入服务依赖，由现有部署脚本安装。
- 用户授权清理的两条现场测试录制 `914a0cf182134b609eba60dac8227cf0`、`dcb0bed3a6fe4164a60b27504ba4725b` 均经 API 删除。手机打印记录没有在本轮删除。单元回归与静态检查见本次提交；新修复发布后仍需 Chrome 完整实录、生成 YAML、Runner 回放与应用目标页核对，发布前不能声称闭环已通过。

## 2026-09-23 部署后 Chrome 实录发现 Sonic 底栏返回漏录

- `e625eff` 已部署，线上 Pillow 为 12.3.0。Chrome 新会话 `2f7870b9a2ac42eeb45fc181e911922d` 在 PHM110 (`ecbfd645`) 实录：点击“我的”与“打印记录”均实际跳转、截图可显示，平台语义正确；点击测试记录 `ep3pro-offset测试` 的删除及确认后，手机列表实际移除该记录，平台相应保存“删除”“确认”。
- Sonic 底栏返回让手机回到“我的”，但平台未记录。再点击手机画面内“首页”后，平台沿用返回前“打印记录”截图，把“首页”错识别成“打印评价”；因此这次录制不可用于直接回放。查线上 Sonic 2.7.2 远控 bundle，底栏发送 `{type:"keyEvent",detail:4}`，现有 hook 只识别 `{type:"text",detail:"CODE_AC_BACK"}`。漏录动作未触发下一张前置截图刷新，造成后续截图过期。
- 本地补齐 Sonic `keyEvent` 4/3/66 到录制 `BACK`/`HOME`/`ENTER` 的镜像映射，不改 Sonic 原生发送顺序。回归测试先红后绿；Sonic hook 18 项、前端静态 84 项、后端静态 63 项、Python 与 JS 语法、`git diff --check` 通过。仍须部署新 hook 后重开会话，实录返回与首页，生成 YAML 并完成 Runner 回放；测试会话和产物验收后清理。

## 2026-09-23 Chrome 录制完整路径复验与结束会话清理

- `29b7ad0` 已经由华为堡垒机部署到服务器，公网健康版本和 Sonic Web hook SHA 与本地一致。PHM110 `ecbfd645` 的 `wangwc 占用中`来自旧远控标签；关闭该标签后设备恢复空闲，新的 Chrome 远控会话可绑定手机。
- 新录制 `0c0bccd7399541f3952075592da6ca5c` 实际完成“智小白3D → 我的 → 打印记录 → 删除 → 确认 → 返回 → 首页”，手机在前两次点击、返回和首页时确实跳转；平台准确保存 `我的`、`打印记录`、`删除`、`确认`、`BACK`、`首页`。一次长按造成的额外 `待识别`步骤已从时间线删除。结束后生成 YAML，包含 `launch: com.kfb.model`、四个 `aiTap`、`runAdbShell: input keyevent 4`、`aiTap: 首页`，基础校验通过。
- 删除确认虽在截图上精准点中蓝色“确认”，远控画面当时仍显示弹窗，不能仅凭“识别 100%”与 YAML 静态通过认定删除成功；这份 YAML 未保存、未跑 Runner 回放。结束录制后再点“我的”，远控画面仍显示首页并提示“第 8 步同步失败”。后续刷新远控才发现视频帧时钟滞留在 22:01，而手机实时已是 22:30 的“我的”页；因此先前“未跳转”和“未删除”均只能表述为画面未更新，需新会话核对手机数据。已确认的独立问题是结束录制后平台没有向 Sonic hook 发停录信号，旧会话仍残留，后续点击继续尝试上传到已结束的会话。
- 本地修复让结束/取消录制向 Sonic 标签发送 `MIDSCENE_RECORDING_STOP`，钩子清理会话和计时器，但保留 Sonic 原生 WebSocket 发送；刷新页面遇到已结束会话的桥接 409 或认证 401 也停止重试，并更新前端缓存键。录制页与 hook 共 44 项 Node 测试先红后绿，前端静态 84、后端静态 63、Python 与 JS 语法、差异检查通过。尚需发布此修复后真实核对停录后的手机点击、测试记录删除结果，再决定是否生成可回放用例；不宣称全链路验收通过。

## 2026-09-24 Chrome 实录回放发现 Windows CLI 依赖损坏

- 用户部署 `8a0c093` 后，Chrome 新录制 `c0ba80982d1c4d4b8ff1628097a9a437` 在 PHM110 手机（序列号 `ecbfd645`）上实际完成“智小白3D → 我的 → 打印记录 → 返回 → 首页”。四次点击都核对了手机目标页，平台记录的文本分别为“我的”“打印记录”“返回按钮”“首页”；录制正常结束，生成 YAML 含 `launch: com.kfb.model` 和四个 `aiTap`。本次没有删除手机打印记录，因为此前用于测试的记录已经删除，剩余记录不应当作一次性测试数据。
