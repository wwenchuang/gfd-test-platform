...
_DOMAINS = (
    ('cpu', 'CPU', {'cpu_percent', 'cpu_cores'},
     ...
     '补采服务 CPU 配额、节流与步骤耗时；保持流量和数据，仅调整一个已授权的 CPU 配额条件，比较性能拐点。'),
    ('memory', '内存', {'memory_percent', 'memory_working_set_bytes'},
     '尚未获得有效内存采样。',
     '内存占用不包含 GC 暂停、分配速率或泄漏证据，缓存增长不能直接视为泄漏。',
     '补采 GC、内存配额与恢复趋势；保持流量和数据，仅延长稳态观察，比较增长和降压后回落。'),
    ('application_pool', '应用排队与连接池', set(),
     '未配置线程池、连接池等待或应用队列采集。', '',
     '采集活跃数、上限、等待时长与超时；保持流量，仅调整一个已授权的池参数，核对等待和业务耗时是否同步变化。'),
    ('database', '数据库', {'postgres_connections', 'postgres_commits_per_second', 'postgres_rollbacks_per_second'},
     ...
     '先确认数据库依赖，补采 SQL 指纹耗时、锁等待和调用关联；保持流量与数据分布，仅验证一个经审阅的查询改动。'),
    ('storage', '磁盘与存储', {'disk_read_bytes_per_second', 'disk_write_bytes_per_second', 'disk_read_iops', 'disk_write_iops', 'disk_read_latency_ms', 'disk_write_latency_ms'},
     ...
     '补采磁盘队列、IO 等待与服务归属；保持流量，仅改变一个测试工作集，比较存储等待与业务耗时。'),
    ('network', '网络', {'network_receive_bytes_per_second', 'network_transmit_bytes_per_second'},
     '未配置有效网络采样。',
     '主机收发速率不包含重传、往返延迟或带宽上限，不能证明网络没有瓶颈。',
     '补采重传、往返延迟、带宽上限与请求连接耗时；保持服务和流量，仅改变一个已授权的发压路径进行比较。'),
    ('downstream', '下游依赖', set(),
     '未配置下游调用链或依赖服务指标；接口耗时无法单独归因到下游。', '',
     '确认依赖并采集调用耗时、错误和限流；在隔离环境保持流量，仅替换一个已授权的下游响应条件进行证伪。'),
    ('load_generator', '压力机', set(),
     '尚未获得有效压力机运行资源采样。', '',
     '核对实际压力、丢弃迭代、节点配额和运行资源；保持全局目标，仅调整发压节点或 VU 预算，比较达压情况。'),
    ('test_data', '测试数据', set(),
     '尚未获得有效步骤业务断言证据。', '',
     '补充账号分配、数据分布与资源清理证据；保持压力，仅改变一个数据分布条件，比较业务失败和热点等待。'),
)
