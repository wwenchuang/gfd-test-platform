    ...
    def diagnose_validation_error(
        ...
        *,
        ...
        diagnosis = (analyzer or AiFailureAnalyzer()).analyze({
            ...
            "request": (
                "请结合当前接口合同，用中文解释原因，明确指出要修改的字段或断言并给出可执行步骤；"
                "不得建议绕过平台校验，不得编造或建议写死 token、设备号、请求 ID 等运行时值。"
                "固定环境值应改为 {{变量名}}；动态业务值应说明需调用哪类前置接口、"
                "从响应的哪类字段提取变量，再传入主体请求；若当前合同无法确定上游，必须明确说明需人工选择上游接口。"
            ),
        })
