...
def positive_overlay_evidence(text):
    ...
    result = []
    positive_patterns = (
        r"(?:权限|授权|系统|引导)?(?:弹窗|对话框)(?:出现|弹出|遮挡|挡住)",
        r"(?:弹出|出现)[^\n]{0,80}(?:权限|授权|相机|存储|文件)[^\n]{0,40}(?:弹窗|对话框)",
        r"温馨提示[^\n]{0,80}(?:权限|授权|相机)[^\n]{0,80}(?:弹窗|对话框)",
        r"(?:截图|画面|页面|界面)?[^\n]{0,40}(?:弹窗|对话框)(?:底部|顶部|内部|内|中)[^\n]{0,80}(?:按钮|只有|包含|显示|可见)",
        r"(?:权限|授权|系统|相机|存储|文件)?(?:请求)?(?:弹窗|对话框)[，,、\s]{0,4}[^\n]{0,80}(?:包含|显示|可见|只有)[^\n]{0,80}(?:按钮|确定|确认|允许|取消)",
        r"(?:浮层|蒙层|引导层)(?:出现|遮挡|挡住)",
        r"(?:按钮|入口|页面|控件|业务入口)被(?:弹窗|对话框|浮层|蒙层|引导)遮挡",
    ...
    negated_terms = (
        ...
        "未被遮挡", "没有遮挡", "无浮层", "未出现浮层", "弹窗未出现",
        "无对话框", "未出现对话框", "没有对话框", "不存在对话框", "对话框未出现",
        "not blocked",
