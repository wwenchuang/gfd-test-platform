...
def tool_save_repair_draft(run, inp):
    ...
    try:
        ...
        return {"ok": True, "draftId": saved["draftId"], "status": saved["status"]}
    except ValueError as exc:
        return {"ok": False, "error": str(exc)}


def tool_apply_repair_after_confirm(run, inp):
    """AI tool input cannot prove a human confirmed a formal YAML replacement."""
    return {
        "ok": False,
        "draftId": inp.get("draftId") or inp.get("draft_id") or "",
        "error": "请在 Runner 的待我处理列表查看草稿并人工确认替换；AI 工具不能代替人工确认覆盖正式 YAML。",
    }
...
def _tool_generate_repair(run, failed_jobs_override=None):
    ...
    try:
        ...
        for index, target_job in enumerate(repair_targets, start=1):
            ...
            try:
                if repair_service and draft.get("status") == "REJECTED" and draft.get("repairSource") == "diagnosis_only":
                    # This rejection is generated by the platform gates above,
                    # never by AI tool input or an HTTP-supplied outcome.
                    saved = repair_service._upsert_repair_draft(draft)
                else:
