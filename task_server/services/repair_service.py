...
24. 报告关键帧若明确显示同级入口行在屏幕边缘被裁切，应在失败等待前使用一次官方 aiScroll。原 YAML 没有横滑时只插入一次；已有横滑时只能替换原动作，禁止追加第二次。区域必须用当前页真实可见文案描述，并明确从内容区中部起手、避开屏幕左右边缘；使用 `scrollType: "singleAction"` + `direction: "right"` + 不超过 400 的 `distance`，滑动后重新等待目标。禁止坐标、ADB swipe、整页盲滑和方向互相矛盾的描述。
