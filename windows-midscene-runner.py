...
def android_sdk_root_from_adb(adb_path=""):
    ...
    for raw in candidates:
        try:
            ...
            if resolved.parent.name.lower() == "platform-tools":
...
def ensure_android_sdk_env(env):
    ...
    if platform_tools and platform_tools.lower() not in path_value.lower():
...
def resolve_command(command, label):
    ...
    if resolved:
        return resolved
    raise RuntimeError(f"找不到 {label} 命令：{command}。请检查环境变量或 PATH")
...
def unique_existing_paths(paths):
    ...
    seen = set()
    for item in paths:
        path = clean_command(str(item or ""))
        if not path:
            continue
...
def adb_candidates():
    candidates = []
    if ADB_BIN and clean_command(ADB_BIN).lower() != "adb":
    ...
    candidates.extend([
        ...
        r"D:\sonic\platform-tools\adb.exe",
    ])
...
def resolve_adb_with_devices(require_devices=True):
    ...
    if _ADB_BIN_CACHE and Path(_ADB_BIN_CACHE).exists():
        ...
        run_cmd([_ADB_BIN_CACHE, "start-server"], timeout=20)
    ...
    for candidate in candidates:
        try:
            run_cmd([candidate, "start-server"], timeout=20)
    ...
    details = "; ".join([
        f"{item.get('adb')}: devices={item.get('devices', [])}, unauthorized={item.get('unauthorized', [])}, offline={item.get('offline', [])}, error={item.get('error') or item.get('stderr') or ''}, stdout={item.get('stdout', '')}"
...
def detect_device_ids():
    ...
    if not devices:
        raise RuntimeError("未检测到可用 Android 设备，请检查 adb devices")
    return devices
...
def detect_devices():
    ...
    try:
        ...
        if now - _LAST_NO_DEVICE_LOG_AT >= ADB_DEVICE_CHECK_INTERVAL:
            ...
            print(r'提示：如果 cmd 能识别手机，可以执行 set "ADB_BIN=C:\Program Files\platform-tools\adb.exe"；也可以临时指定 set "DEVICE_ID=你的设备序列号"。')
...
def execute_midscene(job_id, job_dir, yaml_path, task_names, device_id):
    ...
    def emit_progress(force=False, message="执行中"):
        ...
        progress = 5
        if total:
            progress = min(95, max(5, round((completed / total) * 90) + 5))
        post_job_progress(job_id, {
            "progress": progress,
...
def run_job(job):
    ...
    for index, device_id in enumerate(device_ids, start=1):
        ...
        print(f"Trying device {device_id} for {job_id}")
        try:
    ...
    summary_path = latest_file(report_root, "summary-*.json")
    if not summary_path:
        summary_path = latest_file(report_root, "summary*.json")
