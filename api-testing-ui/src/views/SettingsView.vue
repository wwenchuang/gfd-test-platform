...
import { Check, KeyRound, Plus, Save, Trash2 } from 'lucide-vue-next'
...
<template>
  <section class="workspace setup-page">
    ...
    <p v-if="localError || setup.error" class="inline-error" role="alert">{{ localError || setup.error }}</p><p v-if="setup.message" class="setup-success"><Check :size="16" />{{ setup.message }}</p>
  </section>
</template>
