...
describe('LoadAiAnalysis', () => {
  it('shows evidence citations, low confidence and explains reanalysis does not rerun load', async () => {
    const analysis = { id: 'a1', run_id: 'r1', model: 'qwen-plus', prompt_version: 'v1', evidence_hash: 'abcdef1234567890', state: 'completed', error: '', created_at: '', result: { conclusion: '瓶颈在目标服务', evidence: ['latency.summary', 'agent.a1'], confidence: { level: 'low', reason: '窗口较少' }, recommendations: [{ priority: 'high', action: '检查慢查询', verification: '复跑相同负载' }] } }
