...
<template>
  <section class="case-evidence">
    ...
    <details open><summary>响应明细</summary><pre>{{ responseBlock.text }}</pre><div v-if="responseBlock.oversized" class="evidence-preview-actions"><span>响应共 {{ responseBlock.length.toLocaleString('zh-CN') }} 个字符</span><button data-testid="expand-response-evidence" class="text-command" type="button" @click="toggleBlock('response')">{{ responseBlock.expanded ? '恢复精简预览' : '显示完整响应' }}</button></div></details>
