...
const analysisStatus = computed(() => String(result.value.analysis_status || ''))
const fallbackAdvice = computed(() => analysisStatus.value
  ? analysisStatus.value === 'rule_fallback'
  : /回退|兜底/.test(confidence.value?.reason || ''))
...
<template>
  <section class="load-ai-panel" aria-label="AI性能诊断">
    <header><div><h2>AI 诊断</h2><p>AI 基于本轮证据提出诊断与下一轮验证方案；重新诊断不会重新执行压测。</p></div><button data-testid="load-reanalyze" class="secondary-command" type="button" :disabled="loading" @click="emit('reanalyze')">{{ loading ? '正在提交…' : analysis ? '重新诊断' : '开始诊断' }}</button></header>
    <p v-if="!analysis" class="compact-empty">确定性报告生成后会自动排队诊断，也可以手动重新诊断。</p>
    <template v-else>
      <div class="load-ai-meta"><span>{{ analysisStatus === 'rule_fallback' || fallbackAdvice ? '平台规则建议' : stateLabel }}</span><span>模型：{{ analysis.model }}</span><span>提示词：{{ analysis.prompt_version }}</span><span>证据：{{ analysis.evidence_hash.slice(0, 12) }}</span></div>
