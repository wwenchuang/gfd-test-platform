...
<template>
  <section class="task-status-strip" aria-label="当前测试任务">
    ...
    <div class="task-fact"><span>状态</span><strong :class="task ? `task-state-${task.state}` : ''">{{ task ? stateLabels[task.state] || task.state : '待创建' }}</strong></div>
    <div class="task-actions">
      ...
      <button data-testid="run-task" class="primary-command" type="button" :disabled="running || !task || ['designing','debugging','running'].includes(task.state)" @click="$emit('run')"><Play :size="15" />{{ running ? '创建执行中' : '执行本任务' }}</button>
    </div>
  </section>
</template>
