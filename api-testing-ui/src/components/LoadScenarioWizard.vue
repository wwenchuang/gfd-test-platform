...
const props = defineProps<{ endpoints: ApiEndpoint[]; initialDefinition?: LoadScenarioDefinition | null; projectName?: string; projectId?: string }>()
...
const query = ref('')
const preservedDefinition = ref<LoadScenarioDefinition | null>(props.initialDefinition ? JSON.parse(JSON.stringify(props.initialDefinition)) : null)
...
const canNext = computed(() => step.value === 1 ? Boolean(name.value.trim() && (preservedDefinition.value?.steps.length || selected.value.length)) : !advancedPending.value)
const definition = computed<LoadScenarioDefinition>(() => buildDefinition())
...
function buildDefinition(): LoadScenarioDefinition {
  if (preservedDefinition.value) return {
    ...JSON.parse(JSON.stringify(preservedDefinition.value)), name: name.value.trim(), description: description.value.trim(), mode: mode.value,
    dataset_contract: { ...preservedDefinition.value.dataset_contract, variables: [...preservedDefinition.value.dataset_contract.variables], usage_mode: datasetMode.value },
  }
  return {
...
function applyDefinition(value: LoadScenarioDefinition): void {
  preservedDefinition.value = JSON.parse(JSON.stringify(value))
  name.value = value.name; description.value = value.description; mode.value = value.mode; datasetMode.value = value.dataset_contract.usage_mode
}
function save(): void { if (!advancedPending.value) emit('save', buildDefinition()) }
</script>
...
<template>
  <section class="load-wizard" aria-label="创建性能场景">
    ...
    <div v-if="step === 1" class="load-wizard-body">
      ...
      <div v-if="preservedDefinition" class="load-review-box"><strong>已保留 {{ preservedDefinition.steps.length }} 个完整步骤</strong><p>下一步可编辑原有请求、断言、变量和数据集绑定，即使原始接口资产已经变化，也可继续创建新版本。</p><p v-for="item in preservedDefinition.steps" :key="item.id">{{ item.name }} · {{ item.request?.method }} {{ item.request?.path }}</p></div>
      <template v-else><div class="load-option-grid"><button data-testid="scenario-mode-single" :class="{ active: mode === 'single_interface' }" type="button" @click="setMode('single_interface')"><strong>单接口压测</strong><span>每轮只请求一个接口，适合测接口容量。</span></button><button data-testid="scenario-mode-workflow" :class="{ active: mode === 'workflow' }" type="button" @click="setMode('workflow')"><strong>业务链路压测</strong><span>按顺序执行多个接口，吞吐指完整链路次数。</span></button></div>
      ...
      <p v-if="warning" class="load-warning">写接口必须说明资源归属并配置清理步骤；请在下一步完整配置中补齐，再交由服务端校验。</p></template>
    </div>
    <div v-show="step === 2" class="load-wizard-body">
      <h3>数据取用方式</h3>
      ...
      <div class="load-review-box"><strong>业务校验</strong><p>新接口默认校验 HTTP 状态 200，已有场景保留原断言。HTTP 200 不代表业务成功；请在保存前按真实响应补充业务码、布尔值或领域字段断言。</p></div>
      <LoadScenarioAdvancedEditor :model-value="definition" :project-id="projectId" @update:model-value="applyDefinition" @pending="advancedPending = $event" />
    </div>
    <div v-if="step === 3" class="load-wizard-body"><h3>保存前确认</h3><div class="load-review-box"><strong>{{ name }}</strong><p>{{ mode === 'single_interface' ? '单接口压测' : '业务链路压测' }} · {{ definition.steps.length }} 个步骤 · {{ datasetMode }}</p><p v-for="item in definition.steps" :key="item.id">{{ item.request?.method }} {{ item.request?.path }} · {{ item.assertions.length }} 条断言</p><p>数据集：{{ definition.dataset_contract.dataset_id || '未绑定' }} · {{ definition.dataset_contract.variables.length }} 个声明字段</p></div><p>保存会创建不可变场景版本，并由服务端再次执行安全校验。校验失败会保留问题和处理办法，不会进入压测。</p></div>
    <footer><button v-if="step > 1" data-testid="scenario-back" class="secondary-command" type="button" @click="step--">上一步</button><span /><button v-if="step < 3" data-testid="scenario-next" class="primary-command" type="button" :disabled="!canNext" @click="step++">下一步</button><button v-else data-testid="scenario-save" class="primary-command" type="button" :disabled="advancedPending || (warning && !preservedDefinition)" :title="warning && !preservedDefinition ? '写操作必须先配置资源归属与清理步骤' : ''" @click="save">保存并校验版本</button></footer>
  </section>
</template>
